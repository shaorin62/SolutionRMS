<%@ Page CodeBehind="MDCMCLOUDTAX.aspx.vb" Language="vb" AutoEventWireup="false" Inherits="MD.MDCMCLOUDTAX" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<HEAD>
		<title>CGV클라우드 세금계산서 발행</title> 
		<!--
'****************************************************************************************
'시스템구분 : CGV클라우드 세금계산서 발행(MDCMCLOUDTAX.aspx)
'실행  환경 : ASP.NET, VB.NET, COM+ 
'프로그램명 : MDCMCATVTRUTAX.aspx
'기      능 : CGV클라우드 세금계산서 발행
'파라  메터 : 
'특이  사항 : 
'----------------------------------------------------------------------------------------
...
Sub imgPrint_onclick ()
	...
	IF frmThis.sprSht.MaxRows = 0 then
		...
		with frmThis
			...
			'md_trans_temp삭제 시작
			intRtn = mobjMDCMCLOUDTAX.DeleteRtn_TEMP(gstrConfigXml)
			'md_trans_temp삭제 끝
			...
			for i=1 to .sprSht.MaxRows
				IF mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",i) = "1" THEN
					...
					vntDataTemp = mobjMDCMCLOUDTAX.ProcessRtn_TEMP(gstrConfigXml,strTAXYEARMON, strTAXNO, VATFLAG, FLAG, i, strUSERID)
				END IF
			next
	...
	with frmThis
		...
		Else
			If .rdT.checked = True Then
				...
				gShowModalWindow "../MDOT/MDCMCLOUDTAXDTL.aspx",vntInParams , 898,680
				gFlowWait meWAIT_ON
				SelectRtn
				gFlowWait meWAIT_OFF
			End IF
...
Sub InitPage()
	'서버업무객체 생성	
	set mobjMDCMCLOUDTAX	= gCreateRemoteObject("cMDOT.ccMDOTCLOUDTAX")
...
Sub SelectRtn ()
	...
	with frmThis
		...
		'세금계산서 완료조회
		If .rdT.checked = True Then
			vntData = mobjMDCMCLOUDTAX.Get_CLOUD_TAX(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON, strCLIENTCODE, strFROM, strTO)
			If not gDoErrorRtn ("Get_CLOUD_TAX") then
				'조회한 데이터를 바인딩
		...
		ElseIf .rdF.checked = True Then			
			vntData = mobjMDCMCLOUDTAX.Get_CLOUD_TAXBUILD(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON, strCLIENTCODE, strFROM, strTO)
			If not gDoErrorRtn ("Get_CLOUD_TAXBUILD") then
				'조회한 데이터를 바인딩
				call mobjSCGLSpr.SetClipBinding (frmThis.sprSht,vntData,1,1,mlngColCnt,mlngRowCnt,True)
				'초기 상태로 설정
				mobjSCGLSpr.SetFlag  frmThis.sprSht,meCLS_FLAG
		...
		ElseIf .rdA.checked = True Then
			vntData = mobjMDCMCLOUDTAX.Get_CLOUD_TAXALL(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON, strCLIENTCODE, strFROM, strTO)
			If not gDoErrorRtn ("Get_CLOUD_TAXALL") then
				'초기 상태로 설정
				mobjSCGLSpr.SetFlag  frmThis.sprSht,meCLS_FLAG
...
Sub ProcessRtn ()
	...
	with frmThis
		...
		intTAXNO = 0
		If .cmbGUBUN.value = "taxdiv" Then
		intRtn = mobjMDCMCLOUDTAX.ProcessRtn_Div(gstrConfigXml,vntData, intTAXNO)
		Else
			...
			Else
				...
				Next
				'맥스값만 추가하여 보내기
				intRtn = mobjMDCMCLOUDTAX.ProcessRtn_Group(gstrConfigXml,vntData, intTAXNO,intColFlag)
			End IF
		End If
	...
	with frmThis
		...
		for i = .sprSht.MaxRows to 1 step -1
			if mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",i) = 1 THEN
				...
				intRtn = mobjMDCMCLOUDTAX.DeleteRtn_CommiTax(gstrConfigXml,strTAXYEARMON, strTAXNO)
