...
<HTML>
	<HEAD>
		<title>옥외 세금계산서 발행</title> 
		<!--
'****************************************************************************************
'시스템구분 : MD/케이블 위수탁 세금계산서 발행(MDCMCATVTRUTAX.aspx)
'실행  환경 : ASP.NET, VB.NET, COM+ 
'프로그램명 : MDCMCATVTRUTAX.aspx
'기      능 : 케이블 위수탁 세금계산서 발행
'파라  메터 : 
'특이  사항 : 
'----------------------------------------------------------------------------------------
		...
		If .cmbGUBUN.value = "taxdiv" Then
			For intCnt = 1 To .sprSht.MaxRows
				If  mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",intCnt) = 1 Then
					...
					mobjSCGLSpr.setTextBinding .sprSht,"SUMM",intCnt,"옥외광고비 - (" & mobjSCGLSpr.GetTextBinding(.sprSht,"MEDNAME",intCnt) & ")"
				End If
			Next
		Elseif  .cmbGUBUN.value = "taxgroup" Then
			For intCnt = 1 To .sprSht.MaxRows
				If  mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",intCnt) = 1 Then
					mobjSCGLSpr.setTextBinding .sprSht,"DEMANDDAY",intCnt,strDEMANDDAY
					mobjSCGLSpr.setTextBinding .sprSht,"TAXYEARMON",intCnt,strTAXYEARMON
					mobjSCGLSpr.setTextBinding .sprSht,"SUMM",intCnt,"옥외광고비"
				End If
			Next
		End If
	End With
...
Sub Grid_Setting (strGUBUN)
	With frmThis
		...
		Else
			...
			mobjSCGLSpr.SetHeader .sprSht,		 "선택|거래번호|청구년월|광고주|광고주사업자번호|팀|외주처|외주처사업자번호|외주처|금액|부가세|합계|내수율|내수액|적요|발행일|담당부서|광고주대표자명|광고주주소1|광고주주소2|매체사대표자명|매체사주소1|매체사주소2|전표번호|세금계산서년월|세금계산서번호|거래명세서년월|거래명세서번호|거래명세서순번|신탁년월|신탁순번|광고주코드|광고주AC코드|매체사코드|매체사AC코드|매체코드|부서코드|매체구분|순위|전표구분"
