...
Sub imgModifyDelete_onclick ()
	If frmThis.sprSht.MaxRows = 0 then
		gErrorMsgBox "수정삭제할 데이터가 없습니다.","처리안내!"
		Exit Sub
	End If
	
	If frmThis.rdT.checked <> true then
		gErrorMsgBox "수정삭제는 완료상태일때 가능합니다..","처리안내!"
		Exit Sub
	end if
...
Sub TAXPOP
	...
	Dim strYEARMON
	with frmThis
		vntInParams = array(.txtTRANSYEARMON1.value, .txtTRANSNO1.value,.txtCLIENTCODE1.value,.txtCLIENTNAME1.value, "trans","OUTDOOR") '<< 받아오는경우
...
Sub TRANSPOP
	...
	End If
	'msgbox strYEARMON
		vntInParams = array(strYEARMON, .txtTRANSNO1.value,.txtCLIENTCODE1.value,.txtCLIENTNAME1.value, "trans","OUTDOOR") '<< 받아오는경우
		vntRet = gShowModalWindow("../MDCO/MDCMTRANSPOP.aspx",vntInParams , 413,435)
		if isArray(vntRet) then
			.txtTRANSYEARMON1.value = vntRet(0,0)  ' Code값 저장
			.txtTRANSNO1.value = vntRet(1,0)  ' 코드명 표시
...
Sub InitPage()
    ...
    With frmThis
		...
		mobjSCGLSpr.SetHeader .sprSht,		   "선택|년월|번호|관리번호|년월|번호|순번|청구일|광고주명|광고주사업자등록번호|매체사명|매체사사업자등록번호|매체명|대행금액|부가세액|합계금액|수수료율|수수료|적요|부서명|발행일|광고주코드|광고주AC코드|매체사코드|매체사AC코드|MEDCODE|부서코드|집계구분|전표번호|순위|소재명|협찬구분|CLIENTOWNER| CLIENTADDR1| CLIENTADDR2| REAL_MEDOWNER| REAL_MEDADDR1| REAL_MEDADDR2"
...
Sub DeleteRtn_Modify ()
	...
	strDESCRIPTION = ""
		...
		intRtn = gYesNoMsgbox("자료를 수정삭제하시겠습니까?","자료삭제 확인")
		IF intRtn <> vbYes then exit Sub
		intCnt = 0
												...
												<TABLE id="tblButton" style="HEIGHT: 20px" cellSpacing="0" cellPadding="2" border="0">
													<TR>
														...
														<td><IMG id="imgModifyDelete" onmouseover="JavaScript:this.src='../../../images/imgModifyDeleteOn.gif'"
																...
																height="20" alt="자료를 수정/삭제합니다." src="../../../images/imgModifyDelete.gIF" border="0"
