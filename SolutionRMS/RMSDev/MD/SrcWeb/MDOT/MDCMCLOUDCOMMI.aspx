<%@ Page Language="vb" AutoEventWireup="false" Codebehind="MDCMCLOUDCOMMI.aspx.vb" Inherits="MD.MDCMCLOUDCOMMI" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<HEAD>
		<title>CGV 클라우드 거래명세서</title>
		<meta content="False" name="vs_snapToGrid">
		<META http-equiv="Content-Type" content="text/html; charset=ks_c_5601-1987">
		<!--
'****************************************************************************************
'시스템구분 : 위수탁거래명세서 등록 화면(MDCMCLOUDCOMMI.aspx)
...
Dim mobjMDOTCLOUDCOMMI, mobjMDCOGET, mobjSCCOGET
...
Sub imgPrint_onclick ()
	...
	With frmThis
		...
		'md_trans_temp삭제 시작
		intRtn = mobjMDOTCLOUDCOMMI.DeleteRtn_temp(gstrConfigXml)
		'md_trans_temp삭제 끝
		
		ModuleDir = "MD"
		ReportName = "MDCMCLOUDCOMMI_NEW.rpt"
		...
		strUSERID = ""
		vntData = mobjMDOTCLOUDCOMMI.ProcessRtn_TEMP(gstrConfigXml,strTRANSYEARMON, strTRANSNO, strCNT, strUSERID)
...
Sub ProcessRtn_Confirm_User ()
	...
	with frmThis
		...
		IF intSaveRtn = vbYes then 
			...
			intRtn = mobjMDOTCLOUDCOMMI.ProcessRtn_Confirm_User(gstrConfigXml,vntData,.txtEMPNO.value)
	...
	With frmThis
		If mstrGrid Then
			If .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"AMT") or .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"VAT") OR _
				.sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"CGV_AMT") or .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"TIM_AMT") OR _
				...
				ElseIF .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"VAT") Then
					strCOLUMN = "VAT"
				ElseIF .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"CGV_AMT") Then
					strCOLUMN = "CGV_AMT"
				ElseIF .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"MC_AMT") Then
					strCOLUMN = "MC_AMT"
				ElseIF .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"TIM_AMT") Then
					strCOLUMN = "TIM_AMT"
				...
				FOR i = 0 To intSelCnt -1
					If vntData_col(i) <> "" and (vntData_col(i) = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"AMT")) OR _
												...
												(vntData_col(i) = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"CGV_AMT"))OR _ 
		...
		Else
			If .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"AMT") or .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"VAT") OR _
			   .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"CGV_AMT") OR .sprSht_DTL.ActiveCol = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"MC_AMT") OR _
	...
	With frmThis
		If mstrGrid Then
			If Col = mobjSCGLSpr.CnvtDataField(.sprSht_DTL,"PRINT_SEQ") Then 
				...
				intRtn = mobjMDOTCLOUDCOMMI.UPDATE_PRINTSEQ(gstrConfigXml,strTRANSYEARMON,strTRANSNO, strSEQ, strPRINT_SEQ)
			End If
		End If
	End With
End Sub
...
Sub InitPage()
	...
	Dim intNo,i
	'서버업무객체 생성	
	set mobjMDOTCLOUDCOMMI  = gCreateRemoteObject("cMDOT.ccMDOTCLOUDCOMMI")
    ...
    With frmThis
		...
		mobjSCGLSpr.SpreadDataField .sprSht_HDR, "CHK | REQUESTGBN | CONFIRMGBN | CONFIRMFLAG | TRANSYEARMON | TRANSNO | CLIENTNAME | DEMANDDAY | PRINTDAY | AMT | VAT | SUMAMTVAT | REQUEST_USER | REQUEST_DATE | CONFIRM_USER | CONFIRM_DATE | MED_FLAGNAME | MEMO | CNT"
		mobjSCGLSpr.SetHeader .sprSht_HDR,		  "선택|요청|승인|계산서|거래년월|번호|광고주|청구일|발행일|공급가액|부가세|계|요청자|요청일|승인자|승인일|매체구분|비고|상세행수"
		mobjSCGLSpr.SetColWidth .sprSht_HDR, "-1", "  4|   4|   4|     6|       8|   5|    15|     8|     8|      10|    10|10|    10|     9|    10|     9|       8|  15|      7"
		...
		mobjSCGLSpr.SetCellTypeEdit2 .sprSht_HDR, "REQUESTGBN | CONFIRMGBN | CONFIRMFLAG | TRANSYEARMON | CLIENTNAME | CONFIRM_USER | REQUEST_USER | MED_FLAGNAME | MEMO", -1, -1, 200
		mobjSCGLSpr.SetCellsLock2 .sprSht_HDR, True, "REQUESTGBN | CONFIRMGBN | CONFIRMFLAG | TRANSYEARMON | TRANSNO | CLIENTNAME | DEMANDDAY | PRINTDAY | AMT | VAT | SUMAMTVAT | REQUEST_USER | CONFIRM_USER | MED_FLAGNAME | MEMO | CNT"
...
Sub EndPage()
	set mobjMDOTCLOUDCOMMI = Nothing
...
Sub Grid_Setting ()
	With frmThis
		...
		If mstrGrid Then
			...
			mobjSCGLSpr.SpreadDataField .sprSht_DTL, "TRANSYEARMON | TRANSNO | SEQ | PRINT_SEQ | TRUST_SEQ | CLIENTNAME | AMT | VAT | SUMAMTVAT | MC_AMT | TIM_AMT | CGV_AMT | DEMANDDAY | MEMO | TAXYEARMON | TAXNO | CONFIRMFLAG | TRUST_YEARMON"
			mobjSCGLSpr.SetHeader .sprSht_DTL,		"거래명세년월|거래명세번호|순번|순서|신탁순번|광고주|월금액|부가세|계|MC금액|사내수익|CGV금액|청구일자|비고|계산서년월|계산서번호|확정구분|신탁년월" 
			mobjSCGLSpr.SetColWidth .sprSht_DTL, "-1", "        9|	         5|	  5|   4|       4|	  15|    10|    10|10|    10|      10|     10|       8|  15| 	      9|         9|       9|      5"
			...
			mobjSCGLSpr.SetCellTypeFloat2 .sprSht_DTL, "TRANSNO | SEQ | PRINT_SEQ | TRUST_SEQ | AMT | SUMAMTVAT | MC_AMT | TIM_AMT | CGV_AMT | VAT", -1, -1, 0
			mobjSCGLSpr.SetCellTypeEdit2 .sprSht_DTL, "TRANSYEARMON | CLIENTNAME | MEMO | TAXYEARMON | CONFIRMFLAG | TRUST_YEARMON", -1, -1, 100
			mobjSCGLSpr.SetCellsLock2 .sprSht_DTL, False, "TRANSYEARMON | TRANSNO | SEQ | PRINT_SEQ | TRUST_SEQ | CLIENTNAME | AMT | CGV_AMT | VAT | SUMAMTVAT | DEMANDDAY | MEMO | TAXYEARMON | TAXNO | CONFIRMFLAG | TRUST_YEARMON" 
			mobjSCGLSpr.SetCellsLock2 .sprSht_DTL, True, "TRANSYEARMON | TRANSNO | SEQ | PRINT_SEQ | TRUST_SEQ | CLIENTNAME | AMT | CGV_AMT | VAT | SUMAMTVAT | DEMANDDAY | MEMO | TAXYEARMON | TAXNO | CONFIRMFLAG | TRUST_YEARMON" 
			mobjSCGLSpr.ColHidden .sprSht_DTL, "TRANSYEARMON | TRANSNO | SEQ | PRINT_SEQ | TRUST_SEQ | CLIENTNAME | AMT | CGV_AMT | VAT | DEMANDDAY | MEMO | TAXYEARMON | TAXNO | CONFIRMFLAG | TRUST_YEARMON", False
		...
		Else
			...
			mobjSCGLSpr.SpreadDataField .sprSht_DTL,   "CHK | YEARMON | SEQ | DEMANDDAY | CONT_CODE | CONT_NAME | CLIENTCODE | CLIENTNAME | DEPT_CD | DEPT_NAME | AMT | VAT | SUMAMTVAT | MC_AMT | TIM_AMT | CGV_AMT | MEMO | COMMI_TAX_FLAG | TRANSCUSTRANK"
			mobjSCGLSpr.SetHeader .sprSht_DTL,		   "선택|년월|순번|청구일자|계약코드|계약명|광고주코드|광고주명|부서코드|부서명|월청구액|부가세|계|MC매출|사내수익|CGV비용|비고|COMMI_TAX_FLAG|TRANSCUSTRANK"
			...
			mobjSCGLSpr.SetCellTypeDate2 .sprSht_DTL, "DEMANDDAY", -1, -1, 10
			mobjSCGLSpr.SetCellTypeFloat2 .sprSht_DTL, "SEQ | AMT | VAT | SUMAMTVAT | MC_AMT | TIM_AMT | CGV_AMT", -1, -1, 0
			...
			mobjSCGLSpr.SetCellsLock2 .sprSht_DTL, False, "CHK | YEARMON | SEQ | DEMANDDAY | CONT_CODE | CONT_NAME | CLIENTCODE | CLIENTNAME | DEPT_CD | DEPT_NAME | AMT | VAT | SUMAMTVAT | MC_AMT | TIM_AMT | CGV_AMT | MEMO | COMMI_TAX_FLAG | TRANSCUSTRANK" 
			mobjSCGLSpr.SetCellsLock2 .sprSht_DTL, True, "YEARMON | SEQ | DEMANDDAY | CONT_CODE | CONT_NAME | CLIENTCODE | CLIENTNAME | DEPT_CD | DEPT_NAME | AMT | SUMAMTVAT | MC_AMT | TIM_AMT | CGV_AMT | MEMO | COMMI_TAX_FLAG | TRANSCUSTRANK" 
			mobjSCGLSpr.ColHidden .sprSht_DTL, "CHK | YEARMON | SEQ | DEMANDDAY | CONT_CODE | CONT_NAME | CLIENTCODE | CLIENTNAME | DEPT_CD | DEPT_NAME | AMT | VAT | SUMAMTVAT | MC_AMT | TIM_AMT | CGV_AMT | MEMO | COMMI_TAX_FLAG | TRANSCUSTRANK", False
...
Sub ProcessRtn_CUST ()
	...
	With frmThis
		...
		'On error resume next
		'쉬트의 변경된 데이터만 가져온다.
		vntData = mobjSCGLSpr.GetDataRows(.sprSht_DTL,"CHK | YEARMON | SEQ | DEMANDDAY | CONT_CODE | CONT_NAME | CLIENTCODE | CLIENTNAME | DEPT_CD | DEPT_NAME | AMT | VAT | SUMAMTVAT | MC_AMT | TIM_AMT | CGV_AMT | MEMO | COMMI_TAX_FLAG | TRANSCUSTRANK")
		...
		intRtn = mobjMDOTCLOUDCOMMI.ProcessRtn_CUST(gstrConfigXml,strMasterData,vntData,strTRANSYEARMON,intColFlag)
...
Sub SelectRtn ()
	...
	With frmThis
		...
		vntData = mobjMDOTCLOUDCOMMI.SelectRtn_HDR(gstrConfigXml,mlngRowCnt,mlngColCnt, _
		...
		mlngColCnt=clng(0)
   		...
   		vntData2 = mobjMDOTCLOUDCOMMI.SelectRtn(gstrConfigXml,mlngRowCnt,mlngColCnt, _
												strYEARMON, strCLIENTCODE)
...
Sub SelectRtn_DTL (Col, Row)
	...
	With frmThis
		...
		vntData = mobjMDOTCLOUDCOMMI.SelectRtn_DTL(gstrConfigXml,mlngRowCnt,mlngColCnt, _
	...
	With frmThis
		...
		For i = 1 To .sprSht_HDR.MaxRows
			If mobjSCGLSpr.GetTextBinding(.sprSht_HDR,"CHK",i) = 1 Then
				...
				vntData = mobjMDOTCLOUDCOMMI.DeleteRtn_Check(gstrConfigXml,mlngRowCnt,mlngColCnt, strTRANSYEARMON, strTRANSNO) 
				If mlngRowCnt > 0 Then
		...
		intRtn = mobjMDOTCLOUDCOMMI.DeleteRtn(gstrConfigXml,vntData)
