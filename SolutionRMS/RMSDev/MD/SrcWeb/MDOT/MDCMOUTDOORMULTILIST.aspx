<%@ Page Language="vb" AutoEventWireup="false" Codebehind="MDCMOUTDOORMULTILIST.aspx.vb" Inherits="MD.MDCMOUTDOORMULTILIST" %>
...
Sub CLIENTCODE_POP
	...
	'InitPage
	With frmThis
		vntInParams = array(trim(.txtYEAR.value), "OUTDOOR") '<< 받아오는경우
...
Sub InitPage()
	...
	'서버업무객체 생성
	set mobjMDSROUTDOORMULTILIST	= gCreateRemoteObject("cMDSC.ccMDSCPRINTMULTILIST")
	...
	'pnlTabSum.style.visibility = "visible"  
...
Sub EndPage()
	set mobjMDSROUTDOORMULTILIST = Nothing
	gEndPage	
End Sub
...
Sub SelectRtn ()
	...
	With frmThis
		...
		mvntData2 = mobjMDSROUTDOORMULTILIST.GetOUTDOORMEDCNT(gstrConfigXml,mlngRowCnt,mlngColCnt,.txtYEAR.value, strClientAndMed(1), strClientAndMed(2))
		mintCnt2 = mlngRowCnt
		mlngRowCnt=clng(0)
		mlngColCnt=clng(0)
		vntData = mobjMDSROUTDOORMULTILIST.SelectRtn_OUTDOORCUSTAndMED(gstrConfigXml,mlngRowCnt,mlngColCnt, mintCnt2, mvntDataCust, mvntDataCustCNT, .txtYEAR.value, strClientAndMed(1), strClientAndMed(2))
		
		If not gDoErrorRtn ("SelectRtn_OUTDOORCUSTAndMED") then
...
Sub SetChangeLayout () 
	...
	With frmThis
		...
		mvntDataCust = mobjMDSROUTDOORMULTILIST.GetOUTDOORCLIENTCNT(gstrConfigXml,mlngRowCnt,mlngColCnt,strYEAR, strClientAndMed(1), strClientAndMed(2))
		
		mvntDataCustCNT = mlngRowCnt
		If not gDoErrorRtn ("GetOUTDOORCLIENTCNT") then
			If mlngRowCnt > 0 Then 
				'필드 고정값세팅
				Dim strField
				strField = "YEAR|CUST"
