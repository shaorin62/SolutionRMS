<%@ Page Language="vb" AutoEventWireup="false" Codebehind="MDCMOUTDOORCOMMI.aspx.vb" Inherits="MD.MDCMOUTDOORCOMMI" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<HEAD>
		<title>옥외광고 수수료거래명세표 생성 및 발행</title>
...
Sub REAL_MED_CODE_POP
	...
	with frmThis
		...
		vntInParams = array(.txtTRANSYEARMON.value, .txtREAL_MED_CODE.value, .txtREAL_MED_NAME1.value, "commi","OUTDOOR", strSPONSOR)  
...
Sub txtREAL_MED_NAME1_onkeydown
	if window.event.keyCode = meEnter then
		...
		with frmThis
			...
			vntData = mobjMDCMGET.GetTRANSCUSTNO(gstrConfigXml,mlngRowCnt,mlngColCnt,.txtTRANSYEARMON.value, .txtTRANSNO.value,.txtREAL_MED_NAME1.value,"ALL","commi", "OUTDOOR", strSPONSOR)
...
Sub txtTRANSNO_onkeydown
	if window.event.keyCode = meEnter then
		...
		with frmThis
			...
			vntData = mobjMDCMGET.GetTRANSNO(gstrConfigXml,mlngRowCnt,mlngColCnt,strTRANSYEARMON, .txtTRANSNO.value,.txtREAL_MED_CODE.value,.txtREAL_MED_NAME1.value, "commi", "OUTDOOR", "0")
...
Sub TRU_POP
	...
	with frmThis
		...
		vntInParams = array(strTRANSYEARMON, .txtTRANSNO.value,.txtREAL_MED_CODE.value,.txtREAL_MED_NAME1.value, "commi", "OUTDOOR") '<< 받아오는경우
...
Sub ProcessRtn ()
	...
	with frmThis
		...
		intRtn = mobjMDCMOUTDOORCOMMI.ProcessRtn(gstrConfigXml,strMasterData,vntData, intCOMMINO,strCOMMIYEARMON, intColFlag1)
...
'		END IF
		...
		ELSE
			...
			vntData1 = mobjMDCMOUTDOORCOMMI.SelectRtn_SUSU(gstrConfigXml,mlngRowCnt,mlngColCnt,strTRANSYEARMON,strREAL_MED_CODE, strSPONSOR)
...
Function SelectRtn_HDR (ByVal strYEARMON, ByVal strTRANSNO, ByVal strREAL_MED_CODE)
	...
	vntData = mobjMDCMOUTDOORCOMMI.Get_OUTDOORCOMMI_HDR(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON,strTRANSNO, strREAL_MED_CODE)
	
	IF not gDoErrorRtn ("Get_OUTDOORCOMMI_HDR") then
		IF mlngRowCnt<=0 then
...
Function SelectRtn_DTL (ByVal strYEARMON,ByVal strTRANSNO, ByVal strREAL_MED_CODE)
	...
	vntData = mobjMDCMOUTDOORCOMMI.Get_OUTDOORCOMMI_LIST(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON,strTRANSNO, strREAL_MED_CODE)
	
	IF not gDoErrorRtn ("Get_OUTDOORCOMMI_LIST") then
		'조회한 데이터를 바인딩
								...
								<TR>
									<TD style="WIDTH: 300px" align="left" width="427" height="28">
										<table cellSpacing="0" cellPadding="0" width="100%" border="0">
											...
											<tr>
												<td class="TITLE">옥외광고&nbsp;수수료거래명세 생성및 발행</td>
											</tr>
										</table>
									</TD>
