<%@ Page Language="vb" AutoEventWireup="false" Codebehind="MDCMPOINTADVOCH.aspx.vb" Inherits="MD.MDCMPOINTADVOCH" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<HEAD>
		<title>포인트 친구 AD 전표생성</title>
		<META http-equiv="Content-Type" content="text/html; charset=ks_c_5601-1987">
		<!--
'****************************************************************************************
'시스템구분 : 포인트 친구 AD 전표생성 화면(TRLNREGMGMT0)
'실행  환경 : ASP.NET, VB.NET, COM+ 
'프로그램명 : MDCMPOINTADVOCH.aspx
'기      능 : 포인트 친구 AD 전표생성
'파라  메터 : 
'특이  사항 : 
'----------------------------------------------------------------------------------------
...
Sub btnTab1_onclick
	...
	pnlTab_KAKAO.style.visibility = "hidden" 
...
Sub btnTab3_onclick
	...
	pnlTab_susu.style.visibility = "hidden" 
	pnlTab_outsusu.style.visibility = "hidden" 
	pnlTab_KAKAO.style.visibility = "visible" 
...
Sub imgExcel_onclick()
	...
	With frmThis
		...
		elseif mstrGUBUN = "KO"  then  
			mobjSCGLSpr.ExportExcelFile .sprSht_OUTSUSU
		elseif mstrGUBUN = "KO2"  then  
...
Sub sprSht_KAKAO_Change(ByVal Col, ByVal Row)
	...
	with frmThis
		...
		if	Col = mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"paycode") then
			if mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PAYCODE", Row) = "G" THEN
				mobjSCGLSpr.SetCellTypeEdit2 .sprSht_KAKAO, "DEBTOR", Row, Row, 255
				mobjSCGLSpr.SetTextBinding .sprSht_KAKAO,"DEBTOR",Row, "404150"
			ELSEif mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PAYCODE", Row) = "T" THEN
				mobjSCGLSpr.SetCellTypeEdit2 .sprSht_KAKAO, "DEBTOR", Row, Row, 255
				mobjSCGLSpr.SetTextBinding .sprSht_KAKAO,"DEBTOR",Row, "404100"
			else
				mobjSCGLSpr.SetCellTypeEdit2 .sprSht_KAKAO, "DEBTOR", Row, Row, 255
				mobjSCGLSpr.SetTextBinding .sprSht_KAKAO,"DEBTOR",Row, "404103"
			end if 
			
			strCODE = mobjSCGLSpr.GetTextBinding( frmThis.sprSht_KAKAO,"VENDOR",Row)
			Call Get_SUBCOMBO_VALUE(strCODE, Row, .sprSht_KAKAO)
		end if 
		
		If mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PREPAYMENT",Row) = "Y" Then
			mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,false,"FROMDATE",Row,Row,false
			mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,false,"TODATE",Row,Row,false
		Else
			mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,True,"FROMDATE",Row,Row,false
			mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,True,"TODATE",Row,Row,false
		End If
		
		if	Col = mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"DOCUMENTDATE") and Row = 1 then
			for i=1 to .sprSht_KAKAO.MaxRows
				mobjSCGLSpr.SetTextBinding .sprSht_KAKAO,"DOCUMENTDATE",i, mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DOCUMENTDATE", Row) 
			Next
		end if
		
		if	Col = mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"ACCOUNT") and Row = 1 then
			for i=1 to .sprSht_KAKAO.MaxRows
				mobjSCGLSpr.SetTextBinding .sprSht_KAKAO,"ACCOUNT",i, mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"ACCOUNT", Row) 
			Next
		end if
		
		if	Col = mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"DEBTOR") and Row = 1 then
			for i=1 to .sprSht_KAKAO.MaxRows
				mobjSCGLSpr.SetTextBinding .sprSht_KAKAO,"DEBTOR",i, mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DEBTOR", Row) 
			Next
		end if
		
		if	Col = mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"DEMANDDAY") and Row = 1 then
			for i=1 to .sprSht_KAKAO.MaxRows
				mobjSCGLSpr.SetTextBinding .sprSht_KAKAO,"DEMANDDAY",i, mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DEMANDDAY", Row) 
			Next
		end if
		
	End With
	
	mobjSCGLSpr.CellChanged frmThis.sprSht_KAKAO, Col, Row
End Sub
...
Sub sprSht_KAKAO_Click(ByVal Col, ByVal Row)
	...
	With frmThis
		if Col = 1 and Row = 0 then
			mobjSCGLSpr.SetCellTypeCheckBox .sprSht_KAKAO, 1, 1, , , "", , , , , mstrCheck
...
sub sprSht_KAKAO_DblClick (ByVal Col, ByVal Row)
	with frmThis
		if Row = 0 and Col >1 then
			mobjSCGLSpr.SetSheetSortUser  .sprSht_KAKAO, ""
		end if
	end with
end sub
...
Sub InitPage()
	'서버업무객체 생성	
	Set mobjMDCMPOINTADVOCH = gCreateRemoteObject("cMDOT.ccMDOTPOINTADVOCH")
    ...
    with frmThis
		...
		mobjSCGLSpr.SpreadLayout .sprSht_KAKAO, 38, 0, 4
		mobjSCGLSpr.SpreadDataField .sprSht_KAKAO,    "CHK | POSTINGDATE | CUSTOMERCODE | CUSTNAME | VENDORNAME | SUMM | BA | COSTCENTER | AMT | VAT | SEMU | BP | DUEDATE  | GBN | ACCOUNT | DEBTOR | BMORDER | DOCUMENTDATE | DEMANDDAY | PAYCODE | BANKTYPE | PREPAYMENT | FROMDATE | TODATE | SUMMTEXT | TAXYEARMON | TAXNO | VOCHNO | ERRCODE | ERRMSG | GFLAG | MEDFLAG | AMTGBN | VENDOR | RMSNO | REAL_MED_BUSINO | REAL_MED_NAME | TRANSRANK"
		mobjSCGLSpr.SetHeader .sprSht_KAKAO,		    "선택|전표일자|거래처코드|거래처|외주처|적요|사업영역|코스트센터|금액|부가세|세무코드|BP|광고주지급일|구분|차변계정|계정|BMORDER|증빙일|지급기일|지급방법|BANKTYPE|선수금구분|선수금(시작일)|선수금(종료일)|본문TEXT|RMS년월|RMS번호|전표번호|에러코드|에러메세지|GFLAG|매출구분|AMTGBN|VENDOR|RMSNO|본사사업자번호|본사명|TRANSRANK"
		mobjSCGLSpr.SetColWidth .sprSht_KAKAO, "-1",  "     4|       8|        10|     0|    15|  20|       5|         8|  10|    10|       7| 5|          10|   0|       7|   7|      7|     8|       8|      20|      20|        10|            13|            13|      20|      7|      7|       9|       0|        10|    0|      10|     0|     0|    0|		      0|     0|        0"
		mobjSCGLSpr.SetRowHeight .sprSht_KAKAO, "0", "15"
		mobjSCGLSpr.SetRowHeight .sprSht_KAKAO, "-1", "13"
		mobjSCGLSpr.SetCellTypeCheckBox2 .sprSht_KAKAO, "CHK"
		mobjSCGLSpr.SetCellTypeDate2 .sprSht_KAKAO, "POSTINGDATE | DEMANDDAY | DOCUMENTDATE | FROMDATE | TODATE | DUEDATE"
		mobjSCGLSpr.SetCellTypeEdit2 .sprSht_KAKAO, "CUSTOMERCODE | CUSTNAME | VENDORNAME | BA | COSTCENTER | BP | GBN | ACCOUNT | DEBTOR | TAXYEARMON | TAXNO | VOCHNO | ERRCODE | ERRMSG | GFLAG | MEDFLAG | AMTGBN | VENDOR | RMSNO", -1, -1, 200
		mobjSCGLSpr.SetCellTypeEdit2 .sprSht_KAKAO, "SUMMTEXT", -1, -1, 50
		mobjSCGLSpr.SetCellTypeEdit2 .sprSht_KAKAO, "SUMM", -1, -1, 25
		mobjSCGLSpr.SetCellTypeEdit2 .sprSht_KAKAO, "PAYCODE | BANKTYPE", -1, -1, 255
		mobjSCGLSpr.SetCellTypeFloat2 .sprSht_KAKAO, "AMT | VAT", -1, -1, 0 '숫자형
		mobjSCGLSpr.SetCellTypeComboBox .sprSht_KAKAO,mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"PREPAYMENT"),mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"PREPAYMENT"),-1,-1,strComboPREPAYMENT,,80
		mobjSCGLSpr.SetCellTypeComboBox .sprSht_KAKAO,mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"SEMU"),mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"SEMU"),-1,-1,strSemuComboListA,,50
		mobjSCGLSpr.SetCellTypeComboBox .sprSht_KAKAO,mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"BMORDER"),mobjSCGLSpr.CnvtDataField(.sprSht_KAKAO,"BMORDER"),-1,-1,strBMORDER,,80
		mobjSCGLSpr.SetCellAlign2 .sprSht_KAKAO, "CUSTOMERCODE | BA | SEMU | BP | TAXYEARMON | TAXNO | GBN | VOCHNO | DEBTOR | ACCOUNT ",-1,-1,2,2,false '가운데
		mobjSCGLSpr.SetCellAlign2 .sprSht_KAKAO, "CUSTNAME | SUMM | ERRMSG | VENDORNAME",-1,-1,0,2,false '왼쪽
		mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,true,"CUSTOMERCODE | CUSTNAME  | AMT | BP | GBN | TAXYEARMON | TAXNO | VOCHNO | ERRCODE | ERRMSG | MEDFLAG | REAL_MED_BUSINO | REAL_MED_NAME | TRANSRANK"
		mobjSCGLSpr.ColHidden .sprSht_KAKAO, "GBN | CUSTNAME | GFLAG | MEDFLAG | DUEDATE | AMTGBN | VENDOR | RMSNO | REAL_MED_BUSINO | REAL_MED_NAME", true 
		mobjSCGLSpr.CellGroupingEach .sprSht_KAKAO,"VOCHNO | ERRCODE | ERRMSG"
...
Sub EndPage()
	set mobjMDCMPOINTADVOCH = Nothing
...
Sub Get_COMBO_VALUE ()		
	...
	With frmThis	
		...
		vntData = mobjMDCMPOINTADVOCH.Get_COMBO_VALUE(gstrConfigXml,mlngRowCnt,mlngColCnt,"PD_PAYCODE")
		If not gDoErrorRtn ("Get_COMBO_VALUE") Then 					
			mobjSCGLSpr.SetCellTypeComboBox2 .sprSht_OUTSUSU, "PAYCODE",,,vntData,,160
			mobjSCGLSpr.SetCellTypeComboBox2 .sprSht_KAKAO, "PAYCODE",,,vntData,,160
...
Sub Get_SUBCOMBO_VALUE(strCODE, row, sprsht)
	...
	With frmThis   
		...
		strCODE = replace(strCODE,"-","")
       	...
       	vntData = mobjMDCMPOINTADVOCH.Get_SUBCOMBO_VALUE(gstrConfigXml, mlngRowCnt, mlngColCnt, strCODE)
...
Sub SelectRtn (strVOCH_TYPE)
	with frmThis
		...
		IF strVOCH_TYPE = "S" THEN
			CALL SelectRtn_SUSU()
		ELSEIF strVOCH_TYPE = "KO" THEN
			CALL SelectRtn_OUTSUSU()
		ELSEIF strVOCH_TYPE = "KO2" THEN
			CALL SelectRtn_KAKAO()
		END IF
		mstrSTAY = TRUE
   	end with
End Sub
...
Sub SelectRtn_SUSU ()
	...
	with frmThis
		...
		vntData = mobjMDCMPOINTADVOCH.SelectRtn_SUSU(gstrConfigXml,mlngRowCnt,mlngColCnt,.txtYEARMON.value, _
...
Sub SelectRtn_SUSUDTL (Col, Row)
	...
	with frmThis
		...
		vntData = mobjMDCMPOINTADVOCH.SelectRtn_SUSUDTL(gstrConfigXml,mlngRowCnt,mlngColCnt, strTAXYEARMON, strTAXNO, strPREPAYMENT)
...
Sub SelectRtn_OUTSUSU ()
	...
	with frmThis
		...
		vntData = mobjMDCMPOINTADVOCH.SelectRtn_OUTSUSU(gstrConfigXml, mlngRowCnt, mlngColCnt, strYEARMON, _
													  strCLIENTCODE, strCLIENTNAME, strGBN)
...
Sub SelectRtn_KAKAO ()
	...
	with frmThis
		...
		vntData = mobjMDCMPOINTADVOCH.SelectRtn_KAKAO(gstrConfigXml, mlngRowCnt, mlngColCnt, strYEARMON, _
														strCLIENTCODE, strCLIENTNAME, strGBN)
		...
		if not gDoErrorRtn ("SelectRtn_KAKAO") then
			if mlngRowCnt > 0 Then
				...
				mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,false,"ACCOUNT",-1,-1,false
				
				mobjSCGLSpr.SetClipbinding .sprSht_KAKAO, vntData, 1, 1, mlngColCnt, mlngRowCnt, True
				'ACCOUNT
				For intCnt = 1 To .sprSht_KAKAO.MaxRows
					If  .rdT.checked then
						mobjSCGLSpr.SetCellTypeCheckBox .sprSht_KAKAO, 1,1,intCnt,intCnt,,0,1,2,2,false
						mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,true,"DEMANDDAY",intCnt,intCnt,false
						mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,true,"DUEDATE",intCnt,intCnt,false
						mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,true,"POSTINGDATE",intCnt,intCnt,false
					elseif .rdF.checked or .rdE.checked then
						mobjSCGLSpr.SetCellTypeCheckBox .sprSht_KAKAO, 1,1,intCnt,intCnt,,0,1,2,2,false
						mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,false,"DEMANDDAY",intCnt,intCnt,false
						mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,false,"DUEDATE",intCnt,intCnt,false
						mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,false,"POSTINGDATE",intCnt,intCnt,false
					End If
				
					'선수금 처리시
					If mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PREPAYMENT",intCnt) = "Y" Then
						mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,false,"FROMDATE",intCnt,intCnt,false
						mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,false,"TODATE",intCnt,intCnt,false
					Else
						mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,True,"FROMDATE",intCnt,intCnt,false
						mobjSCGLSpr.SetCellsLock2 .sprSht_KAKAO,True,"TODATE",intCnt,intCnt,false
					End If	
				Next
				AMT_SUM .sprSht_KAKAO
   			end If
   			gWriteText lblStatus, mlngRowCnt & "건의 자료가 검색" & mePROC_DONE
   		end if
   	end with
...
Function DataValidation_KAKAO ()
	...
	With frmThis
		For intCnt =1  To .sprSht_KAKAO.MaxRows
			if mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"CHK",intCnt) = "1" AND mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"duedate",intCnt) = "" Then 
				gErrorMsgBox intCnt & " 번째 행의 광고주청구일 을 확인하십시오","저장오류"
				Exit Function
			End if
		Next
...
Sub ProcessRtn_KAKAO()
	...
	with frmThis
		vntData_ProcesssRtn = mobjSCGLSpr.GetDataRows(.sprSht_KAKAO,"CHK | POSTINGDATE | CUSTOMERCODE | CUSTNAME | VENDORNAME | SUMM | BA | COSTCENTER | AMT | VAT | SEMU | BP | DUEDATE  | GBN | ACCOUNT | DEBTOR | BMORDER | DOCUMENTDATE | DEMANDDAY | PAYCODE | BANKTYPE | PREPAYMENT | FROMDATE | TODATE | SUMMTEXT | TAXYEARMON | TAXNO | VOCHNO | ERRCODE | ERRMSG | GFLAG | MEDFLAG | AMTGBN | VENDOR | RMSNO | REAL_MED_BUSINO | REAL_MED_NAME | TRANSRANK")
		'처리 업무객체 호출
		if  not IsArray(vntData_ProcesssRtn) then 
			gErrorMsgBox "변경된 " & meNO_DATA,"저장취소"
			exit sub
		...
		'시트 전체를 돌면서 체크된 값의 TRANSRANK 의 최대 값을 담는다.
		intColFlag = 0
		For intMaxCnt = 1 To .sprSht_KAKAO.MaxRows
			If mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"CHK",intMaxCnt) = 1 Then
				bsdiv = cint(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TRANSRANK",intMaxCnt))
				IF intColFlag < bsdiv THEN
					intColFlag = bsdiv
				END IF
			End IF
		...
		IF .rdDIV.checked THEN
			if mstrPROCESS = "Create" then
				For intCnt = 1 To .sprSht_KAKAO.MaxRows
					if mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"chk",intCnt) = "1" then	
					
						'채번을 설정한다.
						'--------------------------------------------------------------------------------------
						strPOSTINGDATE = "" :  strMEDFLAG = "" : strRMSTAXYEARMON = "" :  strRMSTAXNO = "" : strVOCHNORMS = "" : strTYPE = ""

						strPOSTINGDATE		= replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"POSTINGDATE",intCnt),"-","")
						strMEDFLAG			= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"MEDFLAG",intCnt)
						strRMSTAXYEARMON	= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXYEARMON",intCnt)
						strRMSTAXNO			= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXNO",intCnt)'
						strTYPE				= "4"
						...
						if strIF_CNT = "1" then
							...
							strITEMLIST = strITEMLIST + cstr(strHSEQ) + "|" + _
										cstr(strISEQ) + "|" + _
										replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"POSTINGDATE",intCnt),"-","") + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"VENDOR",intCnt) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SUMM",intCnt) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BA",intCnt) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"COSTCENTER",intCnt) + "|" + _
										cstr(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"AMT",intCnt)) + "|" + _
										cstr(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"VAT",intCnt)) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SEMU",intCnt) + "|" + _ 
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BP",intCnt) + "|" + _ 
										replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DEMANDDAY",intCnt),"-","") + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"CUSTOMERCODE",intCnt) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXYEARMON",intCnt) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXNO",intCnt) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"GFLAG",intCnt) + "|" + _
										strRMS_DOC_TYPE + "|" + _ 
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"ACCOUNT",intCnt) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DEBTOR",intCnt) + "|" + _
										replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DOCUMENTDATE",intCnt),"-","") + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PREPAYMENT",intCnt) + "|" + _
										replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"FROMDATE",intCnt),"-","") + "|" + _
										replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TODATE",intCnt),"-","") + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SUMMTEXT",intCnt) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"AMTGBN",intCnt) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PAYCODE",intCnt) + "|" + _  
										replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DUEDATE",intCnt),"-","") + "|" + _
										strVOCHNORMS + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BANKTYPE",intCnt) + "|" + _  
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BMORDER",intCnt)
						else
							strHSEQ = strHSEQ + 1
							strISEQ = 1
					...
					if mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"chk",intCnt) = "1" then		
						...
						if strIF_CNT = "1" then
							...
							strITEMLIST = strITEMLIST + cstr(strHSEQ) + "|" + _
										...
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PAYCODE",intCnt) + "|" + _  
										replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DUEDATE",intCnt),"-","") + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"VOCHNO",intCnt) + "|" + _
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BANKTYPE",intCnt) + "|" + _  
										mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BMORDER",intCnt)
						else
							strHSEQ = strHSEQ+1
				...
				For intCnt = 1 To intColFlag
					...
					For i = 1 To .sprSht_KAKAO.MaxRows
						If mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"CHK",i) = 1 Then
							'청구합계
							If CDbl(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TRANSRANK",i)) = intCnt Then
								lngAMT = CDbl(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"AMT",i))
								lngSUMAMT = lngSUMAMT + lngAMT
								lngVAT = CDbl(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"VAT",i))
								lngSUMVAT = lngSUMVAT + lngVAT
							End If
						End If
					Next
						...
						'체크가 되어있는 데이터가 합산할수있는 데이터라면 
							If CDbl(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TRANSRANK",i)) = intCnt Then	
								...
								Else
									...
									strPOSTINGDATE		= replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"POSTINGDATE",i),"-","")
									strMEDFLAG			= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"MEDFLAG",i)
									strRMSTAXYEARMON	= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXYEARMON",i)
									strRMSTAXNO			= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXNO",i)'
									strTYPE				= "4"
									...
									strIF_CNT = strIF_CNT + 1
									'각각의 값을 변수에 담아 한데이터로 만들고 금액은 합산한 금액을 입력한다.
									strPOSTINGDATE  = mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"POSTINGDATE",i)
									strVENDOR		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"REAL_MED_BUSINO",i)
									strSUMM			= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SUMM",i)
									strBA			= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BA",i)
									strCOSTCENTER	= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"COSTCENTER",i)
									strAMT			= lngSUMAMT
									strVAT			= lngSUMVAT
									strSEMU			= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SEMU",i)
									strBP			= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BP",i)
									strDEMANDDAY	= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DEMANDDAY",i)
									strCUSTOMERCODE	= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"REAL_MED_BUSINO",i)
									strTAXYEARMON	= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXYEARMON",i)
									strTAXNO		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXNO",i)
									strGFLAG		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"GFLAG",i)
									strRMS_DOC_TYPE	= "M"
									strACCOUNT		= ""
									strDEBTOR		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DEBTOR",i)
									strDOCUMENTDATE	= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DOCUMENTDATE",i)
									strPREPAYMENT	= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PREPAYMENT",i)
									strFROMDATE		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"FROMDATE",i)
									strTODATE		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TODATE",i)
									strSUMMTEXT		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SUMMTEXT",i)
									strAMTGBN		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"AMTGBN",i)
									strPAYCODE		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PAYCODE",i)
									strDUEDATE		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DUEDATE",i)
									strVOCHNO		= strVOCHNORMS
									strBANKTYPE		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BANKTYPE",i)
									strBMORDER		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BMORDER",i)
									...
									For j = 1 To .sprSht_KAKAO.MaxRows
										If mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"CHK",j) = 1 Then
												'합산 데이터라면 
											If CDbl(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TRANSRANK",j)) = intCnt Then
												strIF_CNT = strIF_CNT + 1
												strISEQ = strISEQ+1
												
												strITEMLIST = strITEMLIST + ":" + cstr(strHSEQ) + "|" + _
															cstr(strISEQ) + "|" + _
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"POSTINGDATE",j),"-","") + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"VENDOR",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SUMM",j) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BA",j) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"COSTCENTER",j) + "|" + _
															cstr(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"AMT",j)) + "|" + _
															cstr(0) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SEMU",j) + "|" + _ 
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BP",j) + "|" + _ 
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DEMANDDAY",j),"-","") + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"CUSTOMERCODE",j) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXYEARMON",j) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXNO",j) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"GFLAG",j) + "|" + _
															strRMS_DOC_TYPE + "|" + _ 
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"ACCOUNT",j) + "|" + _
															"" + "|" + _
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DOCUMENTDATE",j),"-","") + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PREPAYMENT",j) + "|" + _
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"FROMDATE",j),"-","") + "|" + _
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TODATE",j),"-","") + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SUMMTEXT",j) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"AMTGBN",j) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PAYCODE",j) + "|" + _  
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DUEDATE",j),"-","") + "|" + _
															strVOCHNORMS + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BANKTYPE",j) + "|" + _  
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BMORDER",j)
											END IF 
										END IF 
									NEXT
									strHSEQ = strHSEQ + 1
						...
						if mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"chk",i) = "1" then	
							If CDbl(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TRANSRANK",i)) = intCnt Then
								...
								Else
									...
									strPAYCODE		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PAYCODE",i)
									strDUEDATE		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DUEDATE",i)
									strVOCHNO		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"VOCHNO",i)	
									strBANKTYPE		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BANKTYPE",i)
									strBMORDER		= mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BMORDER",i)
									...
									For j = 1 To .sprSht_KAKAO.MaxRows
										If mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"CHK",j) = 1 Then
											If CDbl(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TRANSRANK",j)) = intCnt Then	
												...
												strITEMLIST = strITEMLIST + ":" + cstr(strHSEQ) + "|" + _
															cstr(strISEQ) + "|" + _
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"POSTINGDATE",i),"-","") + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"VENDOR",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SUMM",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BA",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"COSTCENTER",i) + "|" + _
															cstr(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"AMT",i)) + "|" + _
															cstr(0) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SEMU",i) + "|" + _ 
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BP",i) + "|" + _ 
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DEMANDDAY",i),"-","") + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"CUSTOMERCODE",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXYEARMON",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TAXNO",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"GFLAG",i) + "|" + _
															strRMS_DOC_TYPE + "|" + _ 
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"ACCOUNT",i) + "|" + _
															"" + "|" + _
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DOCUMENTDATE",i),"-","") + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PREPAYMENT",i) + "|" + _
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"FROMDATE",i),"-","") + "|" + _
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"TODATE",i),"-","") + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"SUMMTEXT",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"AMTGBN",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"PAYCODE",i) + "|" + _  
															replace(mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"DUEDATE",i),"-","") + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"VOCHNO",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BANKTYPE",i) + "|" + _
															mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"BMORDER",i)
											END IF
										END IF 
									NEXT
									strHSEQ = strHSEQ + 1
...
Sub Set_VochValue (strRETURNLIST)
	...
	With frmThis
		...
		if mstrPROCESS ="Create" then
			if mstrGUBUN = "S" then
				intRtn = mobjMDCMPOINTADVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "K")
			elseif mstrGUBUN = "KO" then
				intRtn = mobjMDCMPOINTADVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "KO")
			elseif mstrGUBUN = "KO2" then
				intRtn = mobjMDCMPOINTADVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "KO2")
			end if 
			...
			if not gDoErrorRtn ("ProcessRtn") then
				...
				ELSEIF mstrGUBUN = "KO2" THEN
					mobjSCGLSpr.SetFlag  .sprSht_KAKAO, meCLS_FLAG
				END IF
   		...
   		elseif mstrPROCESS ="Delete" then
   			if mstrGUBUN = "S" then
				intRtn = mobjMDCMPOINTADVOCH.VOCHDELL(gstrConfigXml, strRETURNLIST, mstrGUBUN, "POINTAD")
			else
				intRtn = mobjMDCMPOINTADVOCH.VOCHDELL_BUY(gstrConfigXml, strRETURNLIST, mstrGUBUN, "POINTAD", mstrGUBUN)
			end if 
...
sub ErrVochDeleteRtn
	...
	with frmThis
		...
		ELSEIF mstrGUBUN = "KO2" THEN
			vntData = mobjSCGLSpr.GetDataRows(.sprSht_KAKAO,"CHK | TAXYEARMON | TAXNO | ERRCODE | GBN | MEDFLAG")
		END IF
		...
		intRtn = mobjMDCMPOINTADVOCH.DeleteRtn(gstrConfigXml,vntData)
		...
		If mstrGUBUN = "S"  then
			for i = .sprSht_SUSU.MaxRows to 1 step -1
				If mobjSCGLSpr.GetTextBinding(.sprSht_SUSU,"CHK",i) = 1 Then
					...
					intRtn = mobjMDCMPOINTADVOCH.DeleteRtn_GANG(gstrConfigXml,strTAXYEARMON, strTAXNO, strVOCHNO, mstrGUBUN, "POINTAD" )
		...
		ELSEIf mstrGUBUN = "KO"  then
			for i = .sprSht_OUTSUSU.MaxRows to 1 step -1
				If mobjSCGLSpr.GetTextBinding(.sprSht_OUTSUSU,"CHK",i) = 1 Then
					...
					intRtn = mobjMDCMPOINTADVOCH.DeleteRtn_GANG_BUY(gstrConfigXml,strTAXYEARMON, strTAXNO, strVOCHNO, mstrGUBUN, "POINTAD", "KO" )
		...
		ELSEIf mstrGUBUN = "KO2"  then
			for i = .sprSht_KAKAO.MaxRows to 1 step -1
				If mobjSCGLSpr.GetTextBinding(.sprSht_KAKAO,"CHK",i) = 1 Then
					...
					intRtn = mobjMDCMPOINTADVOCH.DeleteRtn_GANG_BUY(gstrConfigXml,strTAXYEARMON, strTAXNO, strVOCHNO, mstrGUBUN, "POINTAD", "KO2")
							...
							<TR>
								<TD id="TD1" align="left" width="400" height="20" runat="server">
									<table cellSpacing="0" cellPadding="0" width="100%" border="0">
										...
										<tr>
											<td class="TITLE">포인트 친구 AD 전표관리&nbsp;</td>
										</tr>
									</table>
								</TD>
							...
							<TBODY>
									...
									<TD class="KEYFRAME" vAlign="middle" align="center">
										<TABLE height="15" cellSpacing="0" cellPadding="0" width="100%" background="../../../images/TitleBG.gIF"
											...
											<TR>
												...
												<INPUT class="BTNTABON" id="btnTab1" style="BACKGROUND-IMAGE: url(../../../images/imgTabOn.gIF)"type="button" value="매출" name="btnTab1"> 
												<INPUT class="BTNTAB" id="btnTab2" style="BACKGROUND-IMAGE: url(../../../images/imgTab.gIF)"type="button" size="20" value="대행수수료" name="btnTab2"> 
												<INPUT class="BTNTAB" id="btnTab3" style="BACKGROUND-IMAGE: url(../../../images/imgTab.gIF)"type="button" size="20" value="KAKAO매입" name="btnTab3">
												</TD>
								...
								<TR vAlign="top" align="left">
									...
									<TD class="LISTFRAME" style="WIDTH: 100%; HEIGHT: 80%" vAlign="top" align="center">
										...
										<DIV id="pnlTab_KAKAO" style="LEFT: 7px; VISIBILITY: hidden; WIDTH: 100%; POSITION: absolute; HEIGHT: 100%"
											ms_positioning="GridLayout">
											<OBJECT id="sprSht_KAKAO" style="WIDTH: 100%; HEIGHT: 100%" classid="clsid:41F841C1-AE16-11D5-8817-0050DA6EF5E5" >
												<PARAM NAME="_Version" VALUE="393216">
												<PARAM NAME="_ExtentX" VALUE="31856">
												<PARAM NAME="_ExtentY" VALUE="12568">
												<PARAM NAME="_StockProps" VALUE="64">
