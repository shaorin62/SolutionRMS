<%@ Page CodeBehind="MDCMELECTRICTRUTAX.aspx.vb" Language="vb" AutoEventWireup="false" Inherits="MD.MDCMELECTRICTRUTAX" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<HEAD>
		<title>공중파 위수탁 세금계산서 발행</title> 
		<!--
'****************************************************************************************
...
Dim mobjMDCMELECTRICTRUTAX , mobjMDCMGET
Dim mstrCheck
'=========================================================================================
' 이벤트 프로시져 
'=========================================================================================
...
Sub imgPrint_onclick ()
	...
	IF frmThis.sprSht.MaxRows = 0 then
		...
		with frmThis
			...
			'md_trans_temp삭제 시작
			intRtn = mobjMDCMELECTRICTRUTAX.DeleteRtn_TEMP(gstrConfigXml)
			'md_trans_temp삭제 끝
			...
			for i=1 to .sprSht.MaxRows
				IF mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",i) = "1" THEN
					...
					vntDataTemp = mobjMDCMELECTRICTRUTAX.ProcessRtn_TEMP(gstrConfigXml,strTAXYEARMON, strTAXNO, VATFLAG, FLAG, i, strUSERID)
				END IF
			next
...
Sub TAXPOP
	...
	Dim strYEARMON
	with frmThis
		vntInParams = array(.txtTRANSYEARMON1.value, .txtTRANSNO1.value,.txtCLIENTCODE1.value,.txtCLIENTNAME1.value, "trans","ELEC") '<< 받아오는경우
		...
		if isArray(vntRet) then
			...
			.txtCLIENTCODE1.value = vntRet(2,0)  ' 코드명 표시
...
Sub CLIENTCODE_POP
	...
	Dim vntInParams
	with frmThis
		vntInParams = array(.txtTRANSYEARMON1.value, .txtCLIENTCODE1.value, .txtCLIENTNAME1.value, "trans", "ELEC") 
...
Sub txtCLIENTNAME1_onkeydown
	if window.event.keyCode = meEnter then
		...
		with frmThis
			...
			vntData = mobjMDCMGET.GetTAXCUSTNO(gstrConfigXml,mlngRowCnt,mlngColCnt,.txtTRANSYEARMON1.value,.txtCLIENTCODE1.value,.txtCLIENTNAME1.value,"trans","ELEC")
...
Sub txtTRANSNO1_onkeydown
	if window.event.keyCode = meEnter then
		...
		with frmThis
			...
			vntData = mobjMDCMGET.GetTRANSNO(gstrConfigXml,mlngRowCnt,mlngColCnt,strYEARMON, .txtTRANSNO1.value,.txtCLIENTCODE1.value,.txtCLIENTNAME1.value, "trans", "ELEC", "0")
...
Sub TRANSPOP
	...
	End If
	'msgbox strYEARMON
		vntInParams = array(strYEARMON, .txtTRANSNO1.value,.txtCLIENTCODE1.value,.txtCLIENTNAME1.value, "trans","ELEC") '<< 받아오는경우
		vntRet = gShowModalWindow("../MDCO/MDCMTRANSPOP.aspx",vntInParams , 413,435)
		if isArray(vntRet) then
...
sub sprSht_DblClick (ByVal Col, ByVal Row)
	Dim vntInParams
	Dim strTRANSYEARMON, strTRANSNO, strCLIENTNAME, strREAL_MED_NAME, strSUMM
	Dim strPRINTDAY, strCLIENTCODE, strCLIENTACCODE, strCLIENTBISNO, strREAL_MED_CODE
	Dim strREAL_MED_ACCODE, strREAL_MED_BISNO, strMEDCODE, strDEPTCODE, strMEDFLAG
	Dim strTAXYEARMON, strSPONSOR, strCLIENTOWNER, strCLIENTADDR1, strCLIENTADDR2
	...
	with frmThis
		...
		Else
			...
			strCLIENTNAME = mobjSCGLSpr.GetTextBinding(.sprSht,"CLIENTNAME", Row)
			strREAL_MED_NAME = mobjSCGLSpr.GetTextBinding(.sprSht,"REAL_MED_NAME", Row)
			strAMT = mobjSCGLSpr.GetTextBinding(.sprSht,"AMT", Row)
			strVAT = mobjSCGLSpr.GetTextBinding(.sprSht,"VAT", Row)
			strSUMM = mobjSCGLSpr.GetTextBinding(.sprSht,"SUMM", Row)
			strPRINTDAY = mid(mobjSCGLSpr.GetTextBinding(.sprSht,"PRINTDAY", Row),1,4) & mid(mobjSCGLSpr.GetTextBinding(.sprSht,"PRINTDAY", Row),6,2) & mid(mobjSCGLSpr.GetTextBinding(.sprSht,"PRINTDAY", Row),9,2)
			strCLIENTCODE = mobjSCGLSpr.GetTextBinding(.sprSht,"CLIENTCODE", Row)
			strCLIENTACCODE = mobjSCGLSpr.GetTextBinding(.sprSht,"CLIENTACCODE", Row)
			strCLIENTBISNO = mobjSCGLSpr.GetTextBinding(.sprSht,"CLIENTBISNO", Row)
			strREAL_MED_CODE = mobjSCGLSpr.GetTextBinding(.sprSht,"REAL_MED_CODE", Row)
			strREAL_MED_ACCODE = mobjSCGLSpr.GetTextBinding(.sprSht,"REAL_MED_ACCODE", Row)
			strREAL_MED_BISNO = mobjSCGLSpr.GetTextBinding(.sprSht,"REAL_MED_BISNO", Row)
			strMEDCODE = mobjSCGLSpr.GetTextBinding(.sprSht,"MEDCODE", Row)
			strDEPTCODE = mobjSCGLSpr.GetTextBinding(.sprSht,"DEPTCODE", Row)
			strMEDFLAG = mobjSCGLSpr.GetTextBinding(.sprSht,"MEDFLAG", Row)
			strTAXYEARMON = mobjSCGLSpr.GetTextBinding(.sprSht,"TAXYEARMON", Row)
			strSPONSOR = mobjSCGLSpr.GetTextBinding(.sprSht,"SPONSOR", Row)
			strCLIENTOWNER = mobjSCGLSpr.GetTextBinding(.sprSht,"CLIENTOWNER", Row)
			strCLIENTADDR1 = mobjSCGLSpr.GetTextBinding(.sprSht,"CLIENTADDR1", Row)
			strCLIENTADDR2 = mobjSCGLSpr.GetTextBinding(.sprSht,"CLIENTADDR2", Row)
			strREAL_MEDOWNER = mobjSCGLSpr.GetTextBinding(.sprSht,"REAL_MEDOWNER", Row)
			strREAL_MEDADDR1 = mobjSCGLSpr.GetTextBinding(.sprSht,"REAL_MEDADDR1", Row)
			strREAL_MEDADDR2 = mobjSCGLSpr.GetTextBinding(.sprSht,"REAL_MEDADDR2", Row)
			strDEMANDDAY = mid(mobjSCGLSpr.GetTextBinding(.sprSht,"DEMANDDAY", Row),1,4) & mid(mobjSCGLSpr.GetTextBinding(.sprSht,"DEMANDDAY", Row),6,2) & mid(mobjSCGLSpr.GetTextBinding(.sprSht,"DEMANDDAY", Row),9,2)
			
			vntInParams = array(strTRANSYEARMON,strTRANSNO,strCLIENTNAME,strREAL_MED_NAME,strAMT,strVAT,strSUMM,strPRINTDAY,strCLIENTCODE,strCLIENTACCODE,strCLIENTBISNO,strREAL_MED_CODE,strREAL_MED_ACCODE,strREAL_MED_BISNO,strMEDCODE,strDEPTCODE,strMEDFLAG,strTAXYEARMON,strSPONSOR,strCLIENTOWNER,strCLIENTADDR1,strCLIENTADDR2,strREAL_MEDOWNER,strREAL_MEDADDR1,strREAL_MEDADDR2, strDEMANDDAY)
			gShowModalWindow "MDCMELECTRICTRUTAXPOP.aspx",vntInParams , 528,465
			SelectRtn
			ELSE
			strMEDFLAG = "A"
			vntInParams = array(mobjSCGLSpr.GetTextBinding(.sprSht,"TAXYEARMON", Row),mobjSCGLSpr.GetTextBinding(.sprSht,"TAXNO", Row),strMEDFLAG) '<< 받아오는경우
				gShowModalWindow "MDCMELECTRUTAXDTL.aspx",vntInParams , 813,565
			SelectRtn
			End IF
		end if	
	end with
...
Sub InitPage()
    ...
    With frmThis
		...
		mobjSCGLSpr.SetColWidth .sprSht, "-1", "   4|	5|   4|	     11|	  5|   4|	4|     8|	   19|                  17|	     19|                  17|     9|       9|       9|	     9|  30|    10| 	8|         0|           0|         0|           0|      0|       0|       0|      10|   0|    30|       0|0|0|0|0|0|0"
...
Sub SelectRtn ()
	...
	with frmThis
		...
		If .rdT.checked = True Then
			vntData = mobjMDCMELECTRICTRUTAX.Get_ELEC_TAX(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON,strCUSTCODE,strFROM,strTO,strTAXNO)
			If not gDoErrorRtn ("Get_ELEC_TAX") then
				'조회한 데이터를 바인딩
				...
				mobjSCGLSpr.ColHidden .sprSht, "TRANSYEARMON|TRANSNO|TAXYEARMON|TAXNO|SEQ|MEDNAME|PROGRAM", True
				gWriteText lblstatus, "선택한 자료에 대해서 " & mlngRowCnt & " 건의 자료가 검색" & mePROC_DONE
			End If
		'미완료 거래명세서 디테일 조회
		ElseIf .rdF.checked = True Then
			vntData = mobjMDCMELECTRICTRUTAX.Get_ELEC_TAXBUILD(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON,strTRANSNO, strCUSTCODE,strGUBUN)
			If not gDoErrorRtn ("Get_ELEC_TAXBUILD") then
				'조회한 데이터를 바인딩
				call mobjSCGLSpr.SetClipBinding (frmThis.sprSht,vntData,1,1,mlngColCnt,mlngRowCnt,True)
				'초기 상태로 설정
				mobjSCGLSpr.SetFlag  frmThis.sprSht,meCLS_FLAG
				...
				mobjSCGLSpr.ColHidden .sprSht, "TRANSYEARMON|TRANSNO|SEQ|MEDNAME|PROGRAM", False
				Layout_change
				gWriteText lblstatus, "선택한 자료에 대해서 " & mlngRowCnt & " 건의 자료가 검색" & mePROC_DONE
		...
		ElseIf .rdA.checked = True Then			
			vntData = mobjMDCMELECTRICTRUTAX.Get_ELEC_TAXALL(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON,strTRANSNO, strCUSTCODE, strGUBUN)
			If not gDoErrorRtn ("Get_ELEC_TAXALL") then
				'초기 상태로 설정
				mobjSCGLSpr.SetFlag  frmThis.sprSht,meCLS_FLAG
...
Sub ProcessRtn ()
	...
	with frmThis
		...
		'처리 업무객체 호출
		intTAXNO = 0
		intRtn = mobjMDCMELECTRICTRUTAX.ProcessRtn_Div(gstrConfigXml,vntData, intTAXNO)


		if not gDoErrorRtn ("ProcessRtn") then
			mobjSCGLSpr.SetFlag  .sprSht,meCLS_FLAG
			gErrorMsgBox "세금계산서가 생성되었습니다.","저장안내!"
			.rdT.checked = True
			.txtTRANSNO1.value = ""
			selectRtn
   		end if
...
Sub DeleteRtn ()
	...
	strDESCRIPTION = ""
		...
		intRtn = gYesNoMsgbox("분할생성된자료는 해당번호의 전체가 삭제됩니다. 자료를 삭제하시겠습니까?","자료삭제 확인")
		IF intRtn <> vbYes then exit Sub
		intCnt = 0
		...
		for i = .sprSht.MaxRows to 1 step -1
			if mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",i) = 1 THEN
				strTAXNO = mobjSCGLSpr.GetTextBinding(.sprSht,"TAXNO",i)
				strTAXYEARMON = mobjSCGLSpr.GetTextBinding(.sprSht,"TAXYEARMON",i)
				strATTR05 = mobjSCGLSpr.GetTextBinding(.sprSht,"MEDCODE",i)
			
				intRtn = mobjMDCMELECTRICTRUTAX.DeleteRtn_TruTax(gstrConfigXml,strTAXYEARMON, strTAXNO, strATTR05)
...
Function VOCHNO_CHECKED (ByRef strTAXYEARMON, ByRef strTAXNO)
	...
	vntData = mobjMDCMGET.TRUVOCHNO_CHECKED(gstrConfigXml,mlngRowCnt,mlngColCnt, strTAXYEARMON,strTAXNO)
...
-->
	...
	<body class="base">
		<form id="frmThis" method="post" runat="server">
			<TABLE id="tblForm" style="WIDTH: 1040px" cellSpacing="0" cellPadding="0" width="684" border="0">
				<TR>
					<TD>
									...
									<TABLE class="DATA" id="tblKey" cellSpacing="1" cellPadding="0" width="100%" border="0">
										<TR>
											<TD class="SEARCHLABEL" style="CURSOR: hand" onclick="vbscript:Call gCleanField(txtTRANSYEARMON1, txtTRANSNO1)"
												width="80">년월/번호</TD>
											<TD class="SEARCHDATA" width="200"><INPUT class="INPUT" id="txtTRANSYEARMON1" title="거래명세년월" style="WIDTH: 72px; HEIGHT: 22px"
													accessKey="NUM" type="text" maxLength="6" size="6" name="txtTRANSYEARMON1"><IMG id="ImgTAX" onmouseover="JavaScript:this.src='../../../images/imgPopupOn.gIF'" style="CURSOR: hand"
													onmouseout="JavaScript:this.src='../../../images/imgPopup.gIF'" height="20" src="../../../images/imgPopup.gIF" width="23" align="absMiddle" border="0" name="ImgTAX"><INPUT class="INPUT" id="txtTRANSNO1" title="거래명세번호" style="WIDTH: 56px; HEIGHT: 22px"
													type="text" maxLength="6" size="4" name="txtTRANSNO1" accessKey="NUM"></TD>
											...
											<TD class="SEARCHDATA" width="300"><INPUT class="INPUT_L" id="txtCLIENTNAME1" title="코드명" style="WIDTH: 203px; HEIGHT: 22px"
													...
													border="0" name="ImgCLIENTCODE"><INPUT class="INPUT" id="txtCLIENTCODE1" title="코드조회" style="WIDTH: 65px; HEIGHT: 22px"
											...
											<TD class="SEARCHDATA" width="300"><INPUT class="INPUT" id="txtFROM" title="청구일자" style="WIDTH: 72px; HEIGHT: 22px" accessKey="date"
													type="text" maxLength="10" size="6" name="txtFROM"><IMG id="imgFrom" onmouseover="JavaScript:this.src='../../../images/imgCalEndarOn.gIF'"
													...
													type="text" maxLength="10" size="6" name="txtTO"><IMG id="imgTo" onmouseover="JavaScript:this.src='../../../images/imgCalEndarOn.gIF'"
													style="CURSOR: hand" onmouseout="JavaScript:this.src='../../../images/imgCalEndar.gIF'" height="20" src="../../../images/imgCalEndar.gIF"
										...
										<TR>
											<TD class="SEARCHLABEL" style="CURSOR: hand" onclick="vbscript:Call gCleanField(txtTRANSYEARMON1, txtTRANSNO1)"
												width="80">발행</TD>
											<TD class="SEARCHDATA" colspan="6">&nbsp;&nbsp;&nbsp;&nbsp;<INPUT id="rdT" type="radio" name="rdGBN" title="완료내역조회">&nbsp;완료&nbsp;&nbsp;&nbsp; 
												&nbsp; <INPUT id="rdF" type="radio" name="rdGBN" title="미완료 내역조회" CHECKED>&nbsp;미완료&nbsp;&nbsp;&nbsp; 
									...
									<TABLE class="DATA" id="tblDATA" style="WIDTH: 1040px; HEIGHT: 6px" cellSpacing="1" cellPadding="0"
										...
										<TR>
											...
											<TD class="DATA" style="WIDTH: 294px"><INPUT class="INPUT" id="txtDEMANDDAY" title="청구일자" style="WIDTH: 112px; HEIGHT: 22px"
													accessKey="date" type="text" maxLength="10" size="13" name="txtDEMANDDAY"><IMG id="Img1" onmouseover="JavaScript:this.src='../../../images/imgCalEndarOn.gIF'"
											...
											<TD class="LABEL" style="WIDTH: 71px">계산서종류</TD>
											<TD class="DATA" style="WIDTH: 113px">
												<SELECT id="chkPRINT" title="출력물구분" style="WIDTH: 112px" name="chkPRINT">
			...
			<TABLE cellSpacing="0" cellPadding="0" width="100%" border="0">
				...
				<TR>
					<TD align="center">
						<TABLE cellSpacing="0" cellPadding="0" width="100%" border="0">
							<TR>
								<TD class="LISTFRAME" style="HEIGHT: 670px" height="101">
									<OBJECT id="sprSht" style="WIDTH: 100%; HEIGHT: 670px" classid="clsid:41F841C1-AE16-11D5-8817-0050DA6EF5E5"
										VIEWASTEXT>
										<PARAM NAME="_Version" VALUE="393216">
										<PARAM NAME="_ExtentX" VALUE="24474">
