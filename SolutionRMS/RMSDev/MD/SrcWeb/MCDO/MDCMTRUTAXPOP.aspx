<%@ Page Language="vb" AutoEventWireup="false" Codebehind="MDCMTRUTAXPOP.aspx.vb" Inherits="MD.MDCMTRUTAXPOP" %>

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<HEAD>
		<title>세금계산번호 조회</title> 
		<!--
'****************************************************************************************
	...
	with frmThis
		...
		for i = 0 to intNo
			select case i
				...
				case 3 : mtranscommiflag  = vntInParam(i)			'조회추가필드
				case 4 : mtransTblflag  = vntInParam(i)			'조회추가필드
			end select
		next
		'SpreadSheet 디자인
		...
		mobjSCGLSpr.SpreadDataField .sprSht, "TAXYEARMON | TAXNO | CLIENTCODE | CLIENTNAME"
		IF mtranscommiflag = "trans" then
			txtLB.innerHTML ="광고주코드"
			mobjSCGLSpr.SetHeader .sprSht, "년월|번호|CLIENTCODE|광고주명"
		elseif mtranscommiflag = "commi" then
			txtLB.innerHTML ="청구지코드"
			mobjSCGLSpr.SetHeader .sprSht, "년월|번호|CLIENTCODE|매체명"
		end if
		mobjSCGLSpr.SetColWidth .sprSht, "-1", "10 | 9 | 0| 22"
		...
		mobjSCGLSpr.SetCellAlign2 .sprSht, "TAXYEARMON|CLIENTCODE",-1,-1,2,2,false
...
sub SelectRtn ()
	...
	with frmThis
		...
		vntData = mobjMDCMGET.GetTAXNO(gstrConfigXml,mlngRowCnt,mlngColCnt,.txtTAXYEARMON.value, .txtTAXNO.value,.txtCLIENTCODE.value, mtranscommiflag, mtransTblflag)
							...
							<TR>
								<TD class="KEYFRAME" style="HEIGHT: 20px" vAlign="middle" height="20"><FONT face="굴림">
										<TABLE class="SEARCHDATA" id="tblKey" style="WIDTH: 392px" cellSpacing="0" cellPadding="0" width="392"
											...
											<TBODY>
												<TR>
													<TD class="SEARCHLABEL" style="WIDTH: 97px;CURSOR: hand" onclick="vbscript:Call gCleanField(txtTAXYEARMON,txtTAXNO)">
														세금계산서번호&nbsp;</TD>
													<TD class="SEARCHDATA" style="WIDTH: 135px" ><INPUT class="INPUT" id="txtTAXYEARMON" type="text" size="6" name="txtTAXYEARMON" style="WIDTH: 72px; HEIGHT: 22px">&nbsp;-
														<INPUT class="INPUT" id="txtTAXNO" style="WIDTH: 48px; HEIGHT: 22px" type="text" size="2"
															name="txtTAXNO">
													</TD>
													<TD class="SEARCHLABEL" id="txtLB"  style="WIDTH: 70px; CURSOR: hand" onclick="vbscript:Call gCleanField(txtCLIENTCODE,'')">
														&nbsp;</TD>
													<TD class="SEARCHDATA" style="WIDTH: 90px"><INPUT class="INPUT" id="txtCLIENTCODE" type="text" size="9" name="txtCLIENTCODE" style="WIDTH: 87px; HEIGHT: 22px">
													</TD>
												</TR>
											</TBODY>
										</TABLE>
