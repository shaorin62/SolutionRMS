<%@ Page Language="vb" AutoEventWireup="false" Codebehind="MDCMMEDGBNPOP.aspx.vb" Inherits="MD.MDCMMEDGBNPOP" %>
...
sub InitPage()
	...
	with frmThis
		...
		for i = 0 to intNo
			select case i
				case 0 : .txtREAL_MED_CODE.value = vntInParam(i)			'조회추가필드
				case 1 : .txtREAL_MED_NAME.value = vntInParam(i)		'현재 사용중인 것만
				case 2 : .txtMEDCODE.value = vntInParam(i)	
				case 3 : .txtMEDNAME.value = vntInParam(i)
						'mstrMED_FLAG = "MED_CATV"       mstrMED_FLAG 의 올바른 예제 (컬럼명이 와야함)
				case 4 : mstrMED_FLAG = vntInParam(i)	'medflag를 받는다.  
				case 5 : mstrUseDate = vntInParam(i)		'코드 사용 시점
				case 6 : mblnLikeCode = vntInParam(i)		'조회시 코드를 Like할지 여부
			end select
		next
        ...
        With frmThis
			...
			mobjSCGLSpr.SpreadLayout .sprSht, 7, 0, 0, 0,2
			mobjSCGLSpr.SpreadDataField .sprSht, "CUSTCODE | CUSTNAME | BUSINO | HIGHCUSTCODE | COMPANYNAME | MPP | MPPNAME"
			mobjSCGLSpr.SetHeader .sprSht,		 "코드|매체명|사업자번호|매체사코드|매체사명|MPP코드|MPP명"
			mobjSCGLSpr.SetColWidth .sprSht, "-1", " 5|    14|        10|         0|     15"
			...
			mobjSCGLSpr.SetCellTypeStatic2 .sprSht, "BUSINO"
			mobjSCGLSpr.SetCellTypeStatic2 .sprSht, "HIGHCUSTCODE"
			mobjSCGLSpr.SetCellTypeStatic2 .sprSht, "COMPANYNAME"
			mobjSCGLSpr.SetCellTypeStatic2 .sprSht, "MPP"
			mobjSCGLSpr.SetCellTypeStatic2 .sprSht, "MPPNAME"
			mobjSCGLSpr.ColHidden .sprSht, "HIGHCUSTCODE | MPP | MPPNAME",true
			mobjSCGLSpr.SetScrollBar .sprSht,2,False,0,-1
			mobjSCGLSpr.SetCellAlign2 .sprSht, "CUSTCODE|BUSINO",-1,-1,2,2,false
			mobjSCGLSpr.SetCellAlign2 .sprSht, "CUSTNAME|COMPANYNAME",-1,-1,0,2,false
...
sub SelectRtn ()
	...
	with frmThis
		...
		'체크되어있다면 전체이기때문에 MEDFLAG 대신 "" 를 넣고 검색  // 주의: strMEDFLAG = "" 를 하면 전역변수에서 가지고 있는 값이 없어지므로 금지.
		if .chkGBN.checked Then
			vntData = mobjMDCOGET.GetMEDGUBNCODE(gstrConfigXml,mlngRowCnt,mlngColCnt, .txtREAL_MED_CODE.value, .txtREAL_MED_NAME.value,.txtMEDCODE.value,.txtMEDNAME.value,"")
		ELSE 
			vntData = mobjMDCOGET.GetMEDGUBNCODE(gstrConfigXml,mlngRowCnt,mlngColCnt, .txtREAL_MED_CODE.value, .txtREAL_MED_NAME.value,.txtMEDCODE.value,.txtMEDNAME.value,mstrMED_FLAG)
		End if
							...
							<TR>
								<TD class="KEYFRAME" style="HEIGHT: 20px" vAlign="middle" height="20"><FONT face="굴림">
										<TABLE class="SEARCHDATA" id="tblKey" style="WIDTH: 392px" cellSpacing="0" cellPadding="0" width="392"
											...
											<TBODY>
												<TR>
													<TD class="SEARCHLABEL" width="40" style="CURSOR: hand; HEIGHT: 28px" onclick="vbscript:Call gCleanField(txtREAL_MED_CODE,txtREAL_MED_NAME)">
														코드</TD>
													<TD class="SEARCHDATA" width="60"><INPUT class="INPUT" id="txtREAL_MED_CODE" type="text" size="9" name="txtREAL_MED_CODE"
															style="WIDTH: 60px; HEIGHT: 22px"></TD>
													<TD class="SEARCHLABEL" width="50" style="CURSOR: hand; HEIGHT: 28px" onclick="vbscript:Call gCleanField(txtREAL_MED_NAME,txtREAL_MED_CODE)">
														매체사</TD>
													<TD class="SEARCHDATA"><INPUT class="INPUT" id="txtREAL_MED_NAME" style="WIDTH: 140px; HEIGHT: 22px" type="text"
															name="txtREAL_MED_NAME" tabIndex="1" size="17">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input id="chkGBN" title="매체구분" type="checkbox" name="chkGBN">
														전체</TD>
												</TR>
												<TR>
													<TD class="SEARCHLABEL" style="CURSOR: hand" onclick="vbscript:Call gCleanField(txtMEDCODE,txtMEDNAME)">
														코드</TD>
													<TD class="SEARCHDATA"><INPUT class="INPUT" id="txtMEDCODE" type="text" size="9" name="txtMEDCODE" style="WIDTH: 59px; HEIGHT: 22px"></TD>
													<TD class="SEARCHLABEL" style="CURSOR: hand" onclick="vbscript:Call gCleanField(txtMEDNAME,txtMEDCODE)">
														매체명</TD>
													<TD class="SEARCHDATA"><INPUT class="INPUT" id="txtMEDNAME" style="WIDTH: 140px; HEIGHT: 22px" type="text" size="18"
