<%@ Page CodeBehind="MDCMCATVCOMMITAX.aspx.vb" Language="vb" AutoEventWireup="false" Inherits="MD.MDCMCATVCOMMITAX" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<HEAD>
		<title>인쇄광고 수수료 세금계산서 발행</title> 
		<!--
'****************************************************************************************
'시스템구분 : MD/케이블 화면(MDCMCATVCOMMITAX.aspx)
...
Sub imgPrint_onclick ()
	...
	IF frmThis.sprSht.MaxRows = 0 then
		...
		with frmThis
			...
			'md_trans_temp삭제 시작
			intRtn = mobjMDCMCATVCOMMITAX.DeleteRtn_TEMP(gstrConfigXml)
			'md_trans_temp삭제 끝
			...
			for i=1 to .sprSht.MaxRows
				IF mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",i) = "1" THEN
					...
					vntDataTemp = mobjMDCMCATVCOMMITAX.ProcessRtn_TEMP(gstrConfigXml,strTAXYEARMON, strTAXNO, VATFLAG, FLAG, i, strUSERID)
				END IF
			next
...
Sub REAL_MED_CODE1_POP
	...
	Dim vntInParams
	with frmThis
		vntInParams = array(.txtTRANSYEARMON1.value, .txtREAL_MED_CODE1.value, .txtREAL_MED_NAME1.value, "CATV") 
...
Sub txtREAL_MED_NAME1_onkeydown
	if window.event.keyCode = meEnter then
		...
		with frmThis
			...
			vntData = mobjMDCOGET.GetTAXREAL_MED_NO(gstrConfigXml,mlngRowCnt,mlngColCnt,.txtTRANSYEARMON1.value,.txtREAL_MED_CODE1.value,.txtREAL_MED_NAME1.value, "CATV")
	...
	with frmThis
		...
		Else
			If .rdT.checked = True Then
				...
				gShowModalWindow "../MDCT/MDCMCATVCOMMITAXDTL.aspx",vntInParams , 898,680
				gFlowWait meWAIT_ON
				SelectRtn
				gFlowWait meWAIT_OFF
			End IF
...
Sub InitPage()
	'서버업무객체 생성	
	set mobjMDCMCATVCOMMITAX	= gCreateRemoteObject("cMDCT.ccMDCTCATVCOMMITAX")
...
Sub EndPage()
	set mobjMDCMCATVCOMMITAX = Nothing
...
Sub SelectRtn ()
	...
	with frmThis
		...
		'세금계산서 완료조회
		If .rdT.checked = True Then
			vntData = mobjMDCMCATVCOMMITAX.Get_CATV_TAX(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON, strREAL_MED_CODE, strFROM, strTO)
		...
		ElseIf .rdF.checked = True Then			
			vntData = mobjMDCMCATVCOMMITAX.Get_CATV_TAXBUILD(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON, strREAL_MED_CODE, strFROM, strTO, strGUBUN, strGROUPGBN)
		...
		ElseIf .rdA.checked = True Then
			vntData = mobjMDCMCATVCOMMITAX.Get_CATV_TAXALL(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON, strREAL_MED_CODE, strFROM, strTO, strGUBUN, strGROUPGBN)
...
Sub ProcessRtn ()
	...
	with frmThis
		...
		intTAXNO = 0
		If .cmbGUBUN.value = "taxdiv" Then
		intRtn = mobjMDCMCATVCOMMITAX.ProcessRtn_Div(gstrConfigXml,vntData, intTAXNO)
		Else
			...
			Else
				...
				Next
				'맥스값만 추가하여 보내기
				intRtn = mobjMDCMCATVCOMMITAX.ProcessRtn_Group(gstrConfigXml,vntData, intTAXNO,intColFlag)
			End IF
		End If
	...
	with frmThis
		...
		for i = .sprSht.MaxRows to 1 step -1
			if mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",i) = 1 THEN
				...
				intRtn = mobjMDCMCATVCOMMITAX.DeleteRtn_CommiTax(gstrConfigXml,strTAXYEARMON, strTAXNO)
