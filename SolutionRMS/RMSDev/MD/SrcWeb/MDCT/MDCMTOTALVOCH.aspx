<%@ Page Language="vb" AutoEventWireup="false" Codebehind="MDCMTOTALVOCH.aspx.vb" Inherits="MD.MDCMTOTALVOCH" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<HEAD>
		<title>종합편성방송 전표생성</title>
...
Sub InitPage()
	...
	'서버업무객체 생성	
	Set mobjMDCMTOTALVOCH = gCreateRemoteObject("cMDCT.ccMDCTTOTALVOCH")
...
Sub Get_COMBO_VALUE ()		
	...
	With frmThis	
		...
		vntData = mobjMDCMTOTALVOCH.Get_COMBO_VALUE(gstrConfigXml,mlngRowCnt,mlngColCnt,"PD_PAYCODE")
...
Sub Get_SUBCOMBO_VALUE(strCODE, row, sprsht)
	...
	With frmThis   
		...
		strCODE = replace(strCODE,"-","")
       	...
       	vntData = mobjMDCMTOTALVOCH.Get_SUBCOMBO_VALUE(gstrConfigXml, mlngRowCnt, mlngColCnt, strCODE)
...
Sub SelectRtn_AMT ()
	...
	with frmThis
		...
		vntData = mobjMDCMTOTALVOCH.SelectRtn_AMT(gstrConfigXml, mlngRowCnt, mlngColCnt, strYEARMON, _
...
Sub SelectRtn_GEN ()
	...
	with frmThis
		...
		vntData = mobjMDCMTOTALVOCH.SelectRtn_GEN(gstrConfigXml, mlngRowCnt, mlngColCnt, strYEARMON, _
												 strCLIENTCODE, strCLIENTNAME, strREAL_MED_CODE, strREAL_MED_NAME, _
												 strGBN, strSALEGBN)
...
Sub SelectRtn_GENDTL (Col, Row)
	...
	with frmThis
		...
		if .rdSALE.checked then
			vntData = mobjMDCMTOTALVOCH.SelectRtn_GENDTL(gstrConfigXml,mlngRowCnt,mlngColCnt, strTAXYEARMON, strTAXNO)
		elseif .rdBUY.checked then
			vntData = mobjMDCMTOTALVOCH.SelectRtn_GENBUYDTL(gstrConfigXml,mlngRowCnt,mlngColCnt, strTAXYEARMON, strTAXNO)
		else
			vntData = mobjMDCMTOTALVOCH.SelectRtn_GENMCBUYDTL(gstrConfigXml,mlngRowCnt,mlngColCnt, strTAXYEARMON, strTAXNO)
		end if
...
Sub SelectRtn_SUSU ()
	...
	with frmThis
		...
		vntData = mobjMDCMTOTALVOCH.SelectRtn_SUSU(gstrConfigXml, mlngRowCnt, mlngColCnt, strYEARMON, _
												  strCLIENTCODE, strCLIENTNAME, strREAL_MED_CODE, strREAL_MED_NAME, _
												  strGBN)
...
Sub SelectRtn_SUSUDTL (Col, Row)
	...
	with frmThis
		...
		vntData = mobjMDCMTOTALVOCH.SelectRtn_SUSUDTL(gstrConfigXml,mlngRowCnt,mlngColCnt, strTAXYEARMON, strTAXNO)
...
Sub SelectRtn_OUT ()
	...
	with frmThis
		...
		vntData = mobjMDCMTOTALVOCH.SelectRtn_OUT(gstrConfigXml, mlngRowCnt, mlngColCnt, strYEARMON, strEDYEARMON, _
												strCLIENTCODE, strCLIENTNAME, strREAL_MED_CODE, strREAL_MED_NAME, _
			...
			For intCnt = 1 To .sprSht_SUSUDTL.MaxRows
				...
				if strIF_CNT = "1" then
					...
					strITEMLIST = strITEMLIST + ":" + cstr(strHSEQ) + "|" + _
								...
								replace(mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"DOCUMENTDATE",intCnt),"-","") + "|" + _
								mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"PREPAYMENT",intCnt) + "|" + _
								replace(mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"oFROMDATE",intCnt),"-","") + "|" + _
								replace(mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"TODATE",intCnt),"-","") + "|" + _
								mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"SUMMTEXT",intCnt) + "|" + _
								mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"AMTGBN",intCnt) + "|" + _
								"" + "|" + _ 
								...
								mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"VOCHNO",intCnt) + "|" + _
								"" + "|" + _ 
								mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"f",intCnt)
				end if
...
Sub ProcessRtn_OUT()
	...
	with frmThis
		vntData_ProcesssRtn = mobjSCGLSpr.GetDataRows(.sprSht_OUT,"CHK | POSTINGDATE | CLIENTNAME | CUSTOMERCODE | CUSTNAME | SUMM | BA | COSTCENTER | AMT | VAT | SEMU | BP | DEMANDDAY | DUEDATE | VENDOR | GBN | ACCOUNT | DEBTOR | DOCUMENTDATE | PAYCODE | PREPAYMENT | FROMDATE | TODATE | SUMMTEXT | TAXYEARMON | TAXNO | VOCHNO | ERRCODE | ERRMSG | GFLAG | MEDFLAG | AMTGBN | TRANSRANK")
		'처리 업무객체 호출
		if  not IsArray(vntData_ProcesssRtn) then 
			gErrorMsgBox "변경된 " & meNO_DATA,"저장취소"
			exit sub
...
Sub Set_VochValue (strRETURNLIST)
	...
	With frmThis
		...
		if mstrPROCESS ="Create" then
			IF mstrGUBUN = "T" THEN
				intRtn = mobjMDCMTOTALVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "T")
			ELSEIF mstrGUBUN = "D" THEN
				if .rdSALE.checked then
					intRtn = mobjMDCMTOTALVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "T")
				ELSEif .rdMCBUY.checked then
					intRtn = mobjMDCMTOTALVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "TAA")
				else
					intRtn = mobjMDCMTOTALVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "TD")
				end if
			ELSEIF mstrGUBUN = "S" THEN
				intRtn = mobjMDCMTOTALVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "T")
			ELSEIF mstrGUBUN = "O" THEN
				intRtn = mobjMDCMTOTALVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "T2")
			END IF
   		...
   		elseif mstrPROCESS ="Delete" then
   			IF mstrGUBUN = "D" THEN
				if .rdSALE.checked then
					intRtn = mobjMDCMTOTALVOCH.VOCHDELL(gstrConfigXml, strRETURNLIST, mstrGUBUN, "TOTAL" )
				ELSEif .rdMCBUY.checked then
					intRtn = mobjMDCMTOTALVOCH.VOCHDELL(gstrConfigXml, strRETURNLIST, mstrGUBUN, "TOTAL" )
				else
					intRtn = mobjMDCMTOTALVOCH.VOCHDELL_BUY(gstrConfigXml, strRETURNLIST, mstrGUBUN, "TOTAL", "TD")
				end if
			elseIF mstrGUBUN = "O" THEN
				intRtn = mobjMDCMTOTALVOCH.VOCHDELL_BUY(gstrConfigXml, strRETURNLIST, mstrGUBUN, "TOTAL", "T2")
			ELSE
				intRtn = mobjMDCMTOTALVOCH.VOCHDELL(gstrConfigXml, strRETURNLIST, mstrGUBUN, "TOTAL" )
			END IF
...
sub ErrVochDeleteRtn
	...
	with frmThis
		...
		intRtn = mobjMDCMTOTALVOCH.DeleteRtn(gstrConfigXml,vntData)
		...
		If mstrGUBUN = "T"  then  
			for i = .sprSht_AMT.MaxRows to 1 step -1
				If mobjSCGLSpr.GetTextBinding(.sprSht_AMT,"CHK",i) = 1 Then
					...
					intRtn = mobjMDCMTOTALVOCH.DeleteRtn_GANG(gstrConfigXml,strTAXYEARMON, strTAXNO, strVOCHNO, mstrGUBUN, "TOTAL" )
			...
			for i = .sprSht_GEN.MaxRows to 1 step -1
				If mobjSCGLSpr.GetTextBinding(.sprSht_GEN,"CHK",i) = 1 Then
					...
					ELSEIF .rdMCBUY.checked then
						...
						intRtn = mobjMDCMTOTALVOCH.DeleteRtn_GANG_BUY(gstrConfigXml,strTAXYEARMON, strTAXNO, strVOCHNO, mstrGUBUN, "TOTAL", "TD")
					end if
		...
		ELSEIf mstrGUBUN = "O"  then
			for i = .sprSht_OUT.MaxRows to 1 step -1
				If mobjSCGLSpr.GetTextBinding(.sprSht_OUT,"CHK",i) = 1 Then
					...
					intRtn = mobjMDCMTOTALVOCH.DeleteRtn_GANG_BUY(gstrConfigXml,strTAXYEARMON, strTAXNO, strVOCHNO, mstrGUBUN, "TOTAL", "T2")
							...
							<TR>
								<TD id="TD1" align="left" width="400" height="20" runat="server">
									<table cellSpacing="0" cellPadding="0" width="100%" border="0">
										<tr>
											<td align="left">
												<TABLE cellSpacing="0" cellPadding="0" width="137" background="../../../images/back_p.gIF"
													border="0">
													<TR>
