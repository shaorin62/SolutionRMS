<%@ Page CodeBehind="MDCMTOTALTRUTAX.aspx.vb" Language="vb" AutoEventWireup="false" Inherits="MD.MDCMTOTALTRUTAX" %>
...
<HTML>
	<HEAD>
		...
		<!--
'****************************************************************************************
'시스템구분 : MD/종합편성 화면(MDCMTOTALTRUTAX.aspx)
...
Sub imgPrint_onclick ()
	...
	IF frmThis.sprSht.MaxRows = 0 then
		...
		with frmThis
			...
			'md_trans_temp삭제 시작
			intRtn = mobjMDCMTOTALTRUTAX.DeleteRtn_TEMP(gstrConfigXml)
			'md_trans_temp삭제 끝
			...
			for i=1 to .sprSht.MaxRows
				IF mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",i) = "1" THEN
					...
					vntDataTemp = mobjMDCMTOTALTRUTAX.ProcessRtn_TEMP(gstrConfigXml,strTAXYEARMON, strTAXNO, VATFLAG, FLAG, i, strUSERID)
				END IF
			next
		...
		If .cmbGUBUN.value = "taxdiv" Then
			For intCnt = 1 To .sprSht.MaxRows
				If  mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",intCnt) = 1 Then
					...
					mobjSCGLSpr.setTextBinding .sprSht,"SUMM", intCnt, "종합편성광고비 - (" & mobjSCGLSpr.GetTextBinding(.sprSht,"MEDNAME",intCnt) & ")"
				End If
			Next
		Elseif  .cmbGUBUN.value = "taxgroup" Then
			For intCnt = 1 To .sprSht.MaxRows
				If  mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",intCnt) = 1 Then
					mobjSCGLSpr.setTextBinding .sprSht,"DEMANDDAY",intCnt,strDEMANDDAY
					mobjSCGLSpr.setTextBinding .sprSht,"TAXYEARMON",intCnt,strTAXYEARMON
					mobjSCGLSpr.setTextBinding .sprSht, "SUMM", intCnt, "종합편성광고비"
				End If
			Next
		End If
	End With
...
Sub CLIENTCODE1_POP
	...
	Dim vntInParams
	with frmThis
		vntInParams = array(.txtTRANSYEARMON1.value, .txtCLIENTCODE1.value, .txtCLIENTNAME1.value, "TOTAL") 
...
Sub txtCLIENTNAME1_onkeydown
	if window.event.keyCode = meEnter then
		...
		with frmThis
			...
			vntData = mobjMDCOGET.GetTAXCUSTNO(gstrConfigXml,mlngRowCnt,mlngColCnt,.txtTRANSYEARMON1.value,.txtCLIENTCODE1.value,.txtCLIENTNAME1.value, "TOTAL")
...
Sub TIMCODE1_POP
	...
	with frmThis
		vntInParams = array(trim(.txtTRANSYEARMON1.value), trim(.txtCLIENTCODE1.value), trim(.txtCLIENTNAME1.value), _
							trim(.txtTIMCODE1.value), trim(.txtTIMNAME1.value), "TOTAL")
...
Sub InitPage()
	'서버업무객체 생성	
	set mobjMDCMTOTALTRUTAX	= gCreateRemoteObject("cMDCT.ccMDCTTOTALTRUTAX")
...
Sub EndPage()
	set mobjMDCMTOTALTRUTAX = Nothing
...
Sub Get_COMBO_VALUE ()
	...
	With frmThis
		...
		vntData = mobjMDCMTOTALTRUTAX.Get_COMBOVOCH_VALUE(gstrConfigXml,mlngRowCnt,mlngColCnt)
...
Sub SelectRtn ()
	...
	with frmThis
		...
		'세금계산서 완료조회
		If .rdT.checked = True Then
			vntData = mobjMDCMTOTALTRUTAX.Get_TOTAL_TAX(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON, strCLIENTCODE, strFROM, strTO)
		...
		ElseIf .rdF.checked = True Then	
			vntData = mobjMDCMTOTALTRUTAX.Get_TOTAL_TAXBUILD(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON,strCLIENTCODE,strTIMCODE, strFROM, strTO, strGUBUN, strVOCH_TYPE_TEMP, strGROUPGBN)
		...
		ElseIf .rdA.checked = True Then
			strMED_FLAG = "T"		
			vntData = mobjMDCMTOTALTRUTAX.Get_TOTAL_TAXALL(gstrConfigXml,mlngRowCnt,mlngColCnt, strYEARMON,strCLIENTCODE,strTIMCODE, strFROM, strTO, strGUBUN, strMED_FLAG, strVOCH_TYPE_TEMP)
...
Sub ProcessRtn ()
	...
	with frmThis
		...
		intTAXNO = 0
		If .cmbGUBUN.value = "taxdiv" Then
			intRtn = mobjMDCMTOTALTRUTAX.ProcessRtn_Div(gstrConfigXml,vntData, intTAXNO)
		Elseif  .cmbGUBUN.value = "taxgroup" then	
			...
			Else
				...
				Next
				'맥스값만 추가하여 보내기
				intRtn = mobjMDCMTOTALTRUTAX.ProcessRtn_Group(gstrConfigXml,vntData, intTAXNO, intColFlag)
			End IF
		...
		elseif  .cmbGUBUN.value = "taxgeneralgroup" then	
			If Not TaxGeneralGroup(strVALIDATION) Then 
				...
				Exit Sub
			Else
				intRtn = mobjMDCMTOTALTRUTAX.ProcessRtn_GeneralGroup(gstrConfigXml,vntData, intTAXNO)
			End IF
	...
	with frmThis
		...
		for i = .sprSht.MaxRows to 1 step -1
			if mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",i) = 1 THEN
				...
				intRtn = mobjMDCMTOTALTRUTAX.DeleteRtn_TruTax(gstrConfigXml,strTAXYEARMON, strTAXNO,strVOCH_TYPE)
