<%@ Page Language="vb" AutoEventWireup="false" Codebehind="PDCMPREESTDTL.aspx.vb" Inherits="PD.PDCMPREESTDTL" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<HEAD>
		<title>가견적 내역관리</title>
		<meta content="False" name="vs_showGrid">
		<META http-equiv="Content-Type" content="text/html; charset=ks_c_5601-1987">
		<!--
'****************************************************************************************
'시스템구분 : 가견적내역관리 화면(PDCMPREESTDTL)
...
Sub imgRowDel_onclick
	...
	if frmThis.txtENDFLAG.value = "T" Then
		gErrorMsgBox "거래명세서가 작성되어 삭제가 불가능 합니다.","삭제안내!"
		Exit Sub
	End If
	'if frmThis.txtENDFLAGEXE.value = "T" Then
	'	gErrorMsgBox "외주비 지출내역이 작성되어 삭제가 불가능 합니다.","삭제안내!"
	'	Exit Sub
	'End If
		gFlowWait meWAIT_ON
		DeleteRtn
...
Sub imgRowAdd_onclick ()
	if frmThis.txtENDFLAG.value = "T" Then
		gErrorMsgBox "거래명세서가 작성되어 행추가가 불가능 합니다.","처리안내!"
		Exit Sub
	End If
	'if frmThis.txtENDFLAGEXE.value = "T" Then
	'	gErrorMsgBox "외주비 지출내역이 작성되어 행추가가 불가능 합니다.","처리안내!"
	'	Exit Sub
	'End If
	call sprSht_Keydown(meINS_ROW, 0)
...
Sub InitPage()
	...
	SelectRtn
	'msgbox frmThis.txtENDFLAG.value
End Sub
...
Dim strAGREEDAY		
if frmThis.txtENDFLAG.value = "T" Then
		gErrorMsgBox "거래명세서가 작성되어 확정이 불가능 합니다.","확정처리안내!"
		Exit Sub
End if
if frmThis.txtENDFLAGEXE.value = "T" Then
		gErrorMsgBox "외주비 지출내역이 작성되어 확정이 불가능 합니다.","확정처리안내!"
		Exit Sub
End if
	
	with frmThis
	If .txtAGREEYEARMON.value = "" Then
		gErrorMsgBox "견적확정일을 넣으십시오.","확정처리안내!"
		.txtAGREEYEARMON.focus()
		Exit Sub
	End if
	
	intRtnConfirm = gYesNoMsgbox("자료를 확정 하시겠습니까?","자료확정 확인")
	IF intRtnConfirm <> vbYes then exit Sub 
	strAGREEDAY = MID(.txtAGREEYEARMON.value,1,4) & MID(.txtAGREEYEARMON.value,6,2) & MID(.txtAGREEYEARMON.value,9,2)
	intRtn = mobjPDCMPREESTDTL.ProcessRtn_Confirm(gstrConfigXml,Trim(.txtPREESTNO.value),Trim(.txtJOBNO.value),strAGREEDAY)
			if not gDoErrorRtn ("ProcessRtn_Confirm") then
				gErrorMsgBox " 자료가 확정 되었습니다.","확정처리안내" 
...
if frmThis.txtENDFLAG.value = "T" Then
		gErrorMsgBox "거래명세서가 작성되어 확정취소가 불가능 합니다.","확정취소안내!"
		Exit Sub
End if
if frmThis.txtENDFLAGEXE.value = "T" Then
		gErrorMsgBox "외주비 지출내역이 작성되어 확정취소가 불가능 합니다.","확정취소안내!"
		Exit Sub
End if
Dim intRtnConfirm
Dim intRtn
...
Sub ProcessRtn ()
	...
	'On error resume next
		...
		'vntData = mobjSCGLSpr.GetDataRows(.sprSht,"PREESTNO|ITEMCODESEQ|DIVNAME|CLASSNAME|ITEMCODE|ITEMCODENAME|STD|COMMIFLAG|QTY|PRICE|AMT")
		'처리 업무객체 호출
							...
							<TR>
								<TD class="DATAFRAME" style="WIDTH: 100%; HEIGHT: 72px" vAlign="top" align="center">
									...
									<TABLE class="DATA" id="tblDATA" cellSpacing="1" cellPadding="0" width="1040" border="0" align="left">
										<TR>
											<TD class="LABEL" style="CURSOR: hand" width="80">가견적 코드</TD>
											...
											<TD class="LABEL" style="WIDTH: 92px; CURSOR: hand" width="92">가견적명</TD>
											<TD class="DATA" width="260"><INPUT dataFld="PREESTNAME" class="NOINPUT_L" id="txtPREESTNAME" title="가견적명" style="WIDTH: 255px; HEIGHT: 22px"
													dataSrc="#xmlBind" readOnly type="text" size="37" name="txtPREESTNAME" accessKey="M"></TD>
											...
											<TD class="DATA"><INPUT dataFld="AGREEYEARMON" class="INPUT" id="txtAGREEYEARMON" title="견적합의일" style="WIDTH: 96px; HEIGHT: 22px"
													accessKey="DATE" dataSrc="#xmlBind" type="text" maxLength="10" size="10" name="txtAGREEYEARMON"><IMG id="imgCalEndarAGREE" onmouseover="JavaScript:this.src='../../../images/imgCalEndarOn.gIF'"
										...
										<TR>
											...
											<TD class="DATA"><INPUT class="INPUT" id="txtPRINTDAY" title="견적서발행일" style="WIDTH: 96px; HEIGHT: 22px"
													...
													width="54" align="absMiddle" border="0" name="imgPrint">&nbsp;<INPUT id="txtJOBNO" style="WIDTH: 28px; HEIGHT: 21px" type="hidden" size="1" name="txtJOBNO"
													dataFld="JOBNO" dataSrc="#xmlBind"></TD>
										</TR>
										<TR>
