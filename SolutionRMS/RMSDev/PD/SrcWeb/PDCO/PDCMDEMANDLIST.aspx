<%@ Page Language="vb" AutoEventWireup="false" Codebehind="PDCMDEMANDLIST.aspx.vb" Inherits="PD.PDCMDEMANDLIST" %>
...
'팀
		...
		Elseif Col = mobjSCGLSpr.CnvtDataField(.sprSht1,"DIVAMT")  Then
			...
			intAMT = mobjSCGLSpr.GetTextBinding(.sprSht,"ADJAMT", .sprSht.ActiveRow)

			intMAXDIVAMT = 0
			for intCnt = 1 to .sprSht1.MaxRows 
				intMAXDIVAMT = intMAXDIVAMT + mobjSCGLSpr.GetTextBinding(.sprSht1,"DIVAMT",intCnt)
			Next

			
			'If intMAXDIVAMT > intAMT Then
   			'	gErrorMsgBox "하위의 분할대상금액은 상단의 청구금액 보다 클수 없습니다.","입력안내!"
			...
			'	intPreCharge = mobjSCGLSpr.GetTextBinding(.sprSht,"ADJAMT", .sprSht.ActiveRow)
				
				
			'	for intCnt = 1 to .sprSht1.MaxRows 
			'		intPreCharge = intPreCharge - mobjSCGLSpr.GetTextBinding(.sprSht1,"DIVAMT", intCnt)
			'	Next
			'	mobjSCGLSpr.SetTextBinding .sprSht1,"CHARGE",.sprSht1.MaxRows, intPreCharge
...
Sub InitPage()
    ...
    With frmThis
		...
		mobjSCGLSpr.SetCellsLock2 .sprSht1,true, "SAVEFLAG|SUBNO|SEQ|USENO|CHARGE|CONFIRMFLAG|ADJAMT"
...
Sub InitPageData
	...
	with frmThis
		.sprSht.maxrows = 0
		.txtYEARMON1.value  = MID(gNowDate2,1,4) & MID(gNowDate2,6,2) '추후 이것으로 대처 임시로 테스트값 연결 하였음
		.txtYEARMON2.value  = MID(gNowDate2,1,4) & MID(gNowDate2,6,2) '추후 이것으로 대처 임시로 테스트값 연결 하였음
...
Sub Get_COMBO_VALUE ()		
	...
	With frmThis	
		...
		vntData_Demand = mobjPDCODEMAND.Get_COMBO_VALUE2(gstrConfigXml,mlngRowCnt,mlngColCnt,"PD_DEMAND")
...
Sub DefaultValue
	...
	with frmThis
		...
		If mobjSCGLSpr.GetTextBinding(.sprSht,"DEMANDFLAG",.sprSht.ActiveRow) = "DI03" Or mobjSCGLSpr.GetTextBinding(.sprSht,"DEMANDFLAG",.sprSht.ActiveRow) = "DI04" Then 
			...
			For intCnt = 1 To .sprSht1.MaxRows
				dblAMT = 0
				dblAMT = mobjSCGLSpr.getTextBinding(.sprSht1,"ADJAMT",intCnt) 
				dblSumAMT = dblSumAMT + dblAMT
				
				IF mobjSCGLSpr.getTextBinding(.sprSht1,"DIVAMT",intCnt) - mobjSCGLSpr.getTextBinding(.sprSht1,"ADJAMT",intCnt) = 0 THEN
					mobjSCGLSpr.SetTextBinding .sprSht1,"CHARGE",intCnt,0
				END IF
			Next
			
			If .sprSht1.ActiveRow = 1 Then
				dblSetAmt = mobjSCGLSpr.getTextBinding(.sprSht,"ADJAMT",.sprSht.ActiveRow)
			else
				dblSetAmt = mobjSCGLSpr.getTextBinding(.sprSht,"ADJAMT",.sprSht.ActiveRow)- dblSumAMT
			End If
			...
			intGBN = mobjSCGLSpr.GetTextBinding(.sprSht,"DIVAMT",.sprsht.ActiveRow)
			
			'견적금액이 - 이면 
			'if mid(intGBN,1,1) = "-" then
			'	If dblSetAmt => 0 Then
			'		gErrorMsgBox "분할대상금액은 청구확정 금액을 넘을수 없습니다.","처리안내"
			'		
			'		mobjSCGLSpr.SetTextBinding .sprSht1,"CHARGE",.sprSht1.ActiveRow - 1,dblSetAmt
			'		mobjSCGLSpr.DeleteRow .sprSht1,.sprSht1.ActiveRow
			'		Exit Sub
			'	End If
			''견적금액이 + 이면 
			'else
			'	If dblSetAmt <= 0 Then
			'		gErrorMsgBox "분할대상금액 은 청구확정 금액을 넘을수 없습니다.","처리안내"
			'		mobjSCGLSpr.SetTextBinding .sprSht1,"CHARGE",.sprSht1.ActiveRow - 1,dblSetAmt
			'		mobjSCGLSpr.DeleteRow .sprSht1,.sprSht1.ActiveRow
			'		Exit Sub
			'	End If
			...
			if .sprSht1.ActiveRow > 1 Then
				mobjSCGLSpr.SetTextBinding .sprSht1,"CHARGE",.sprSht1.MaxRows-1,0
			end if 
		...
		ELSE
			If mobjSCGLSpr.GetTextBinding(.sprSht,"DEMANDFLAG",.sprSht.ActiveRow) = "" Then 
				gErrorMsgBox "청구기준을 선택하셔야 합니다.","처리안내"
				mobjSCGLSpr.DeleteRow .sprSht1,.sprSht1.ActiveRow
				Exit Sub
			else
				gErrorMsgBox "견적금액은 분할할수 없습니다.","처리안내"
				mobjSCGLSpr.DeleteRow .sprSht1,.sprSht1.ActiveRow
				Exit Sub
			End if 
		END IF 
...
Sub SelectRtn
	...
	with frmThis
		...
		vntData = mobjPDCODEMAND.SelectRtn(gstrConfigXml,mlngRowCnt,mlngColCnt,.txtYEARMON1.value,.txtYEARMON2.value,.txtJOBNAME.value,.txtJOBNO.value,mstrDEPTCD,mstrMANAGER)
			...
			if mlngRowCnt > 0 Then
				...
				'mobjSCGLSpr.ColHidden .sprSht,strCols,true
   				...
   				For intCnt = 1 To .sprSht.MaxRows
					...
					If mobjSCGLSpr.GetTextBinding(.sprSht,"SUBNO",intCnt) > 100 Then
...
Sub ProcessRtn_HDR
	...
	with frmThis
		...
		If .txtYEARMON1.value = "" OR  .txtYEARMON2.value = "" Then
			gErrorMsgBox "등록월이 비어있습니다.","저장오류"
			EXIT SUB
		End If
...
Function DataValidation ()
	...
	with frmThis
   		...
   		for intCnt = 1 to .sprSht1.MaxRows
			...
			If mobjSCGLSpr.getTextBinding(.sprSht1,"DEMANDFLAG",intCnt) = ""  Then
   				gErrorMsgBox "하단의 " & intCnt & " 번째 로우의 청구 기준을 선택하세요.!","저장오류"
   				exit Function
   			end if 
		next
		
   		'If mobjSCGLSpr.getTextBinding(.sprSht,"ADJAMT",.sprSht.ActiveRow) < dblSumAmt Then
   		'	gErrorMsgBox "하단의 분할대상금액의 합은 상단의 청구금액을 초과할수 없습니다","저장오류"
   		'	Exit Function
   		'End If
   		
   		If mobjSCGLSpr.getTextBinding(.sprSht1,"CHARGE",.sprSht1.MaxRows) <> 0 Then
   			gErrorMsgBox "분할금액이 모자릅니다.","저장오류"
   			Exit Function
   		End If
			...
			<TABLE id="tblForm" style="WIDTH: 100%; HEIGHT: 100%" cellSpacing="0" cellPadding="0" border="0">
				<TR valign="top">
					<TD>
						...
						<TABLE cellSpacing="0" cellPadding="0" width="1024" background="../../../images/TitleBG.gIF"
							border="0">
							<TR>
									...
									<TABLE class="SEARCHDATA" id="tblKey" cellSpacing="1" cellPadding="0" width="100%" border="0">
										<TR>
											<TD class="SEARCHLABEL" title="세금계산서조회 조건을 삭제합니다" style="CURSOR: hand" onclick="vbscript:Call gCleanField(txtYEARMON1, '')"
												width="80">등록월</TD>
											<TD class="SEARCHDATA" width="200"><INPUT class="INPUT" id="txtYEARMON1" title="등록월" style="WIDTH: 88px; HEIGHT: 22px" accessKey="NUM"
												...
												~ <INPUT class="INPUT" id="txtYEARMON2" title="등록월" style="WIDTH: 88px; HEIGHT: 22px" accessKey="NUM"
													type="text" maxLength="6" onchange="vbscript:Call gYearmonCheck(txtYEARMON2)" size="9"
													name="txtYEARMON2"></TD>
											<TD class="SEARCHLABEL" title="JOBNO 조건을 삭제합니다" style="CURSOR: hand" onclick="vbscript:Call gCleanField(txtJOBNAME, txtJOBNO)"
												width="80">JOBNO</TD>
											<TD class="SEARCHDATA"><INPUT class="INPUT_L" id="txtJOBNAME" title="조회용JOB명" style="WIDTH: 208px; HEIGHT: 22px"
													type="text" maxLength="100" size="29" name="txtJOBNAME"> <IMG id="ImgJOBNO" onmouseover="JavaScript:this.src='../../../images/imgPopupOn.gIF'"
													style="CURSOR: hand" onmouseout="JavaScript:this.src='../../../images/imgPopup.gIF'" src="../../../images/imgPopup.gIF" align="absMiddle" border="0"
													name="ImgJOBNO"> <INPUT class="INPUT" id="txtJOBNO" title="조회용광고주코드" style="WIDTH: 57px; HEIGHT: 22px" type="text"
													maxLength="7" size="4" name="txtJOBNO"></TD>
											<td class="SEARCHDATA" width="315" align="right"><IMG id="imgQuery" onmouseover="JavaScript:this.src='../../../images/imgQueryOn.gIF'"
				...
				<TR>
					<TD style="WIDTH: 100%; HEIGHT: 50%" vAlign="top" align="left">
						<OBJECT id="sprSht" style="WIDTH: 100%; HEIGHT: 100%" classid="clsid:41F841C1-AE16-11D5-8817-0050DA6EF5E5">
							...
							<PARAM NAME="_ExtentY" VALUE="6059">
				...
				<TR>
					<TD style="WIDTH: 100%; HEIGHT: 50%" vAlign="top" align="left">
						<OBJECT id="sprSht1" style="WIDTH: 100%; HEIGHT: 95%" classid="clsid:41F841C1-AE16-11D5-8817-0050DA6EF5E5"
							...
							<PARAM NAME="_Version" VALUE="393216">
							<PARAM NAME="_ExtentX" VALUE="31856">
							<PARAM NAME="_ExtentY" VALUE="5768">
							<PARAM NAME="_StockProps" VALUE="64">
							<PARAM NAME="Enabled" VALUE="-1">
							<PARAM NAME="AllowCellOverflow" VALUE="0">
							<PARAM NAME="AllowDragDrop" VALUE="0">
