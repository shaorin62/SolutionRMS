<%@ Page Language="vb" AutoEventWireup="false" Codebehind="PDCMCONTRACT_BASE.aspx.vb" Inherits="PD.PDCMCONTRACT_BASE" %>
...
Sub InitPage()
    ...
    With frmThis
		...
		mobjSCGLSpr.SetColWidth .sprSht, "-1","  4|       8|         9|        10|         9|2|    15|    16|   8|     8|        10|        10|      12|   0"
		...
		mobjSCGLSpr.SetCellsLock2 .sprSht, True, "CONTRACTNO | GBN | GUBUN | CONFIRMFLAG"
...
Sub InitPageData
	...
	With frmThis
		...
		'승인 화면이 생김에 따라 이곳에서 승인을 지정하지 않는다.
		.chkCONFIRMFLAG.className = "NOINPUT"
		.chkCONFIRMFLAG.disabled = true
		...
		If .sprSht.MaxRows > 0 Then
			...
			'기본계약서의 경우 계약기간을 정하지 않는다.
				if .cmbGUBUN.value = "기본" then
					.txtSTDATE.className			= "NOINPUT"   : .txtSTDATE.readOnly			= True
					.txtEDDATE.className			= "NOINPUT"   : .txtEDDATE.readOnly			= True
					mobjSCGLSpr.SetCellsLock2 .sprSht, True, "STDATE | EDDATE"
					
				else
					.txtSTDATE.className			= "INPUT"   : .txtSTDATE.readOnly			= False
					.txtEDDATE.className			= "INPUT"   : .txtEDDATE.readOnly			= False
					mobjSCGLSpr.SetCellsLock2 .sprSht, false, "STDATE | EDDATE"
				end if
				.cmbGUBUN.className				= "NOINPUT"   : .cmbGUBUN.disabled			= True 
				.cmbGBN.className				= "NOINPUT"   : .cmbGBN.disabled			= True 
				...
				mobjSCGLSpr.SetCellsLock2 .sprSht, True, "OUTSCODE | BTN  | OUTSNAME"
			Else 
				'기본계약서의 경우 계약기간을 정하지 않는다.
				if .cmbGUBUN.value = "기본" then
					.txtSTDATE.className			= "NOINPUT"   : .txtSTDATE.readOnly			= True
...
Sub ProcessRtn_HDR ()
	...
	With frmThis
		...
		lngchkCnt = 0 :  strCONTRACTNO = "" : strGUBUN = "" : strOUTSCODE = ""

		For i = 1 to .sprSht.MaxRows
			If mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",i) = 1 Then
				strOUTSCODE = mobjSCGLSpr.GetTextBinding(.sprSht,"OUTSCODE", i)
				mobjSCGLSpr.CellChanged frmThis.sprSht, i, 1
		...
		strCONTRACTNO = mobjSCGLSpr.GetTextBinding(.sprSht,"CONTRACTNO", .sprSht.ActiveRow)
		
		'신규 저장 데이터만  validation 하기 위해.
		For j = 1 to .sprSht.MaxRows
			if mobjSCGLSpr.GetTextBinding(.sprSht,"CONTRACTNO",j) = "" then
...
Function DataValidation (strOUTSCODE)
	...
	with frmThis
		'Master 입력 데이터 Validation : 필수 입력항목 검사 
   		...
   		vntData = mobjPDCMCONTRACT_BASE.SelctRtn_validation(gstrConfigXml, mlngRowCnt,mlngColCnt, strOUTSCODE, .cmbGBN.value)
   		...
   		If not gDoErrorRtn ("SelctRtn_validation") Then
			...
			If mlngRowCnt >0 Then
				'계약서 GBN 으로 구분 짓는다.
				if vntData(1,1) = "ATL" and vntData(0,1) = "기본" then
					if .cmbGBN.value = "ATL" and .cmbGUBUN.value = "기본" then
						gErrorMsgBox "해당 외주처의 ATL 기본계약서가 존재 합니다." ,"저장안내"
						exit function
					elseif .cmbGBN.value = "ATL" and .cmbGUBUN.value = "단가" then
						intYNRtn = gYesNoMsgbox("기본계약서가 존해 합니다 단가계약서를 추가 생성 하시겠습니까?","확정확인")
						IF intYNRtn <> vbYes then exit function	
					end if
				elseif vntData(1,1) = "ATL" and vntData(0,1) = "단가" then
					if .cmbGBN.value = "ATL" and (.cmbGUBUN.value = "기본" or .cmbGUBUN.value = "단가") then
						gErrorMsgBox "해당 외주처의 ATL 단가계약서가 존재 합니다. ATL 기본 및 단가 계약서를 생성할 수 없습니다." ,"저장안내"
						exit function
					end if
				elseif vntData(1,1) = "BTL" and vntData(0,1) = "기본" then
					gErrorMsgBox "해당 외주처의 BTL 기본계약서가 존재 합니다." ,"저장안내"
					exit function
				end if 
