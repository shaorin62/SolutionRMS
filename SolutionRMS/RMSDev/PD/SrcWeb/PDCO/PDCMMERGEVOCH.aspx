<%@ Page Language="vb" AutoEventWireup="false" Codebehind="PDCMMERGEVOCH.aspx.vb" Inherits="PD.PDCMMERGEVOCH" %>
	...
	with frmThis
		...
		if mstrGUBUN = "M"  then 
			...
			Elseif Trim(.txtSUMM.value) <> "" Then 
				...
				'For intCnt2 = 1 To .sprSht_SUSU.MaxRows
				'	If mobjSCGLSpr.GetTextBinding( .sprSht_SUSU,"CHK",intCnt2) = 1 Then 
				'		mobjSCGLSpr.SetTextBinding .sprSht_SUSU,"SUMM",intCnt2, .txtSUMM.value 
				'	End If
				'Next
...
Sub DeleteRtn_SUSUDTL (Row)
	...
	With frmThis
		...
		for i = .sprSht_SUSUDTL.MaxRows to 1 step -1
			strTAXYEARMON = mobjSCGLSpr.GetTextBinding(.sprSht_SUSU,"MTAXYEARMON",Row)
			strTAXNO = mobjSCGLSpr.GetTextBinding(.sprSht_SUSU,"MTAXNO",Row)

			if mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"MTAXYEARMON",i) = strTAXYEARMON and _
			   mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"MTAXNO",i) = strTAXNO then
...
Sub InitPage()
	'서버업무객체 생성	
	Set mobjPDCMMERGEVOCH	 = gCreateRemoteObject("cPDCO.ccPDCOMERGEVOCH")
    ...
    with frmThis
		...
		strSemuComboListB =  "B0" & vbTab & "D0" & vbTab & "B7" & vbTab & "C0"
		strSemuComboListA =  "I0" & vbTab & "X9" & vbTab & "V0"
		strBMORDER = "AD0110" & vbTab & "AD0120" & vbTab & "AD0130" & vbTab & "AD0140" & vbTab & "AD0150" & vbTab & "AD0160" & vbTab & "AD0190" _
					...
					& vbTab & "AD0420" & vbTab & "AD0430" & vbTab & "AD0440" & vbTab & "AD0450" & vbTab & "AD0510" & vbTab & "AD0610" & vbTab & "AD0005"
		'**************************************************
		'매출 시트 디자인 hdr
		'**************************************************	
		...
		mobjSCGLSpr.SpreadDataField .sprSht_SUSU,    "CHK | POSTINGDATE | CUSTOMERCODE | CUSTNAME | AMT | VAT | MTAXYEARMON | MTAXNO | VOCHNO | ERRCODE | ERRMSG | VOCH_TYPE"
		mobjSCGLSpr.SetHeader .sprSht_SUSU,		    "선택|전표일자|거래처코드|거래처|금액|부가세|RMS년월|RMS번호|전표번호|에러코드|에러메세지|VOCH_TYPE"
		mobjSCGLSpr.SetColWidth .sprSht_SUSU, "-1",  "  4|       8|        10|    20|  11|    11|      7|      7|      10|       0|        15|		  0"
		...
		mobjSCGLSpr.SetCellAlign2 .sprSht_SUSU, "CUSTOMERCODE | MTAXYEARMON | MTAXNO | VOCHNO",-1,-1,2,2,false '가운데
		...
		mobjSCGLSpr.SetCellsLock2 .sprSht_SUSU,true,"POSTINGDATE | CUSTOMERCODE | CUSTNAME | AMT | VAT | MTAXYEARMON | MTAXNO | VOCHNO | ERRCODE | ERRMSG | VOCH_TYPE"
		...
		mobjSCGLSpr.SpreadLayout .sprSht_SUSUDTL, 34, 0, 4
		mobjSCGLSpr.SpreadDataField .sprSht_SUSUDTL,    "MEDFLAGNAME | POSTINGDATE | CUSTOMERCODE | CUSTNAME | SUMM | BA | COSTCENTER | DEPT_NAME | AMT | VAT | SEMU | BP | DEMANDDAY | DUEDATE  | VENDOR | GBN | ACCOUNT | DEBTOR | BMORDER | DOCUMENTDATE | PREPAYMENT | FROMDATE | TODATE | SUMMTEXT | MTAXYEARMON | MTAXNO | MTAXNOSEQ | VOCHNO | ERRCODE | ERRMSG | GFLAG | AMTGBN | MEDFLAG | RMSNO"
		mobjSCGLSpr.SetHeader .sprSht_SUSUDTL,		    "구분|전표일자|거래처코드|거래처|적요|사업영역|코스트센터|담당부서|금액|부가세|세무코드|BP|지급기일|지급일|VENDOR|구분|차변계정|계정|BMORDER|증빙일|선수금구분|선수금(시작일)|선수금(종료일)|본문TEXT|RMS년월|RMS번호|RMS부번호|전표번호|에러코드|에러메세지|GFLAG|AMTGBN|MEDFLAG|RMSNO"
		mobjSCGLSpr.SetColWidth .sprSht_SUSUDTL, "-1",  "   6|       8|        10|    15|  20|       5|         8|      10|  10|    10|       7| 5|       8|    10|     0|   0|       7|   7|      7|     8|        10|            13|            13|      20|      7|      7|        7|       9|       0|        10|    0|     0|      0|    0"
		...
		mobjSCGLSpr.SetCellTypeEdit2 .sprSht_SUSUDTL, "MEDFLAGNAME | CUSTOMERCODE | CUSTNAME | BA | COSTCENTER | BP | GBN | ACCOUNT | DEBTOR | MTAXYEARMON | MTAXNO | VOCHNO | ERRCODE | ERRMSG | GFLAG | AMTGBN", -1, -1, 200
		mobjSCGLSpr.SetCellAlign2 .sprSht_SUSUDTL, "BA | SEMU | BP | MTAXYEARMON | MTAXNO | GBN | VOCHNO | CUSTOMERCODE|DEBTOR | ACCOUNT ",-1,-1,2,2,false '가운데
		...
		mobjSCGLSpr.SetCellAlign2 .sprSht_SUSUDTL, "CUSTOMERCODE | BA | SEMU | BP | DEBTOR | ACCOUNT | MTAXYEARMON | MTAXNO | GBN | VOCHNO",-1,-1,2,2,false '가운데
		mobjSCGLSpr.SetCellsLock2 .sprSht_SUSUDTL,true,"MEDFLAGNAME | CUSTOMERCODE | CUSTNAME | AMT | BP | GBN | MTAXYEARMON | MTAXNO | VOCHNO | ERRCODE | ERRMSG | MEDFLAG"
		mobjSCGLSpr.ColHidden .sprSht_SUSUDTL, "GBN | GFLAG | DEMANDDAY | AMTGBN | MTAXNOSEQ | MEDFLAG | RMSNO", true 
		
		'**************************************************
		'매입 시트 디자인
		'**************************************************	
		gSetSheetColor mobjSCGLSpr, .sprSht_OUT
		mobjSCGLSpr.SpreadLayout .sprSht_OUT, 33, 0, 4
		mobjSCGLSpr.SpreadDataField .sprSht_OUT,    "CHK | POSTINGDATE | CUSTOMERCODE | CUSTNAME | VENDORNAME | SUMM | BA | COSTCENTER | AMT | VAT | SEMU | BP | DEMANDDAY | DUEDATE  | GBN | ACCOUNT | DEBTOR | BMORDER | DOCUMENTDATE | PAYCODE | BANKTYPE | PREPAYMENT | FROMDATE | TODATE | SUMMTEXT | TAXYEARMON | TAXNO | VOCHNO | ERRCODE | ERRMSG | GFLAG | JOBBASE | AMTGBN"
		mobjSCGLSpr.SetHeader .sprSht_OUT,		    "선택|전표일자|거래처코드|거래처|외주처|적요|사업영역|코스트센터|금액|부가세|세무코드|BP|지급기일|지급일|구분|차변계정|계정|BMORDER|증빙일|지급방법|BANKTYPE|선수금구분|선수금(시작일)|선수금(종료일)|본문TEXT|RMS년월|RMS번호|전표번호|에러코드|에러메세지|GFLAG|매출구분| AMTGBN"
		mobjSCGLSpr.SetColWidth .sprSht_OUT, "-1",  "   4|       8|        10|    15|    15|  20|       5|         8|  10|    10|       7| 5|       8|    10|   0|       7|   7|      7|     8|      20|      20|        10|            13|            13|      20|      7|      7|       9|       0|        10|    0|      10|      0"
...
Sub Get_COMBO_VALUE ()		
	...
	With frmThis	
		...
		vntData = mobjPDCMMERGEVOCH.Get_COMBO_VALUE(gstrConfigXml,mlngRowCnt,mlngColCnt,"PD_PAYCODE")
...
Sub Get_SUBCOMBO_VALUE(strCODE, row, sprsht)
	...
	With frmThis   
		...
		strCODE = replace(strCODE,"-","")
       	...
       	vntData = mobjPDCMMERGEVOCH.Get_SUBCOMBO_VALUE(gstrConfigXml, mlngRowCnt, mlngColCnt, strCODE)
...
Sub SelectRtn_SUSU ()
	...
	with frmThis
		...
		if .rdALL.checked then
			strVOCH_TYPE = "ALL"
		elseif .rdAOR.checked then
			strVOCH_TYPE = "AOR"
		end if 
		
		vntData = mobjPDCMMERGEVOCH.SelectRtn_SUSU(gstrConfigXml, mlngRowCnt, mlngColCnt, strYEARMON, _
...
Sub SelectRtn_SUSUDTL (Col, Row)
	...
	with frmThis
		...
		strTAXYEARMON = mobjSCGLSpr.GetTextBinding(.sprSht_SUSU,"MTAXYEARMON",Row)
		strTAXNO	  = mobjSCGLSpr.GetTextBinding(.sprSht_SUSU,"MTAXNO",Row)
		strVOCH_TYPE  = mobjSCGLSpr.GetTextBinding(.sprSht_SUSU,"VOCH_TYPE",Row)
		
		if .rdALL.checked then
			strVOCH_TYPE = ""
			vntData = mobjPDCMMERGEVOCH.SelectRtn_SUSUDTL(gstrConfigXml,mlngRowCnt,mlngColCnt, strTAXYEARMON, strTAXNO, strVOCH_TYPE)
		else
			strVOCH_TYPE = "3"
			vntData = mobjPDCMMERGEVOCH.SelectRtn_SUSUDTL(gstrConfigXml,mlngRowCnt,mlngColCnt, strTAXYEARMON, strTAXNO, strVOCH_TYPE)
		end if
...
Sub SelectRtn_OUT ()
	...
	with frmThis
		...
		vntData = mobjPDCMMERGEVOCH.SelectRtn_OUT(gstrConfigXml, mlngRowCnt, mlngColCnt, strYEARMON, _
	...
	with frmThis
		...
		if mstrPROCESS = "Create" then
			For intCnt = 1 To .sprSht_SUSUDTL.MaxRows
				...
				If strTAXYEARMON = mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"MTAXYEARMON",intCnt) and _
						strTAXNO = mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"MTAXNO",intCnt) Then
				ELSE
					...
					strRMSTAXYEARMON	= mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"MTAXYEARMON",intCnt)
					strRMSTAXNO			= mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"MTAXNO",intCnt)'
					strTYPE				= "1"
				...
				if strIF_CNT = "1" then
					...
					strITEMLIST = strITEMLIST + cstr(strHSEQ) + "|" + _
									...
									mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"CUSTOMERCODE",intCnt) + "|" + _
									mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"MTAXYEARMON",intCnt) + "|" + _
									mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"MTAXNO",intCnt) + "|" + _
				...
				strTAXYEARMON = mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"MTAXYEARMON",intCnt)
				strTAXNO = mobjSCGLSpr.GetTextBinding(.sprSht_SUSUDTL,"MTAXNO",intCnt)
			Next
		...
		if mstrPROCESS = "Create" then
			For intCnt = 1 To .sprSht_OUT.MaxRows
				if mobjSCGLSpr.GetTextBinding(.sprSht_OUT,"chk",intCnt) = "1" then		
					...
					strRMSTAXYEARMON	= mobjSCGLSpr.GetTextBinding(.sprSht_OUT,"MTAXYEARMON",intCnt)
					strRMSTAXNO			= mobjSCGLSpr.GetTextBinding(.sprSht_OUT,"MTAXNO",intCnt)'
					strTYPE				= "4"
...
Sub Set_VochValue (strRETURNLIST)
	...
	With frmThis
		if mstrPROCESS ="Create" then
			if mstrGUBUN = "M" then
				IF .rdALL.checked THEN
					intRtn = mobjPDCMMERGEVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "ALL")
				ELSEIF .rdAOR.checked THEN
					intRtn = mobjPDCMMERGEVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN, "AOR")
				END IF 
				
			elseif mstrGUBUN = "B" then
				intRtn = mobjPDCMMERGEVOCH.ProcessRtn(gstrConfigXml,vntData_ProcesssRtn, strRETURNLIST, mstrGUBUN)
			end if 
   		...
   		elseif mstrPROCESS ="Delete" then
   			IF .rdALL.checked THEN
   				intRtn = mobjPDCMMERGEVOCH.VOCHDELL(gstrConfigXml, strRETURNLIST, mstrGUBUN, "ALL")
   			ELSEIF .rdAOR.checked THEN
   				intRtn = mobjPDCMMERGEVOCH.VOCHDELL(gstrConfigXml, strRETURNLIST, mstrGUBUN, "AOR")
   			end if
...
sub ErrVochDeleteRtn
	...
	with frmThis
		...
		IF mstrGUBUN = "M" THEN
			vntData = mobjSCGLSpr.GetDataRows(.sprSht_SUSU,"CHK | MTAXYEARMON | MTAXNO | ERRCODE")
		ELSEIF mstrGUBUN = "B" THEN
			vntData = mobjSCGLSpr.GetDataRows(.sprSht_OUT,"CHK | TAXYEARMON | TAXNO | ERRCODE")
		END IF
		...
		intRtn = mobjPDCMMERGEVOCH.DeleteRtn(gstrConfigXml,vntData)
		...
		If mstrGUBUN = "M"  then  
			for i = .sprSht_SUSU.MaxRows to 1 step -1
				If mobjSCGLSpr.GetTextBinding(.sprSht_SUSU,"CHK",i) = 1 Then
					...
					strTAXYEARMON = mobjSCGLSpr.GetTextBinding(.sprSht_SUSU,"MTAXYEARMON",i)
					strTAXNO = mobjSCGLSpr.GetTextBinding(.sprSht_SUSU,"MTAXNO",i)
					strVOCHNO = mobjSCGLSpr.GetTextBinding(.sprSht_SUSU,"VOCHNO",i)

					if .rdALL.checked then
						intRtn = mobjPDCMMERGEVOCH.DeleteRtn_GANG(gstrConfigXml,strTAXYEARMON, strTAXNO, strVOCHNO, mstrGUBUN,"ALL")
					elseif .rdAOR.checked then
						intRtn = mobjPDCMMERGEVOCH.DeleteRtn_GANG(gstrConfigXml,strTAXYEARMON, strTAXNO, strVOCHNO, mstrGUBUN,"AOR")
					end if 
		...
		ELSEIf mstrGUBUN = "B"  then
			for i = .sprSht_OUT.MaxRows to 1 step -1
				If mobjSCGLSpr.GetTextBinding(.sprSht_OUT,"CHK",i) = 1 Then
					...
					intRtn = mobjPDCMMERGEVOCH.DeleteRtn_GANG(gstrConfigXml,strTAXYEARMON, strTAXNO, strVOCHNO, mstrGUBUN)
									...
									<TABLE class="SEARCHDATA" id="tblKey" cellSpacing="1" cellPadding="0" width="100%" border="0">
										...
										<TR>
											...
											<TD class="SEARCHDATA" colspan="4">
												...
												<INPUT id="rdF" title="미완료 내역조회" type="radio" CHECKED value="rdF" name="rdGBN" onclick="vbscript:Call Set_delete('imgVochDelco')">&nbsp;미완료&nbsp;
												<INPUT id="rdE" title="오류전표 내역조회" type="radio" value="rdE" name="rdGBN" onclick="vbscript:Call Set_delete('imgVochDelco')">&nbsp;오류&nbsp;
											</TD>
										</TR>
									</TABLE>
								</TD>
								...
								<TD>
									<TABLE class="SEARCHDATA" id="tblKey1" cellSpacing="1" cellPadding="0" width="100%" border="0">
										<TR>
											...
											<TD class="SEARCHDATA" style="WIDTH: 468px"><INPUT class="INPUT_L" id="txtSUMM" title="적요적용" style="WIDTH: 402px; HEIGHT: 21px" size="61"
											...
											<TD class="SEARCHDATA">
												<INPUT id="rdALL" title="통합전표처리" type="radio" CHECKED value="rdALL" name="rdGUBUN">
												통합전표 <INPUT id="rdAOR" title="AOR통합전표처리" type="radio" value="rdAOR" name="rdGUBUN">
												AOR 대행매출
											</TD>
