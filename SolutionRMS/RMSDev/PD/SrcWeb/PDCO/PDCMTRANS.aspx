...
<HTML>
	<HEAD>
		...
		<!--
'****************************************************************************************
'시스템구분 : 위수탁거래명세서 등록 화면(PDCMTRANS.aspx)
'실행  환경 : ASP.NET, VB.NET, COM+ 
'프로그램명 : PDCMTRANS.aspx
'기      능 : 거래명세서 입력/삭제 처리
'파라  메터 : 
'특이  사항 : 
'----------------------------------------------------------------------------------------
'HISTORY    :1) 2009/09/30 By 황덕수
'****************************************************************************************
-->
...
Sub ImgConfirmRequest_onclick()
	...
	with frmThis
		IF .sprSht_HDR.MaxRows = 0 THEN
			gErrorMsgBox "청구요청할 내역이 없습니다.","삭제안내!"
			Exit Sub
		END IF
		...
		IF lngchkCnt = 0 Then
			gErrorMsgBox "선택된 내역이 없습니다.","삭제안내!"
			EXIT SUB
		END IF
...
Sub imgPrint_onclick ()
	...
	if mstrGrid = FALSE THEN
		gErrorMsgBox "미생성 내역은 인쇄할수 없습니다.",""	
		Exit Sub
	END IF
...
Sub InitPage()
    ...
    With frmThis
		...
		mobjSCGLSpr.SetHeader .sprSht_HDR,		  "선택|상태|승인|계산서|광고주코드|광고주|공급가액|부가세액|합계금액|청구일|발행일|거래년월|번호|승인자|비고|상세행수"
...
Sub Grid_Setting ()
	With frmThis
		...
		If mstrGrid Then	
			...
			mobjSCGLSpr.SetHeader .sprSht_DTL,		"거래명세년월|거래명세번호|순서|순번|광고주|팀|브랜드|JOB명|JOBNO|SEQ|청구금액|정산액|차액|부가세|비고|계산서년월|계산서번호|텍스코드|세금계산서구분" 
		...
		Else
			...
			mobjSCGLSpr.SetHeader .sprSht_DTL,		   "선택|견적번호|광고주코드|광고주명|팀코드|팀명|브랜드코드|브랜드명|JOB명|JOBNO|SEQ|청구금액|정산액|차액|부가세|청구기준|비고|TRANSTIMRANK|TRANSCUSTRANK|ENDFLAGNAME|INCJOBNOSEQ|INCJOBNO|담당부서|텍스코드|세금계산서구분|청구기준코드"
...
Sub COMBO_TYPE()
    ...
    With frmThis   
		...
		vntJOBGUBN = mobjPDCMTRANS.GetDataType(gstrConfigXml, mlngRowCnt, mlngColCnt,"JOBGUBN")  '제작구분
...
Sub ProcessRtn_CUST ()
	...
	with frmThis
		...
		if .txtDEMANDDAY.value = "" then
			gErrorMsgBox "청구일자는 필수 입니다..","저장안내!"
			Exit Sub
		end if
		...
		For intCnt2 = 1 To .sprSht_DTL.MaxRows
			IF mobjSCGLSpr.GetTextBinding(.sprSht_DTL,"CHK",intCnt2) = 1 THEN
				...
				IF mobjSCGLSpr.GetTextBinding(.sprSht_DTL,"ADJAMT",intCnt2) = ""  Or  mobjSCGLSpr.GetTextBinding(.sprSht_DTL,"ADJAMT",intCnt2) = 0 THEN
					gErrorMsgBox "선택한 거래명세서의 정산금액은 반드시 입력 하여야 합니다.","저장안내!"
					Exit Sub
				End If
			END IF
		next
								...
								<TR>
									<TD align="left" width="400" height="20">
										<table cellSpacing="0" cellPadding="0" width="100%" border="0">
											...
											<tr>
												<td class="TITLE" valign="top">제작 거래명세서</td>
											</tr>
										</table>
									</TD>
										...
										<TABLE class="SEARCHDATA" id="tblKey" cellSpacing="1" cellPadding="0" width="100%" align="left"
											...
											<TR>
												<TD class="SEARCHLABEL" style="CURSOR: hand" onclick="vbscript:Call gCleanField(txtYEARMON1,'')"
													width="60">청구년월</TD>
												<TD class="SEARCHDATA" width="100"><INPUT class="INPUT" id="txtYEARMON1" title="년월조회" style="WIDTH: 98px; HEIGHT: 22px" accessKey="NUM"
												...
												<TD class="SEARCHLABEL" style="CURSOR: hand" onclick="vbscript:Call gCleanField(txtTIMNAME1,txtTIMCODE1)"
													width="60">팀</TD>
												<TD class="SEARCHDATA" width="250"><INPUT class="INPUT_L" id="txtTIMNAME1" title="팀명" style="WIDTH: 173px; HEIGHT: 22px" type="text"
												...
												<TD class="SEARCHLABEL" style="CURSOR: hand" onclick="vbscript:Call gCleanField(cmbJOBGUBN1,'')"
													width="60">제작구분</TD>
												<TD class="SEARCHDATA"><SELECT dataFld="cmbJOBGUBN1" id="cmbJOBGUBN1" title="제작구분" style="WIDTH: 98px" name="cmbJOBGUBN1"></SELECT></TD>
									...
									<TD class="KEYFRAME" style="WIDTH: 100%" vAlign="top" align="center">
										<TABLE height="28" cellSpacing="0" cellPadding="0" width="100%" background="../../../images/TitleBG.gIF"
											...
											<TR>
												...
												<TD class="TITLE" vAlign="absmiddle" align="left" width="650" height="22">작성선택 :
													<SELECT id="cmbGUBUN" title="그룹구분" style="WIDTH: 90px" name="cmbGUBUN">
														<OPTION value="taxgroup" selected>합산작성</OPTION>
														<OPTION value="nongroup">분할작성</OPTION>
													</SELECT>
													청구일자 : <INPUT dataFld="DEMANDDAY" class="INPUT" id="txtDEMANDDAY" title="브랜드명" style="WIDTH: 100px; HEIGHT: 22px"
													...
													<TABLE id="tblButtonDTR" style="HEIGHT: 20px" cellSpacing="0" cellPadding="2" border="0">
														<TR>
															<TD><IMG id="ImgTimSave" onmouseover="JavaScript:this.src='../../../images/ImgTimSaveOn.gif'"
																	...
																	alt="팀별로 거래명세서를 생성합니다." src="../../../images/ImgTimSave.gif" border="0" name="ImgTimSave"></TD>
