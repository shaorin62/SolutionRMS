<%@ Page Language="vb" AutoEventWireup="false" Codebehind="PDCMDEMANDCONFIRM.aspx.vb" Inherits="PD.PDCMDEMANDCONFIRM" %>
...
Sub InitPageData
	...
	with frmThis
		.sprSht.maxrows = 0
		.txtYEARMON.value  = MID(gNowDate2,1,4) & MID(gNowDate2,6,2) '추후 이것으로 대처 임시로 테스트값 연결 하였음
...
Sub Data_Confirm(byVal strConfirmFlag)
	...
	with frmThis
			...
			For intCnt4= 1 To .sprSht.MaxRows
				If mobjSCGLSpr.GetTextBinding(.sprSht,"CHK",intCnt4) = "1" Then 
					If mobjSCGLSpr.GetTextBinding(.sprSht,"DELCHK",intCnt4) = "Y" Then
						...
						Exit Sub
					End If
					If mobjSCGLSpr.GetTextBinding(.sprSht,"DELCHK",intCnt4) = "N" Then
						strJOBNO = mobjSCGLSpr.GetTextBinding(.sprSht,"JOBNO",intCnt4)
						strSEQ = mobjSCGLSpr.GetTextBinding(.sprSht,"SEQ",intCnt4)
						if NOT TRANSNO_CHECKED (strJOBNO, strSEQ) then
							gErrorMsgBox "JOBNO [" & strJOBNO & "-" & strSEQ & "] 데이터 는 거래명세서가 존재합니다." & vbcrlf & "발행된 거래명세서를 먼저 삭제해주세요.","승인취소안내!"
							Exit Sub
						END IF
					End If
				End If
		...
		select case strConfirmFlag
			case "2": strMSG = "승인취소" : strM = "승인취소"
			case "3": strMSG = "승인 하시겠습니까?" : strM = "승인"
			case "0": strMSG = "승인취소시 반려 됩니다." & vbcrlf & "승인취소 하시겠습니까?"  : strM = "승인취소"
		end select
		
		intSaveRtn = gYesNoMsgbox("해당데이터를 " & strMSG,"청구요청 확인")
...
Function TRANSNO_CHECKED (ByRef strJOBNO, ByRef strSEQ)
	Dim vntData
	Dim intCnt
	Dim strCOUNT
	'on error resume next
	...
	'초기화
	TRANSNO_CHECKED = false
	mlngRowCnt=clng(0): mlngColCnt=clng(0)
	
	vntData = mobjPDCODEMAND.TRANSNO_CHECKED(gstrConfigXml,mlngRowCnt,mlngColCnt, strJOBNO, strSEQ)
	
	IF mlngRowCnt >0 THEN
		TRANSNO_CHECKED = false
	ELSE
		TRANSNO_CHECKED = TRUE	
	End IF
End Function
								...
								<TD style="WIDTH: 100%" vAlign="middle">
									<TABLE class="SEARCHDATA" id="tblKey" cellSpacing="1" cellPadding="0" width="100%" border="0">
										<TR>
											...
											<TD class="SEARCHLABEL" title="세금계산서조회 조건을 삭제합니다" style="CURSOR: hand" width="80">작업선택</TD>
											<TD class="SEARCHDATA">&nbsp;<INPUT id="rdT" title="요청내역조회" type="radio" CHECKED value="rdT" name="rdGBN">&nbsp;요청내역 
												조회 <INPUT id="rdF" title="승인내역조회" type="radio" value="rdF" name="rdGBN">&nbsp;승인내역조회&nbsp;</TD>
											<td align="right" width="250"><IMG id="imgQuery" onmouseover="JavaScript:this.src='../../../images/imgQueryOn.gIF'"
				...
				<TR>
					<TD style="WIDTH: 100%; HEIGHT: 100%" vAlign="top" align="left">
						<OBJECT id="sprSht" style="WIDTH: 100%; HEIGHT: 100%" classid="clsid:41F841C1-AE16-11D5-8817-0050DA6EF5E5" VIEWASTEXT>
							...
							<PARAM NAME="_ExtentY" VALUE="7779">
