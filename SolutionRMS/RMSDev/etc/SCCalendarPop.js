...
var img_close;
img_del=new Image();
img_del.src="../../../Images/CalDelSmall.gif";
img_close=new Image();
img_close.src="../../../Images/CalCloseSmall.gif";

minYearList=todayDate.getFullYear()-70;
...
function gshowCalendar(frmName, dteBox,btnImg, runFuncs,hideDrops, MnDt, MnMo, MnYr, MxDt, MxMo, MxYr) {
    ...
    if (IsCalendarVisible) {
        ...
        curImg = btnImg;
        curDateBox = dteBox;
        if ( ppcIE ) { //By Kimks for Popup
            ppcX = 0; //getOffsetLeft(document.images[btnImg]);    
            ppcY = 0; //getOffsetTop(document.images[btnImg]) + document.images[btnImg].height;
        }
        else if (ppcNN){
...
function Calendar(whatMonth,whatYear) {
    ...
    //By KimKS for Popup 
    output += '<td class="cal-HeadCell" align="center" width="100%">&nbsp;&nbsp;<a href="#" onclick="javascript:scrollMonth(-1);" class="cal-DayLink">&lt;</a>&nbsp;<SELECT class="cal-TextBox" NAME="cboMonth" onChange="changeMonth();">';
    ...
    //By KimKS for Popup 
    output += '<\/SELECT>&nbsp;<a href="#" onclick="javascript:scrollMonth(1);" class="cal-DayLink">&gt;</a>&nbsp;&nbsp;<\/td><\/tr><tr><td width="100%" align="center">';
    ...
    if (!(IsUsingMinMax)) {
        if (isGreyDate) {
            ...
            if (isDayToday(linkDay)) {
                templink='<td align="center" class="cal-DayCell">' + '<a class="cal-TodayLink" onmouseover="self.status=\' \';return true" href="#" onclick="javascript:changeDay(' + linkDay + ');">' + linkDay + '<\/a>' +'<\/td>';
            }
            else {
                templink='<td align="center" class="cal-DayCell">' + '<a class="cal-DayLink" onmouseover="self.status=\' \';return true" href="#" onclick="javascript:changeDay(' + linkDay + ');">' + linkDay + '<\/a>' +'<\/td>';
            }
        }
    }
    else {
...
function changeYear() {
    if (ppcIE) {
        ...
        domlay('PopupCalendar',1,ppcX,fppcY,Calendar(curDate.getMonth(),curDate.getFullYear()));
    }
