...
<table width="940" border="0" align="left" cellpadding="0" cellspacing="0" >
  ...
  <tr onmouseover="this.style.backgroundColor='<%=trgbcolor%>'" onmouseout="this.style.backgroundColor='#FFFFFF'"> 
    ...
    <td   height="25" align="left" bgcolor="<%=trgbcolor%>">&nbsp;<a  href="Gview.asp?page=<%=page%>&num=<%=rs("board_num")%>&b_num=<%=rs("b_num")%>&part=<%=part%>&tail_part=<%=tail_part%>" title="<%=rs("title")%>">&nbsp;
    <%
    Dim strTitle
    Dim intCnt
    intCnt = len(rs("title"))
    ...
    %></a> 
      <%  '등록일자를 24시간을 비교해서 작으면 뉴이미지를 찍는다(최신글 리스트)  
		                   if datediff("h",rs("writeday"),now()) < 168 then %>
 ...
 <!-- <tr> 
  ...
  <form action="Glist.asp" name="board_list" method="post" ID="Form1">
	...
	<table width="580" border="0" cellpadding="0" cellspacing="0" ID="Table1">
          <tr > 
            ...
            <td  ><a href="#"><input type="image" src="Image/search_icon.gif"  border="0" align=absmiddle ID="Image1" NAME="Image1"></a></td>
          </tr>
        </table>
	</td></form>
