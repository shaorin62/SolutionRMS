	...
	with frmThis
		intColSum = 0
  		If .sprSht2.MaxRows = 0 Then
  			gErrorMsgBox "오른쪽의 송신데이터가 없습니다.","처리안내"
  			Exit Sub
  		End If
		...
		if  not IsArray(vntData)  then 
			gErrorMsgBox "변경된 " & meNO_DATA,"처리안내"
			exit sub
		End If
		...
		if not gDoErrorRtn ("ProcessRtn") then
			'msgbox "Popup창의 받아온 번호" & dblID
			
			window.returnvalue = dblID
			call Window_OnUnload()	
		Else
			gErrorMsgBox "SMS 발송명단 저장중 오류가 발생 하였습니다." & vbcrlf & "관리자에게 문의 하십시오.","처리안내"				
			Exit Sub
		End If
	...
	with frmThis
		...
		for i = 0 to intNo
			select case i
				...
				case 4 : .txtMSG.value = vntInParam(i)				'보낼메세지
				case 5 : mstrFromUserName = vntInParam(i)				'보낼메세지
				case 6 : mstrFromUserEmail = vntInParam(i)				'보낼메세지
				case 7 : mstrFromUserPhone = vntInParam(i)				'보낼메세지
        ...
        With frmThis
			...
			mobjSCGLSpr.SetHeader .sprSht, "선택|사원코드|사원명|부서코드|부서명|전화번호"
			...
			mobjSCGLSpr.SetHeader .sprSht2, "선택|사원코드|사원명|부서코드|부서명|전화번호"
...
Sub imgRight_onclick
	...
	with frmThis 
		...
		If intColSum = 0 Then 
			gErrorMsgBox "왼쪽의 선택된 데이터가 없습니다.","이동안내"
			exit Sub
		End If
...
Sub imgLeft_onclick
	...
	with frmThis
		...
		If intColSum = 0 Then 
			gErrorMsgBox "오른쪽의 선택된 데이터가 없습니다.","이동안내"
			exit Sub
		End If
