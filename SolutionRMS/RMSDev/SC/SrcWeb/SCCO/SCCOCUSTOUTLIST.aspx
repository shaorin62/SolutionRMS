<%@ Page Language="vb" AutoEventWireup="false" Codebehind="SCCOCUSTOUTLIST.aspx.vb" Inherits="SC.SCCOCUSTOUTLIST" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<HEAD>
		<title>거래선 등록(외주처)</title>
		<META http-equiv="Content-Type" content="text/html; charset=ks_c_5601-1987">
		<!--
'****************************************************************************************
'시스템구분 : 거래처관리 (외주처) 
'실행  환경 : ASP.NET, VB.NET, COM+ 
'프로그램명 : SCCOCUSTOUTLIST.aspx
'특이  사항 : 
'----------------------------------------------------------------------------------------
...
Function Busino_Check ()
	...
	With frmThis
		...
		vntData = mobjSCCOCUSTLIST.Busino_Check(gstrConfigXml,mlngRowCnt,mlngColCnt, _
												trim(Replace(mobjSCGLSpr.GetTextBinding( .sprSht_CUST,"BUSINO",.sprSht_CUST.ActiveRow),"-","")), "M")
...
Sub sprSht_CUST_Keydown(KeyCode, Shift)
	...
	If KeyCode = meINS_ROW Then
		...
		mobjSCGLSpr.SetCellsLock2 frmThis.sprSht_CUST,false,frmThis.sprSht_CUST.ActiveRow,2,2,true
...
'----------------------------------------------------------------------
    ...
    With frmThis
		...
		mobjSCGLSpr.SetHeader .sprSht_CUST,		  "선택|사업자번호|상호명|거래처명|코드|대표자|사용|계열|업태|업종|우편번호|주소1|주소2|전화번호|팩스|비고|입력자"
...
Sub SelectRtn ()
	...
	With frmThis
		...
		vntData = mobjSCCOCUSTLIST.SelectRtn_OUTHDR(gstrConfigXml,mlngRowCnt,mlngColCnt, strCLIENTNAME, strCOMPANYNAME, strBUSINO)

		If not gDoErrorRtn ("SelectRtn_OUTHDR") Then
...
Sub ProcessRtn_EXEHDR ()
	...
	With frmThis
		...
		intRtn = mobjSCCOCUSTLIST.ProcessRtn_EXEHDR(gstrConfigXml,vntData, "M")
	...
	With frmThis
		For i = 1 to .sprSht_CUST.MaxRows
			if mobjSCGLSpr.GetTextBinding(.sprSht_CUST,"CHK",i) = 1 Then
				...
				Else
					vntData = mobjSCCOCUSTLIST.SelectRtn_PDCountCheck(gstrConfigXml,mlngRowCnt,mlngColCnt, strHIGHCUSTCODE) 
					If mlngRowCnt > 0 Then
						strMSG = ""
						strMSG = strMSG & " 제작외주에서: " & vntData(0,0) & "건" 
						gErrorMsgBox i & "행의 코드는 " & strMSG & " 이 저장되어있습니다.","삭제안내!"
						Exit Sub
					End If
				End If
										...
										<TABLE class="SEARCHDATA" id="tblKey" cellSpacing="1" cellPadding="0" width="100%" align="left"
											...
											<TR>
												...
												<TD class="SEARCHLABEL" style="CURSOR: hand" onclick="vbscript:Call gCleanField(txtBUSINO,'')"
													width="100">사업자번호</TD>
												<TD class="SEARCHDATA" style="WIDTH: 371px"><INPUT class="INPUT_L" id="txtBUSINO" title="코드조회" style="WIDTH: 168px; HEIGHT: 22px" type="text"
