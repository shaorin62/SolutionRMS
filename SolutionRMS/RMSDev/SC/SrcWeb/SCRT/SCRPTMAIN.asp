...
Dim strtemp              
   ...
   strtemp =   reportname & " 이 없습니다." 
    If Not objFileSys.FileExists(path & reportname ) Then
		...
		Response.Redirect "SCRPTERRMSG.asp?MSG=" & Replace("리포트의 파일명[" & ReportName & "]이 잘못되었습니다.", " ", "%20")
	Else
		Set objFileSys = Nothing
	End If
...
Dim  strDB, strUID, strPWD     
If DSN = "" then
	strDB  =  "HPCDEV"       : strUID =  "SYSDBA"      : strPWD =  "SYSDBA"
Else	
	vntDBParam = Split(DSN, ";") '--->>DB Connection 정보(구분자 체크)  
	...
	<HEAD>
		...
		<!--
			...
			Sub EndPage()
				...
				'Window 창의 이름이 같은 경우 에러발생합니다.
				'절대 사용하지 말것(IIS5.0 과 IIS6.0의 차이라고 생각됨)
				window.open "Cleanup.asp","","left=10000"  'SESSION CLEAN
			End Sub
