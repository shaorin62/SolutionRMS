...
Public Class ccMDCTTOTALVOCH
    Inherits ccControl
#Region "GROUP BLOCK : 전역 또는 모듈레벨의 변수/상수 선언"
    Private CLASS_NAME = "ccMDCTTOTALVOCH"                  '자신의 클래스명
...
#Region "GROUP BLOCK : 외부에비공개"
    ...
    Public Function SelectRtn_AMT(ByVal strInfoXML As String, _
        ...
        With mobjSCGLConfig
            ...
            strFormat = strFormat & " case isnull(b.summ,'') when '' then convert(char(11),DBO.MD_SUMMREALMED_FUN(A.TAXYEARMON,A.TAXNO))+'종합편성광고비' Else b.summ end as summ,"
            ...
            strFormat = strFormat & " CASE ISNULL(B.BMORDER,'') WHEN '' THEN 'AD0130' ELSE B.BMORDER END AS  BMORDER,"
            ...
            strFormat = strFormat & " case isnull(b.summtext,'') when '' then convert(char(11),DBO.MD_SUMMREALMED_FUN(A.TAXYEARMON,A.TAXNO))+'종합편성광고비' Else b.summtext end as summtext,"
            ...
            strFormat = strFormat & " WHERE A.MEDFLAG IN ('T') "
    ...
    Public Function SelectRtn_GEN(ByVal strInfoXML As String, _
        ...
        With mobjSCGLConfig
            ...
            If strSALEGBN = "Y" Then
                ...
                strFormat = strFormat & " replace(A.clientbisno,'-','') customercode,"
                strFormat = strFormat & " CLIENTNAME CUSTNAME,"
                strFormat = strFormat & " case isnull(b.summ,'') when '' then  convert(char(11),RTRIM(LTRIM(CLIENTNAME)))+'종합편성일반청구' else b.summ end as summ,"
                strFormat = strFormat & " case when isnull(A.AMT,0) < 0 then abs(isnull(A.AMT,0)) else  dbo.lpad(A.AMT,13,'0') end  amt, "
                strFormat = strFormat & " case isnull(b.vat,0) when 0 then case when isnull(A.vat,0) < 0 then abs(isnull(A.vat,0)) else dbo.lpad(A.vat,13,'0') end else isnull(b.vat,0) end vat, "
                strFormat = strFormat & " CASE isnull(b.semu,'') when '' then 'B0' else b.semu end as SEMU,"
                strFormat = strFormat & " CASE isnull(b.BP,'') when '' then '8000' else b.BP end as BP,"
                ...
                strFormat = strFormat & " CASE isnull(b.prepayment,'') when '' then '' else b.fromdate end as  fromdate,"
                strFormat = strFormat & " CASE isnull(b.prepayment,'') when '' then '' else b.todate end as  todate,"
                strFormat = strFormat & " case isnull(b.summtext,'') when '' then  convert(char(11),RTRIM(LTRIM(CLIENTNAME)))+'종합편성일반청구' else b.summ end as summtext,"
                strFormat = strFormat & " case when isnull(A.AMT,0) < 0 then '-' else '+' end AMTGBN,"
                strFormat = strFormat & " A.TAXYEARMON,"
                strFormat = strFormat & " A.TAXNO,"
                strFormat = strFormat & " CASE A.VOCHNO WHEN '' THEN NULL ELSE A.VOCHNO END VOCHNO,"
                ...
                strFormat = strFormat & " B.ERRCODE,"
                strFormat = strFormat & " B.ERRMSG,"
                strFormat = strFormat & " 'TA'MEDFLAG "
                strFormat = strFormat & " FROM MD_TRUTAX_HDR  A "
                strFormat = strFormat & " LEFT JOIN MD_TRUVOCH_MST B "
                strFormat = strFormat & " ON A.TAXYEARMON = B.TAXYEARMON AND A.TAXNO = B.TAXNO and b.GBN = 'D' AND B.MEDFLAG = 'TA' "
                strFormat = strFormat & " WHERE A.MEDFLAG IN ('T') "
                strFormat = strFormat & " and isnull(A.attr10,0) <> 999999"
            ...
            ElseIf strSALEGBN = "C" Then
                ...
                strFormat = strFormat & " B.ERRCODE,"
                strFormat = strFormat & " B.ERRMSG,"
                strFormat = strFormat & " 'TAA'MEDFLAG "
                strFormat = strFormat & " FROM MD_TRUTAX_HDR  A "
                strFormat = strFormat & " LEFT JOIN MD_TRUVOCH_MST B "
                strFormat = strFormat & " ON A.TAXYEARMON = B.TAXYEARMON AND A.TAXNO = B.TAXNO and b.GBN = 'D' AND B.MEDFLAG = 'TAA' "
                strFormat = strFormat & " WHERE A.MEDFLAG IN ('T') "
                strFormat = strFormat & " and isnull(A.attr10,0) <> 999999"
                strFormat = strFormat & " AND ISNULL(A.VOCH_TYPE,'') = '2'"
            ...
            Else
                ...
                strFormat = strFormat & " 'TD'MEDFLAG "
                strFormat = strFormat & " FROM MD_TRUTAXGENERAL_HDR  A "
                strFormat = strFormat & " LEFT JOIN MD_TRUVOCH_MST B "
                strFormat = strFormat & " ON A.TAXYEARMON = B.TAXYEARMON AND A.TAXNO = B.TAXNO and b.GBN = 'O' AND B.MEDFLAG = 'TD' "
                strFormat = strFormat & " WHERE A.MEDFLAG IN ('T') "
                strFormat = strFormat & " and isnull(A.attr10,0) <> 999999"
                strFormat = strFormat & " AND ISNULL(A.VOCH_TYPE,'') = '2'"
                strFormat = strFormat & " {0} "
    ...
    Public Function SelectRtn_GENDTL(ByVal strInfoXML As String, _
        ...
        With mobjSCGLConfig
            ...
            strFormat = strFormat & " B.ERRMSG, "
            strFormat = strFormat & " '' PAYCODE,"
            strFormat = strFormat & " 'TB' MEDFLAG "
            strFormat = strFormat & " FROM MD_TRUTAXGENERAL_HDR  A "
            strFormat = strFormat & " LEFT JOIN MD_TRUVOCH_MST B "
            strFormat = strFormat & " ON A.TAXYEARMON = B.TAXYEARMON AND A.TAXNO = B.TAXNO and b.GBN = 'D' AND B.MEDFLAG = 'TB'  "
            strFormat = strFormat & " WHERE A.MEDFLAG IN ('T')  and isnull(a.amt,0) <> 0"
            strFormat = strFormat & " and isnull(A.attr10,0) <> 999999"
            strFormat = strFormat & " {0} "
            strFormat = strFormat & " UNION ALL"
            strFormat = strFormat & " SELECT "
            ...
            strFormat = strFormat & " B.ERRMSG,"
            strFormat = strFormat & " '' PAYCODE,"
            strFormat = strFormat & " 'TC' MEDFLAG "
            strFormat = strFormat & " FROM MD_TRUTAXOUTLIST_HDR  A "
            strFormat = strFormat & " LEFT JOIN MD_TRUVOCH_MST B "
            strFormat = strFormat & " ON A.TAXYEARMON = B.TAXYEARMON AND A.TAXNO = B.TAXNO and b.GBN = 'D' AND B.MEDFLAG = 'TC' "
            strFormat = strFormat & " WHERE A.MEDFLAG IN ('T') and isnull(a.amt,0) <> 0 "
            strFormat = strFormat & " and isnull(A.attr10,0) <> 999999"
            strFormat = strFormat & " AND ISNULL(A.VOCH_TYPE,'') = '2'"
            strFormat = strFormat & " {0}"
    ...
    Public Function SelectRtn_GENMCBUYDTL(ByVal strInfoXML As String, _
        ...
        With mobjSCGLConfig
            ...
            strFormat = strFormat & " B.ERRMSG, "
            strFormat = strFormat & " '' PAYCODE,"
            strFormat = strFormat & " 'TBB' MEDFLAG "
            strFormat = strFormat & " FROM MD_TRUTAX_HDR  A "
            strFormat = strFormat & " LEFT JOIN MD_TRUVOCH_MST B "
            strFormat = strFormat & " ON A.TAXYEARMON = B.TAXYEARMON AND A.TAXNO = B.TAXNO and b.GBN = 'D' AND B.MEDFLAG = 'TBB'  "
            strFormat = strFormat & " WHERE A.MEDFLAG IN ('T') and isnull(a.amt,0) <> 0"
            strFormat = strFormat & " and isnull(A.attr10,0) <> 999999"
            strFormat = strFormat & " {0} "
    ...
    Public Function SelectRtn_SUSUDTL(ByVal strInfoXML As String, _
        ...
        strFormat = strFormat & " WHERE 1=1 "
        strFormat = strFormat & " AND A.MEDFLAG IN ('T') "
        strFormat = strFormat & " and isnull(A.attr10,0) <> 999999"
        strFormat = strFormat & " and isnull(A.MERGEFLAG,'') <> '1' "
    ...
    Public Function SelectRtn_OUT(ByVal strInfoXML As String, _
        ...
        With mobjSCGLConfig
            ...
            strFormat = strFormat & " FROM MD_TOTAL_MEDIUM A LEFT JOIN MD_TRUVOCH_MST B   "
            strFormat = strFormat & " ON A.YEARMON = B.TAXYEARMON AND A.SEQ = B.TAXNO And B.GBN = 'O' and b.medflag = 'T2' "
