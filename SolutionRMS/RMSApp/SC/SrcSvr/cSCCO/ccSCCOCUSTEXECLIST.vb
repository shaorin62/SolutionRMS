'****************************************************************************************
'Generated By: MakeSFAR V.2.0.0 - 컨트롤 클래스 메이커 - 한화 S&C
'시스템구분    : 솔루션명 /시스템명/Server Control Class
'실행   환경    : COM+ Service Server Package
'프로그램명    : ccMDCMCUST_TRAN.vb
'기         능    : - 기능을 명시 합니다.
'특이  사항     : - 특이사항에 대해 표현
'                     -
'----------------------------------------------------------------------------------------
'HISTORY    :1) 2004-03-30 오전 10:32:13 By MakeSFARV.2.0.0
'                  2) 2004-03-30 오전 10:32:13 By 작성자명을 씁니다.
'****************************************************************************************

Imports System.Xml                        ' XML처리
Imports SCGLControl                       ' ControlClass의 Base Class 
Imports SCGLUtil.cbSCGLConfig       ' ConfigurationClass
Imports SCGLUtil.cbSCGLErr            '오류처리 클래스
Imports SCGLUtil.cbSCGLXml           'XML처리 클래스
Imports SCGLUtil.cbSCGLUtil            '기타유틸리티 클래스
Imports eSCCO '엔터티 추가

' 엔티티 클래스 사용시 해당 엔티티 클래스의 프로젝트를 참조한 후 Imports 하십시요. 
' Imports 엔티티프로젝트

Public Class ccSCCOCUSTEXECLIST
    Inherits ccControl

#Region "GROUP BLOCK : 전역 또는 모듈레벨의 변수/상수 선언"
    Private CLASS_NAME = "ccSCCOCUSTEXECLIST"                  '자신의 클래스명
    Private mobjceSC_CUST_DTL As eSCCO.ceSC_CUST_DTL            '사용할 Entity 변수 선언
    Private mobjceSC_CUST_HDR As eSCCO.ceSC_CUST_HDR
    'Private Const .DBConnStr = "Provider=SQLOLEDB;Data Source=10.110.10.86;Initial Catalog=MCDEV;DSN=MCDEV;UID=devadmin;Pwd=password"  '커넥션Setting
#End Region

#Region "GROUP BLOCK : Property 선언"
#End Region

#Region "GROUP BLOCK : Event 선언"
#End Region

#Region "GROUP BLOCK : 외부에 공개 Method"


    ' =============== SelectRtnSample Code selectrtn_hdr
    Public Function SelectRtn(ByVal strInfoXML As String, _
                              ByRef intRowCnt As Integer, _
                              ByRef intColCnt As Integer, _
                              ByVal strFlag As String, _
                              ByVal strCUSTNAME As String, _
                              ByVal strBUSINO As String) As Object     'XML  데이터 조회시

        Dim strSQL As String
        Dim strFormet, strSelFields, strWhere As String
        Dim strChkDate As String = ""
        Dim Con1, Con2, Con3, Con4, Con5 As String
        Dim vntData As Object

        With mobjSCGLConfig
            Try
                SetConfig(strInfoXML)   '기본정보 설정

                Con1 = ""
                Con2 = ""
                Con3 = ""
                Con4 = ""

                If strFlag <> "" Then Con1 = String.Format(" AND (MEDFLAG = '{0}')", strFlag)
                'If strCUSTCODE <> "" Then Con2 = String.Format(" AND (MEDDIV = '{0}')", strCUSTCODE)
                If strCUSTNAME <> "" Then Con3 = String.Format(" AND (CUSTNAME LIKE '%{0}%')", strCUSTNAME)
                If strBUSINO <> "" Then Con4 = String.Format(" AND (BUSINO LIKE '%{0}%')", strBUSINO)

                strWhere = BuildFields(" ", Con1, Con2, Con3, Con4)
                strSelFields = "0 CHK, BUSINO ,COMPANYNAME,CUSTNAME,HIGHCUSTCODE AS CUSTCODE,HIGHCUSTCODE , CUSTOWNER ,USE_FLAG, "
                'strSelFields = strSelFields & " CASE USE_FLAG WHEN '1' THEN '사용' ELSE '비사용' END AS USE_FLAG, "
                strSelFields = strSelFields & " CASE CUSTTYPE WHEN '2' THEN '계열' ELSE '비계열' END AS CUSTTYPE, "
                strSelFields = strSelFields & " BUSISTAT,BUSITYPE,ADDRESS1,ADDRESS2,"
                strSelFields = strSelFields & " ACCUSTCODE,GREATCODE,MEDFLAG,REGNOTYPE,JURENTNO,BANKCODE,BANKACCNO, "
                strSelFields = strSelFields & " DEPNAME,ZIPCODE,TEL,FAX,OLDCUSTCODE,MEMO,DEMANDFLAG,ATTR01,ATTR02,ATTR03, "
                strSelFields = strSelFields & " ATTR03,ATTR04,ATTR05,ATTR06,ATTR07,ATTR08,ATTR09,ATTR10 "

                strFormet = "select {0} from SC_CUST_HDR where 1=1  {1} "

                strSQL = String.Format(strFormet, strSelFields, strWhere)

                .mobjSCGLSql.SQLConnect(.DBConnStr)
                vntData = .mobjSCGLSql.SQLSelectArr(strSQL, intRowCnt, intColCnt, , True)

                Return vntData
            Catch err As Exception
                Throw RaiseSysErr(err, CLASS_NAME & ".SelectRtn")
            Finally
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function

    Public Function SelectRtn_ELECTRIC(ByVal strInfoXML As String, _
                                 ByRef intRowCnt As Integer, _
                                 ByRef intColCnt As Integer, _
                                 ByVal strEXClIENTCODE As String) As Object     'XML  데이터 조회시

        Dim strSQL As String
        Dim strFormet, strSelFields, strWhere As String
        Dim Con1 As String
        Dim vntData As Object

        With mobjSCGLConfig
            Try
                SetConfig(strInfoXML)   '기본정보 설정

                Con1 = ""


                If strEXClIENTCODE <> "" Then Con1 = String.Format(" AND (EXClIENTCODE = '{0}')", strEXClIENTCODE)
                'If strMEDDIV <> "" Then Con2 = String.Format(" AND (MEDDIV = '{0}')", strMEDDIV)
                'If strCUSTNAME <> "" Then Con3 = String.Format(" AND (CUSTNAME LIKE '%{0}%')", strCUSTNAME)
                'If strBUSINO <> "" Then Con4 = String.Format(" AND (BUSINO LIKE '%{0}%')", strBUSINO)


                strWhere = BuildFields(" ", Con1)

                strSelFields = " EXClIENTCODE  "

                strFormet = "select {0} from MD_ELECTRIC_MEDIUM where 1=1  {1} "

                strSQL = String.Format(strFormet, strSelFields, strWhere)

                .mobjSCGLSql.SQLConnect(.DBConnStr)
                vntData = .mobjSCGLSql.SQLSelectArr(strSQL, intRowCnt, intColCnt)
                Return vntData
            Catch err As Exception
                Throw RaiseSysErr(err, CLASS_NAME & ".SelectRtn")
            Finally
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function


    Public Function SelectRtn_EXClIENTCODE(ByVal strInfoXML As String, _
                                           ByRef intRowCnt As Integer, _
                                           ByRef intColCnt As Integer, _
                                           ByVal strEXClIENTCODE As String, _
                                           ByVal strTABLE As String) As Object     'XML  데이터 조회시

        Dim strSQL As String
        Dim strFormet, strSelFields, strWhere As String
        Dim Con1 As String
        Dim vntData As Object

        With mobjSCGLConfig
            Try
                SetConfig(strInfoXML)   '기본정보 설정

                Con1 = ""


                If strEXClIENTCODE <> "" Then Con1 = String.Format(" AND (EXClIENTCODE = '{0}')", strEXClIENTCODE)
                'If strMEDDIV <> "" Then Con2 = String.Format(" AND (MEDDIV = '{0}')", strMEDDIV)
                'If strCUSTNAME <> "" Then Con3 = String.Format(" AND (CUSTNAME LIKE '%{0}%')", strCUSTNAME)
                'If strBUSINO <> "" Then Con4 = String.Format(" AND (BUSINO LIKE '%{0}%')", strBUSINO)


                strWhere = BuildFields(" ", Con1)

                strSelFields = " EXClIENTCODE  "

                strFormet = "select {0} from {1} where 1=1  {2} "

                strSQL = String.Format(strFormet, strSelFields, strTABLE, strWhere)

                .mobjSCGLSql.SQLConnect(.DBConnStr)
                vntData = .mobjSCGLSql.SQLSelectArr(strSQL, intRowCnt, intColCnt)
                Return vntData
            Catch err As Exception
                Throw RaiseSysErr(err, CLASS_NAME & ".SelectRtn")
            Finally
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function





    Public Function SelectRtn_DTL(ByVal strInfoXML As String, _
                                 ByRef intRowCnt As Integer, _
                                 ByRef intColCnt As Integer, _
                                 ByVal strHIGHCUSTCODE As String) As Object     'XML  데이터 조회시

        Dim strSQL As String
        Dim strFormet, strSelFields, strWhere As String
        Dim strChkDate As String = ""
        Dim Con1, Con2, Con3, Con4, Con5 As String
        Dim vntData As Object

        With mobjSCGLConfig
            Try
                SetConfig(strInfoXML)   '기본정보 설정

                Con1 = ""
                Con2 = ""
                Con3 = ""
                Con4 = ""

                If strHIGHCUSTCODE <> "" Then Con1 = String.Format(" AND (HIGHCUSTCODE = '{0}')", strHIGHCUSTCODE)
                'If strMEDDIV <> "" Then Con2 = String.Format(" AND (MEDDIV = '{0}')", strMEDDIV)
                'If strCUSTNAME <> "" Then Con3 = String.Format(" AND (CUSTNAME LIKE '%{0}%')", strCUSTNAME)
                'If strBUSINO <> "" Then Con4 = String.Format(" AND (BUSINO LIKE '%{0}%')", strBUSINO)


                strWhere = BuildFields(" ", Con1, Con2, Con3, Con4)
                strSelFields = "0 CHK,CUSTNAME,CUSTCODE,DBO.MD_GET_REALMEDCODE_FUN(HIGHCUSTCODE) HIGHCUSTNAME,HIGHCUSTCODE,"
                strSelFields = strSelFields & "MED_TV,MED_RD,MED_DMB,MED_CATV,MED_PAP,MED_MAG,MED_NET,MED_OUT,MPP,DBO.MD_GET_CUSTNAME_FUN(MPP) MPPNAME,USE_FLAG"

                strFormet = "select {0} from SC_CUST_DTL where 1=1  {1} "

                strSQL = String.Format(strFormet, strSelFields, strWhere)

                .mobjSCGLSql.SQLConnect(.DBConnStr)
                vntData = .mobjSCGLSql.SQLSelectArr(strSQL, intRowCnt, intColCnt, , True)

                Return vntData
            Catch err As Exception
                Throw RaiseSysErr(err, CLASS_NAME & ".SelectRtn")
            Finally
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function

    Public Function ProcessRtn_DTL(ByVal strInfoXML As String, _
                                   ByVal vntData As Object, _
                                   ByVal strHIGHCUSTCODE As String) As Object                          '데이터 INSERT/UPDATE


        Dim intRtn As Integer
        Dim i, intColCnt, intRows As Integer
        Dim strCREDAY As String
        Dim strYEARMON As String
        Dim dblCUSTSEQ As String



        SetConfig(strInfoXML)
        With mobjSCGLConfig
            Try
                .mobjSCGLSql.SQLConnect(.DBConnStr)
                .mobjSCGLSql.SQLBeginTrans()
                If IsArray(vntData) Then
                    '''사용할 Entity 개체생성(Config 정보를 넘겨생성)
                    mobjceSC_CUST_DTL = New ceSC_CUST_DTL(mobjSCGLConfig)
                    '''vntData의 컬럼수, 로우수를 변수입력
                    intColCnt = UBound(vntData, 1) : intRows = UBound(vntData, 2)
                    For i = 1 To intRows
                        strYEARMON = ""
                        strCREDAY = ""
                        If GetElement(vntData, "CUSTCODE", intColCnt, i, OPTIONAL_STR) = "" Then
                            ' dblCUSTSEQ = SelectRtn_CUSTCODE(Flag)
                            'strCREDAY = Mid(GetElement(vntData, "CREDAY", intColCnt, i, OPTIONAL_STR), 1, 4) & Mid(GetElement(vntData, "CREDAY", intColCnt, i, OPTIONAL_STR), 6, 2) & Mid(GetElement(vntData, "CREDAY", intColCnt, i, OPTIONAL_STR), 9, 2)
                            'strYEARMON = Mid(strCREDAY, 1, 6)
                            'intRtn = InsertRtn_SC_CUST_DTL(vntData, intColCnt, i, dblCUSTSEQ, strHIGHCUSTCODE, Flag)
                        Else

                            strCREDAY = Mid(GetElement(vntData, "CREDAY", intColCnt, i, OPTIONAL_STR), 1, 4) & Mid(GetElement(vntData, "CREDAY", intColCnt, i, OPTIONAL_STR), 6, 2) & Mid(GetElement(vntData, "CREDAY", intColCnt, i, OPTIONAL_STR), 9, 2)
                            strYEARMON = Mid(strCREDAY, 1, 6)
                            intRtn = UpdateRtn_SC_CUST_DTL(vntData, intColCnt, i)
                        End If
                    Next
                End If
                .mobjSCGLSql.SQLCommitTrans()
                Return intRows
            Catch err As Exception
                .mobjSCGLSql.SQLRollbackTrans()
                Throw RaiseSysErr(err, CLASS_NAME & ".ProcessRtn")
            Finally
                .mobjSCGLSql.SQLDisconnect()
                mobjceSC_CUST_DTL.Dispose()
            End Try
        End With
    End Function


    ' =============== SelectRtnSample Code
    Public Function GetCUST_ID(ByVal strInfoXML As String, _
                              ByRef intRowCnt As Integer, _
                              ByRef intColCnt As Integer, _
                              ByVal strCUSTNAME As String, _
                              ByVal strMEDFLAG As String) As Object

        Dim strSQL As String
        Dim strFormet, strSelFields, strWhere As String
        Dim strChkDate As String = ""
        Dim Con1, Con2 As String
        Dim vntData As Object

        With mobjSCGLConfig
            Try
                SetConfig(strInfoXML)   '기본정보 설정

                Con1 = ""
                Con2 = ""

                If strCUSTNAME <> "" Then Con1 = String.Format(" AND (CUSTNAME LIKE '%{0}%')", strCUSTNAME)
                If strMEDFLAG <> "" Then Con2 = String.Format(" AND (MEDFLAG= '{0}')", strMEDFLAG)

                strWhere = BuildFields(" ", Con1, Con2)

                strSelFields = "CUSTCODE , CUSTNAME, BUSINO"

                strFormet = "select {0} from SC_CUST_TEMP where 1=1 {1}  ORDER BY CUSTNAME"


                strSQL = String.Format(strFormet, strSelFields, strWhere)

                .mobjSCGLSql.SQLConnect(.DBConnStr)
                vntData = .mobjSCGLSql.SQLSelectArr(strSQL, intRowCnt, intColCnt)
                Return vntData
            Catch err As Exception
                Throw RaiseSysErr(err, CLASS_NAME & ".GetCUSTNO")
            Finally
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function

    Public Function Get_Highcustcode(ByVal strInfoXML As String, _
                                     ByRef intRowCnt As Integer, _
                                     ByRef intColCnt As Integer, _
                                     ByVal strBUSINO As String) As Object

        Dim strSQL As String
        Dim strFormet, strSelFields, strWhere As String
        Dim strChkDate As String = ""
        Dim Con1, Con2 As String
        Dim vntData As Object

        With mobjSCGLConfig
            Try
                SetConfig(strInfoXML)   '기본정보 설정

                Con1 = ""

                If strBUSINO <> "" Then Con1 = String.Format(" AND (BUSINO = '{0}')", strBUSINO)
                strWhere = BuildFields(" ", Con1)

                strSelFields = "HIGHCUSTCODE, COMPANYNAME"

                strFormet = "select {0} from SC_CUST_TEMP where 1=1  AND ATTR10 =1 {1} GROUP BY HIGHCUSTCODE, COMPANYNAME  "


                strSQL = String.Format(strFormet, strSelFields, strWhere)

                .mobjSCGLSql.SQLConnect(.DBConnStr)
                vntData = .mobjSCGLSql.SQLSelectArr(strSQL, intRowCnt, intColCnt)
                Return vntData
            Catch err As Exception
                Throw RaiseSysErr(err, CLASS_NAME & ".Get_Highcustcode")
            Finally
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function



    Public Function GetInfoTosprSht(ByVal strInfoXML As String, _
                              ByRef intRowCnt As Integer, _
                              ByRef intColCnt As Integer, _
                              ByVal strCUSTCODE As String) As String ' xml 리턴은 스트링 스프레드는 object 


        Dim strCols As String         '컬럼변수
        Dim strWhere As String       'Where조건 변수
        Dim strFormat As String      'SQL Format 변수
        Dim strSQL As String          'SQL 변수
        Dim vntData As Object        'Array Return 변수(Array 를사용할 때 선언)
        Dim strXMLData As String    'XML  Return 변수(XML  을 사용할 때 선언)

        strCols = "OLDCUSTCODE,CUSTCODE,HIGHCUSTCODE,ACCUSTCODE,CUSTNAME,CUSTTYPE,MEDFLAG,COMPANYNAME,CUSTOWNER,REGNOTYPE,BUSINO,JURENTNO,BANKCODE,BANKACCNO,DEPNAME,BUSISTAT,BUSITYPE,ZIPCODE,ADDRESS1,ADDRESS2,TEL,FAX, DIVFLAG,ATTR06,DBO.MD_GET_REALMEDCODE_FUN(HIGHCUSTCODE) HIGHCUSTNAME, ATTR10, MEMO,MEDDIV,DEMANDFLAG,MPP,DBO.MD_GET_CUSTNAME_FUN(MPP) MPPNAME"
        strWhere = String.Format("WHERE CUSTCODE = '{0}' ", strCUSTCODE)
        strFormat = "SELECT {0} from SC_CUST_TEMP {1}"

        SetConfig(strInfoXML) '기본정보 Setting
        With mobjSCGLConfig '기본정보 Config 개체
            strSQL = String.Format(strFormat, strCols, strWhere)
            Try
                ' DB 접속
                .mobjSCGLSql.SQLConnect(.DBConnStr)

                ' ------ XML 데이터 조회
                strXMLData = .mobjSCGLSql.SQLSelectXml(strSQL, intRowCnt, intColCnt)
                Return strXMLData
            Catch err As Exception
                ' 오류 전송
                Throw RaiseSysErr(err, CLASS_NAME & ".SelectRtn")
            Finally
                ' DB 접속 종료
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function

    Public Function GetInfoToNew(ByVal strInfoXML As String, _
                                 ByRef intRowCnt As Integer, _
                                 ByRef intColCnt As Integer, _
                                 ByVal strCUSTCODE As String) As String ' xml 리턴은 스트링 스프레드는 object 


        Dim strCols As String         '컬럼변수
        Dim strWhere As String       'Where조건 변수
        Dim strFormat As String      'SQL Format 변수
        Dim strSQL As String          'SQL 변수
        Dim vntData As Object        'Array Return 변수(Array 를사용할 때 선언)
        Dim strXMLData As String    'XML  Return 변수(XML  을 사용할 때 선언)

        strCols = "CUSTOWNER,COMPANYNAME,BUSISTAT, " & _
                  "BUSITYPE,TEL,FAX,ZIPCODE, " & _
                  "ADDRESS1,ADDRESS2,JURENTNO,BANKCODE, " & _
                  "DEPNAME,BANKACCNO,HIGHCUSTCODE, DBO.MD_GET_REALMEDCODE_FUN(HIGHCUSTCODE) HIGHCUSTNAME, BUSINO  "

        strWhere = String.Format("WHERE CUSTCODE = '{0}' ", strCUSTCODE)
        strFormat = "SELECT {0} from SC_CUST_TEMP {1}"

        SetConfig(strInfoXML) '기본정보 Setting
        With mobjSCGLConfig '기본정보 Config 개체
            strSQL = String.Format(strFormat, strCols, strWhere)
            Try
                ' DB 접속
                .mobjSCGLSql.SQLConnect(.DBConnStr)

                ' ------ XML 데이터 조회
                strXMLData = .mobjSCGLSql.SQLSelectXml(strSQL, intRowCnt, intColCnt)
                Return strXMLData
            Catch err As Exception
                ' 오류 전송
                Throw RaiseSysErr(err, CLASS_NAME & ".SelectRtn")
            Finally
                ' DB 접속 종료
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function

    Public Function SelectRtn_CUSTCODE(ByVal strMEDFLAG As String) As String
        '여기부터 단순조회
        Dim strSQL, strFormat, strRtn As String

        'SetConfig(strInfoXML) '기본정보 Setting

        With mobjSCGLConfig '기본정보 Config 개체

            Try
                ' ------ Array 데이터 조회 (True 일때 헤더정보 포함 조회(Sheet Data Binding 할 경우 사용), False 일때 데이터만 조회)
                strSQL = String.Format("select '{0}' + dbo.lpad(isnull(Max(substring(custcode,2,6)),0)+1,5,0) From SC_CUST_TEMP WHERE MEDFLAG =  '{1}'", strMEDFLAG, strMEDFLAG)
                strRtn = .mobjSCGLSql.SQLSelectOneScalar(strSQL)
                Return strRtn
                ' ------ XML 데이터 조회
                'strXMLData = .mobjSCGLSql.SQLSelectXml(strSQL, intRowCnt, intColCnt)
                'Return strXMLData
            Catch err As Exception
                ' 오류 전송
                Throw RaiseSysErr(err, CLASS_NAME & ".SelectRtn")
            Finally
                ' DB 접속 종료
                '.mobjSCGLSql.SQLDisconnect()
            End Try
        End With
        '여기까지 단순조회
    End Function

    '********************************************************
    ' GetDataType()  소득구분 콤보 select 처리
    '********************************************************
    Public Function GetDataType(ByVal strInfoXML As String, _
                                ByRef intRowCnt As Integer, ByRef intColCnt As Integer) As Object

        Dim strSQL, strFormat, strSelFields As String
        Dim vntData As Object

        SetConfig(strInfoXML)   '기본정보 설정

        '조회 필드 설정
        strSelFields = "CMCODE, CODENM"

        'SQL문 생성

        strFormat = "SELECT {0} " & _
                    "FROM SC_PDCDTEMP " & _
                    "WHERE SUBSTR(CMCODE,1,2) = 'MP' " & _
                    "AND SUBSTR(CMCODE,3,2) != '00' " & _
                    "ORDER BY CMCODE "

        With mobjSCGLConfig
            strSQL = String.Format(strFormat, strSelFields)

            ''데이터 조회
            Try
                .mobjSCGLSql.SQLConnect(.DBConnStr)
                vntData = .mobjSCGLSql.SQLSelectArr(strSQL, intRowCnt, intColCnt)
                Return vntData
            Catch err As Exception
                Throw RaiseSysErr(err, CLASS_NAME & ".GetDataType")
            Finally
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function

    '********************************************************
    ' GetDataType()  소득구분 콤보 select 처리
    '********************************************************
    Public Function GetDataType_All(ByVal strInfoXML As String, _
                                ByRef intRowCnt As Integer, ByRef intColCnt As Integer) As Object

        Dim strSQL, strFormat, strSelFields As String
        Dim vntData As Object

        SetConfig(strInfoXML)   '기본정보 설정

        '조회 필드 설정
        strSelFields = "CMCODE, CODENM"

        'SQL문 생성

        strFormat = " SELECT '', '전체' CODENM FROM DUAL UNION ALL SELECT {0} " & _
                    "FROM SC_PDCDTEMP " & _
                    "WHERE SUBSTR(CMCODE,1,2) = 'MP' " & _
                    "AND SUBSTR(CMCODE,3,2) != '00' "


        With mobjSCGLConfig
            strSQL = String.Format(strFormat, strSelFields)

            ''데이터 조회
            Try
                .mobjSCGLSql.SQLConnect(.DBConnStr)
                vntData = .mobjSCGLSql.SQLSelectArr(strSQL, intRowCnt, intColCnt)
                Return vntData
            Catch err As Exception
                Throw RaiseSysErr(err, CLASS_NAME & ".GetDataType")
            Finally
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function


    Public Function DeleteRtn(ByVal strInfoXML As String, ByVal strCUSTCODE As String) As Integer   '데이터 DELETE

        Dim intRtn_desc As Integer      'Return변수( 처리건수 또는 0 )
        Dim intRtn As Integer      'Return변수( 처리건수 또는 0 )

        SetConfig(strInfoXML)    '기본정보 Setting
        With mobjSCGLConfig    '기본정보 Config 개체
            Try
                ' 사용할Entity 개체생성(Config 정보를 넘겨생성)
                mobjceSC_CUST_HDR = New ceSC_CUST_HDR(mobjSCGLConfig)
                ' DB 접속 및 트랜잭션 시작
                .mobjSCGLSql.SQLConnect(.DBConnStr)
                .mobjSCGLSql.SQLBeginTrans()
                ' 엔티티 오브젝트의 Delete 메소드 호출
                intRtn = mobjceSC_CUST_HDR.DeleteDo(strCUSTCODE)
                ' 트랜잭션 Commit
                .mobjSCGLSql.SQLCommitTrans()
                Return intRtn
            Catch err As Exception
                '트랜잭션 RollBack 및 오류 전송
                .mobjSCGLSql.SQLRollbackTrans()
                Throw RaiseSysErr(err, CLASS_NAME & "DeleteRtn")
            Finally
                'DB접속 종료
                .mobjSCGLSql.SQLDisconnect()
                '사용한 Entity(개체Dispose)
                mobjceSC_CUST_HDR.Dispose()
            End Try
        End With


    End Function


    '============== ProcessRtn_DETAIL_ONE ( Sheet ) Sample Code
    Public Function ProcessRtn_DETAIL_ONE(ByVal strYEARMON As String, ByVal vntData As Object, ByVal strSEQ As Integer) As String 'ByVal strInfoXML As String,ByRef strIDs As String, _ '자동 ID를 사용할 때만 사용
        ''ByVal vntData As Object) As Integer '데이터 INSERT/UPDATE
        'Dim intRtn As Integer
        'Dim i, intColCnt, intRows As Integer
        'Dim dblID As Double '자동 ID 를사용할 때만 사용

        ''SetConfig(strInfoXML)
        'With mobjSCGLConfig
        '    Try
        '        If IsArray(vntData) Then
        '            '''사용할 Entity 개체생성(Config 정보를 넘겨생성)
        '            mobjceSC_CUST_DTL = New ceSC_CUST_DTL(mobjSCGLConfig)
        '            '''vntData의 컬럼수, 로우수를 변수입력
        '            intColCnt = UBound(vntData, 1) : intRows = UBound(vntData, 2)
        '            '''해당하는Row 만큼 Loop
        '            For i = 1 To intRows
        '                intRtn = InsertRtn_MD_BOOKING_DESC(strYEARMON, vntData, intColCnt, i, strSEQ)
        '                '새로 생성된 ID를 반환하기 위한 문자열 생성[By TaeHoKim 각 행생성시 시퀀스]
        '                'strIDs &= dblID.ToString & meFIELD_SEP
        '                ' strIDs &= strIDs & strKeyTemp & meFIELD_SEP
        '            Next
        '        End If
        '        '.mobjSCGLSql.SQLCommitTrans()
        '        Return intRows
        '    Catch err As Exception
        '        '.mobjSCGLSql.SQLRollbackTrans()
        '        Throw RaiseSysErr(err, CLASS_NAME & ".ProcessRtn_DETAIL_ONE")
        '    Finally
        '        '.mobjSCGLSql.SQLDisconnect()
        '        mobjceSC_CUST_DTL.Dispose()
        '    End Try
        'End With
    End Function

    '============== ProcessRtn4_MASTER_TABLE ( XML ) Sample Code 
    Public Function ProcessRtn_MASTER_TABLE(ByVal vntData As Object, _
                                            ByRef strSEQFLAG As String, _
                                            ByRef strHIGHCUSTCODE As String) As Integer

        Dim intRtn As Integer
        Dim intRowCnt As Integer
        Dim intColCnt As Integer
        Dim strMEDFLAG As String
        Dim strSEQNO
        Dim intRows, i
        Dim strCOMPANYNAME
        With mobjSCGLConfig
            Try
                mobjceSC_CUST_DTL = New ceSC_CUST_DTL(mobjSCGLConfig)
                mobjceSC_CUST_HDR = New ceSC_CUST_HDR(mobjSCGLConfig)


                If strSEQFLAG = "New" Or strSEQFLAG = "Newbrand" Then
                    'strMEDFLAG = XMLGetElement(xmlRoot, "MEDFLAG")
                    'strCUSTCODE = SelectRtn_CUSTCODE(strMEDFLAG)

                    'If XMLGetElement(xmlRoot, "HIGHCUSTCODE") = "" Then
                    '    strHIGHCUSTCODE = strCUSTCODE
                    'Else
                    '    strHIGHCUSTCODE = XMLGetElement(xmlRoot, "HIGHCUSTCODE")
                    'End If
                    'If XMLGetElement(xmlRoot, "COMPANYNAME") = "" Then
                    '    strCOMPANYNAME = XMLGetElement(xmlRoot, "CUSTNAME")
                    'Else
                    '    strCOMPANYNAME = XMLGetElement(xmlRoot, "COMPANYNAME")
                    'End If
                    'intRtn = InsertRtn_SC_CUST_TEMP(xmlRoot, strCUSTCODE, strDIVFLAG, strHIGHCUSTCODE, strUSE, strDEMANDFLAG, strCOMPANYNAME)

                    'If strFlag = "Newbrand" Then
                    '    If strMEDFLAG = "A" Then
                    '        strSEQNO = SelectRtn_SEQNO()
                    '        intRtn = InsertRtn_SC_JOBCUST(xmlRoot, strSEQNO, strHIGHCUSTCODE)
                    '    End If
                    'End If
                Else
                    intColCnt = UBound(vntData, 1) : intRows = UBound(vntData, 2)

                    For i = 1 To intRows
                        intRtn = UpdateRtn_SC_CUST_HDR(vntData, intColCnt, i)
                        intRtn = STOP_UpdateRtn_SC_CUST_DTL(vntData, intColCnt, i)
                    Next



                End If

                '.mobjSCGLSql.SQLCommitTrans()
                Return intRows
            Catch err As Exception
                '.mobjSCGLSql.SQLRollbackTrans()
                Throw RaiseSysErr(err, CLASS_NAME & ".ProcessRtn_MASTER_TABLE")
            Finally
                '.mobjSCGLSql.SQLDisconnect()
                mobjceSC_CUST_DTL.Dispose()
            End Try
        End With

    End Function


    ' ============== ProcessRtn (Master & Detail) Sample Code 
    Public Function ProcessRtn(ByVal strInfoXML As String, _
                               ByVal vntData As Object, _
                               ByRef strSEQFLAG As String, _
                               ByVal strHIGHCUSTCODE As String) As Integer '데이터 INSERT/UPDATE 인수는 마지막에만 ByVal 이며 중간 경유는 ByRef

        Dim intRtn As Integer '결과값 변수
        Dim i, intColCnt, intRows As Integer '루프, 컬럼Cnt, 로우Cnt 변수

        SetConfig(strInfoXML) '기본정보 Setting

        With mobjSCGLConfig '기본정보를 가지고 있는 Config 개체
            Try
                'XML Element 변수 선언 (strMasterXML을 변환)


                'DB접속 및 트랜잭션 시작
                .mobjSCGLSql.SQLConnect(.DBConnStr)
                .mobjSCGLSql.SQLBeginTrans()
                'Master 데이터 처리
                intRtn = ProcessRtn_MASTER_TABLE(vntData, strSEQFLAG, strHIGHCUSTCODE)
                If intRtn = 9 Then
                    Return 9
                End If

                'DetailOne 데이터 처리
                'If IsArray(vntData) Then
                '    intRtn = ProcessRtn_DETAIL_ONE(strYEARMON, vntData, strSEQ)
                'End If

                'DetailTwo 데이터 처리
                'If IsArray(vntDataTwo) Then _
                'intRtn = ProcessRtn_DETAIL_TWO(dblHEAD_ID, vntDataTwo, strDETAIL_TWO_IDs)

                '트랜잭션Commit
                .mobjSCGLSql.SQLCommitTrans()
                Return intRtn
            Catch err As Exception
                '트랜잭션RollBack 및 오류 전송
                .mobjSCGLSql.SQLRollbackTrans()
                Throw RaiseSysErr(err, CLASS_NAME & ".ProcessRtn")
            Finally
                'Resource해제
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function

    ' ============== ProcessRtn PROJECT 코드 등록
    Public Function ProcessRtn_exe(ByVal strInfoXML As String, _
                               ByVal strMasterXML As String, _
                               ByVal strSEQFLAG As String) As Integer '데이터 INSERT/UPDATE
        Dim intRtn As Integer '결과값 변수
        Dim i, intColCnt, intRows As Integer '루프, 컬럼Cnt, 로우Cnt 변수

        SetConfig(strInfoXML) '기본정보 Setting
        With mobjSCGLConfig '기본정보를 가지고 있는 Config 개체
            Try
                'XML Element 변수 선언 (strMasterXML을 변환)
                Dim xmlRoot As XmlElement
                xmlRoot = XMLGetRoot(strMasterXML) 'XML 데이터

                'DB접속 및 트랜잭션 시작
                .mobjSCGLSql.SQLConnect(.DBConnStr)
                .mobjSCGLSql.SQLBeginTrans()
                'Master 데이터 처리
                intRtn = ProcessRtn_Seq(xmlRoot, strSEQFLAG)

                .mobjSCGLSql.SQLCommitTrans()
                Return intRtn
            Catch err As Exception
                '트랜잭션RollBack 및 오류 전송
                .mobjSCGLSql.SQLRollbackTrans()
                Throw RaiseSysErr(err, CLASS_NAME & ".ProcessRtn")
            Finally
                'Resource해제
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function

    '============== ProcessRtn PROJECT 신규 실제 저장 로직
    Public Function ProcessRtn_Seq(ByVal xmlRoot As XmlElement, _
                                   ByVal strSEQFLAG As String) As Integer
        Dim strPONO
        Dim intRtn
        Dim strCUSTCODE As String
        Dim strYEAR As String
        Dim strCREDAY As String

        With mobjSCGLConfig

            Try
                If XMLGetElement(xmlRoot, "CREDAY") <> "" Then strCREDAY = XMLGetElement(xmlRoot, "CREDAY").Substring(0, 4) & XMLGetElement(xmlRoot, "CREDAY").Substring(5, 2) & XMLGetElement(xmlRoot, "CREDAY").Substring(8, 2)
                ''' 사용할 Entity 개체생성(Config 정보를 넘겨생성)
                mobjceSC_CUST_DTL = New ceSC_CUST_DTL(mobjSCGLConfig)


                strYEAR = Mid(XMLGetElement(xmlRoot, "CREDAY"), 3, 2)
                If strSEQFLAG = "new" Then
                    'strPONO = SelectRtn_SEQNO(strYEAR)
                    intRtn = InsertRtn_SC_CUST_TEMP(xmlRoot, strCUSTCODE, 1, 1, 1, 1, 1)
                Else

                    intRtn = UpdateRtn_SC_CUST_TEMP(xmlRoot, "", "", "", "")

                End If

                Return intRtn
            Catch err As Exception

                Throw RaiseSysErr(err, CLASS_NAME & ".ProcessRtn_Seq")
            Finally
                mobjceSC_CUST_DTL.Dispose()
            End Try
        End With
    End Function


    '============== ProcessRtn PROJECT CODE 생성
    Public Function SelectRtn_SEQNO(ByVal strYEAR As String) As String
        '여기부터 단순조회
        Dim strSQL, strFormat, strRtn As String
        'SetConfig(strInfoXML) '기본정보 Setting

        With mobjSCGLConfig '기본정보 Config 개체

            Try
                strSQL = "SELECT 'G'+'" & strYEAR & "'+DBO.LPAD(ISNULL(MAX(CAST(SUBSTRING(PROJECTNO,4,3) AS NUMERIC(4,0))),0)+1,3,'0') FROM PD_PONO WHERE SUBSTRING(PROJECTNO,2,2) = '" & strYEAR & "'"
                strRtn = .mobjSCGLSql.SQLSelectOneScalar(strSQL)
                Return strRtn
            Catch err As Exception
                ' 오류 전송
                Throw RaiseSysErr(err, CLASS_NAME & ".SelectRtn_SEQNO")
            Finally
            End Try
        End With
        '여기까지 단순조회
    End Function




    Public Function SelectRtn_SEQNO() As String
        '여기부터 단순조회
        Dim strSQL, strFormat, strRtn As String

        'SetConfig(strInfoXML) '기본정보 Setting

        With mobjSCGLConfig '기본정보 Config 개체

            Try
                strSQL = String.Format("select DBO.LPAD(LTRIM(STR(ISNULL(max(convert(int,SEQNO)),0)+1)),6,'0') from sc_jobcust")
                strRtn = .mobjSCGLSql.SQLSelectOneScalar(strSQL)
                Return strRtn
            Catch err As Exception
                ' 오류 전송
                Throw RaiseSysErr(err, CLASS_NAME & ".SelectRtn_SEQNO")
            Finally
            End Try
        End With
        '여기까지 단순조회
    End Function

    Public Function CUSTNAME_CHECKED(ByVal strInfoXML As String, _
                                     ByRef intRowCnt As Integer, _
                                     ByRef intColCnt As Integer, _
                                     ByVal strBUSINO As String, _
                                     ByVal strMEDFLAG As String, _
                                     ByVal strCUSTNAME As String) As Object

        Dim strSQL As String            'SQL문
        Dim strFormat As String         '임시 SQL문
        Dim strSelFields As String      '조회필드
        Dim strWhere As String
        Dim vntData As Object
        Dim Con1 As String
        Dim Con2 As String
        Dim Con3 As String

        SetConfig(strInfoXML)   '기본정보 설정
        With mobjSCGLConfig

            Con1 = ""
            Con2 = ""
            Con3 = ""

            If strBUSINO <> "" Then Con1 = String.Format(" AND (BUSINO = '{0}')", strBUSINO)
            If strMEDFLAG <> "" Then Con2 = String.Format(" AND (MEDFLAG = '{0}')", strMEDFLAG)
            If strCUSTNAME <> "" Then Con3 = String.Format(" AND (RTRIM(LTRIM(CUSTNAME)) = '{0}')", Trim(strCUSTNAME))

            strWhere = BuildFields(" ", Con1, Con2, Con3)
            strFormat = "SELECT CUSTCODE, CUSTNAME FROM SC_CUST_TEMP WHERE 1=1 {0} "
            strSQL = String.Format(strFormat, strWhere)
            '데이터 조회
            Try
                .mobjSCGLSql.SQLConnect(.DBConnStr)
                vntData = .mobjSCGLSql.SQLSelectArr(strSQL, intRowCnt, intColCnt, , True)
                Return vntData
            Catch err As Exception
                Throw RaiseSysErr(err, CLASS_NAME & ".CUSTNAME_CHECKED")
            Finally
                .mobjSCGLSql.SQLDisconnect()
            End Try
        End With
    End Function


    Private Function UpdateRtn_SC_CUST_HDR(ByVal vntData As Object, _
                                           ByVal intColCnt As Integer, _
                                           ByVal intRow As Integer) As Integer
        Dim intRtn As Integer

        intRtn = mobjceSC_CUST_HDR.UpdateDo( _
                                       GetElement(vntData, "HIGHCUSTCODE", intColCnt, intRow), _
                                       GetElement(vntData, "ACCUSTCODE", intColCnt, intRow), _
                                       GetElement(vntData, "GREATCODE", intColCnt, intRow), _
                                       GetElement(vntData, "CUSTNAME", intColCnt, intRow), _
                                       GetElement(vntData, "COMPANYNAME", intColCnt, intRow), _
                                       GetElement(vntData, "CUSTTYPE", intColCnt, intRow), _
                                       GetElement(vntData, "MEDFLAG", intColCnt, intRow), _
                                       GetElement(vntData, "CUSTOWNER", intColCnt, intRow), _
                                       GetElement(vntData, "REGNOTYPE", intColCnt, intRow), _
                                       GetElement(vntData, "BUSINO", intColCnt, intRow), _
                                       GetElement(vntData, "JURENTNO", intColCnt, intRow), _
                                       GetElement(vntData, "BANKCODE", intColCnt, intRow), _
                                       GetElement(vntData, "BANKACCNO", intColCnt, intRow), _
                                       GetElement(vntData, "DEPNAME", intColCnt, intRow), _
                                       GetElement(vntData, "BUSISTAT", intColCnt, intRow), _
                                       GetElement(vntData, "BUSITYPE", intColCnt, intRow), _
                                       GetElement(vntData, "ZIPCODE", intColCnt, intRow), _
                                       GetElement(vntData, "ADDRESS1", intColCnt, intRow), _
                                       GetElement(vntData, "ADDRESS2", intColCnt, intRow), _
                                       GetElement(vntData, "TEL", intColCnt, intRow), _
                                       GetElement(vntData, "FAX", intColCnt, intRow), _
                                       GetElement(vntData, "OLDCUSTCODE", intColCnt, intRow), _
                                       GetElement(vntData, "MEMO", intColCnt, intRow), _
                                       GetElement(vntData, "DEMANDFLAG", intColCnt, intRow), _
                                       GetElement(vntData, "KOBACOCUSTCODE", intColCnt, intRow), _
                                       GetElement(vntData, "USE_FLAG", intColCnt, intRow), _
                                       GetElement(vntData, "ATTR01", intColCnt, intRow), _
                                       GetElement(vntData, "ATTR02", intColCnt, intRow), _
                                       GetElement(vntData, "ATTR03", intColCnt, intRow), _
                                       GetElement(vntData, "ATTR04", intColCnt, intRow), _
                                       GetElement(vntData, "ATTR05", intColCnt, intRow), _
                                       GetElement(vntData, "ATTR06", intColCnt, intRow, NULL_NUM, True), _
                                       GetElement(vntData, "ATTR07", intColCnt, intRow, NULL_NUM, True), _
                                       GetElement(vntData, "ATTR08", intColCnt, intRow, NULL_NUM, True), _
                                       GetElement(vntData, "ATTR09", intColCnt, intRow, NULL_NUM, True), _
                                       GetElement(vntData, "ATTR10", intColCnt, intRow, NULL_NUM, True))

        Return intRtn
    End Function

    Private Function STOP_UpdateRtn_SC_CUST_DTL(ByVal vntData As Object, _
                                                ByVal intColCnt As Integer, _
                                                ByVal intRow As Integer) As Integer
        Dim intRtn As Integer

        'intRtn = mobjceSC_CUST_DTL.STOP_UpdateDo( _
        '                               GetElement(vntData, "HIGHCUSTCODE", intColCnt, intRow), _
        '                               GetElement(vntData, "USE_FLAG", intColCnt, intRow))

        Return intRtn
    End Function

    Private Function UpdateRtn_SC_CUST_DTL(ByVal vntData As Object, _
                                           ByVal intColCnt As Integer, _
                                           ByVal intRow As Integer) As Integer
        Dim intRtn As Integer

        intRtn = mobjceSC_CUST_DTL.UpdateDo( _
                                       GetElement(vntData, "CUSTCODE", intColCnt, intRow), _
                                       GetElement(vntData, "HIGHCUSTCODE", intColCnt, intRow), _
                                       GetElement(vntData, "CUSTNAME", intColCnt, intRow), _
                                       GetElement(vntData, "MEDFLAG", intColCnt, intRow), _
                                       GetElement(vntData, "MED_TV", intColCnt, intRow), _
                                       GetElement(vntData, "MED_RD", intColCnt, intRow), _
                                       GetElement(vntData, "MED_DMB", intColCnt, intRow), _
                                       GetElement(vntData, "MED_CATV", intColCnt, intRow), _
                                       GetElement(vntData, "MED_PAP", intColCnt, intRow), _
                                       GetElement(vntData, "MED_MAG", intColCnt, intRow), _
                                       GetElement(vntData, "MED_NET", intColCnt, intRow), _
                                       GetElement(vntData, "MED_OUT", intColCnt, intRow), _
                                       GetElement(vntData, "MED_ETC", intColCnt, intRow), _
                                       GetElement(vntData, "MPP", intColCnt, intRow), _
                                       GetElement(vntData, "USE_FLAG", intColCnt, intRow))

        Return intRtn
    End Function


#End Region

#Region "GROUP BLOCK : 외부에 비공개 Method"
    Private Function InsertRtn_SC_CUST_TEMP(ByVal xmlRoot As XmlElement, _
                                            ByVal strCUSTCODE As String, _
                                            ByVal strDIVFLAG As String, _
                                            ByVal strHIGHCUSTCODE As String, _
                                            ByVal strUSE As Integer, _
                                            ByVal strDEMANDFLAG As String, _
                                            ByVal strCOMPANYNAME As String) As Integer
        Dim intRtn As Integer
        intRtn = mobjceSC_CUST_DTL.InsertDo( _
                                       strCUSTCODE, _
                                       strHIGHCUSTCODE, _
                                       XMLGetElement(xmlRoot, "CUSTNAME"), _
                                       XMLGetElement(xmlRoot, "CUSTTYPE"), _
                                       XMLGetElement(xmlRoot, "MEDFLAG"), _
                                       XMLGetElement(xmlRoot, "BUSINO"), _
                                       strDIVFLAG, _
                                       XMLGetElement(xmlRoot, "MEMO"), _
                                       XMLGetElement(xmlRoot, "CUSTOWNER"), _
                                       strCOMPANYNAME, _
                                       XMLGetElement(xmlRoot, "BUSISTAT"), _
                                       XMLGetElement(xmlRoot, "BUSITYPE"), _
                                       XMLGetElement(xmlRoot, "TEL"), _
                                       XMLGetElement(xmlRoot, "FAX"), _
                                       XMLGetElement(xmlRoot, "ZIPCODE"), _
                                       XMLGetElement(xmlRoot, "ADDRESS1"), _
                                       XMLGetElement(xmlRoot, "ADDRESS2"), _
                                       XMLGetElement(xmlRoot, "JURENTNO"), _
                                       XMLGetElement(xmlRoot, "BANKCODE"), _
                                       XMLGetElement(xmlRoot, "DEPNAME"), _
                                       XMLGetElement(xmlRoot, "BANKACCNO"), _
                                       XMLGetElement(xmlRoot, "MEDDIV"), _
                                       strDEMANDFLAG, _
                                       XMLGetElement(xmlRoot, "MPP"), _
                                       XMLGetElement(xmlRoot, "ATTR01"), _
                                       XMLGetElement(xmlRoot, "ATTR02"), _
                                       XMLGetElement(xmlRoot, "ATTR03"), _
                                       XMLGetElement(xmlRoot, "ATTR04"), _
                                       XMLGetElement(xmlRoot, "ATTR05"), _
                                       XMLGetElement(xmlRoot, "ATTR06", NULL_NUM, True), _
                                       XMLGetElement(xmlRoot, "ATTR07", NULL_NUM, True), _
                                       XMLGetElement(xmlRoot, "ATTR08", NULL_NUM, True), _
                                       XMLGetElement(xmlRoot, "ATTR09", NULL_NUM, True), _
                                       strUSE)
        Return intRtn
    End Function
    Private Function UpdateRtn_SC_CUST_TEMP(ByVal xmlRoot As XmlElement, _
                                            ByVal strCUSTCODE As String, _
                                            ByVal strDIVFLAG As String, _
                                            ByVal strUSE As Integer, _
                                            ByVal strDEMANDFLAG As String) As Integer
        Dim intRtn As Integer

        intRtn = mobjceSC_CUST_DTL.UpdateDo( _
                                       strCUSTCODE, _
                                       XMLGetElement(xmlRoot, "HIGHCUSTCODE"), _
                                       XMLGetElement(xmlRoot, "CUSTNAME"), _
                                       XMLGetElement(xmlRoot, "CUSTTYPE"), _
                                       XMLGetElement(xmlRoot, "MEDFLAG"), _
                                       XMLGetElement(xmlRoot, "BUSINO"), _
                                       strDIVFLAG, _
                                       XMLGetElement(xmlRoot, "MEMO"), _
                                       XMLGetElement(xmlRoot, "MEDDIV"), _
                                       strDEMANDFLAG, _
                                       XMLGetElement(xmlRoot, "MPP"), _
                                       strUSE)
        Return intRtn
    End Function

    ' Private Function InsertRtn_XXX(ByVal vntData As Object,ByVal intColCnt As Integer, ByVal intRow As Integer) As Integer
    '     '해당 엔티티 클래스Maker에서 기본 코드 제공(Sheet)
    ' End Function
    ' Private Function UpdateRtn_XXX(ByVal vntData As Object,ByVal intColCnt As Integer, ByVal intRow As Integer) As Integer
    '     '해당 엔티티 클래스Maker에서 기본코드 제공(Sheet)
    ' End Function

#End Region
End Class



